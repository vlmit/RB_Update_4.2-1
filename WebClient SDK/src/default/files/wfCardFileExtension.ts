import { TypedField, ValidationResultBuilder } from '@tessa/core';
...
import { unseal } from 'tessa/platform/sealable';
import { FileViewModel } from 'tessa/ui/cards/controls';

@extension({ name: 'WfCardFileExtension' })
export class WfCardFileExtension extends FileExtension {
  ...
  private static async createCopyInMainCard(
    ...
    for (const fileVM of files) {
      ...
      if (!file.category || file.category.id !== 'ef065661-6613-4c87-bf93-0e1dd558a751') {
        ...
        try {
          ...
          copiedFile.info['CopiedToMainCard'] = TypedField.trueBoolean;
    ...
    if (result.isSuccessful) {
      for (const files of copiedFileList) {
