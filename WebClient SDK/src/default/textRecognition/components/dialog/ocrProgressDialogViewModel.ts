...
import { ValidationKeys, ValidationResult, ValidationResultType } from 'tessa/platform/validation';
...
export class OcrProgressDialogViewModel {
   ...
   * @param {string} operationId Идентификатор отслеживаемой операции.
   ...
   */
  constructor(operationId: string, timeout: number, progress = 0, canCancel = true) {
    ...
    const handler = async () => {
      ...
      if (!operation || operation.progress == null || this.isOperationCompleted(operation)) {
        try {
          ...
          // Для завершенной операции также может существовать карточка с ошибкой.
          ...
          } else if (!result.items?.some(i => i.key === ValidationKeys.InstanceNotFound)) {
