...
export class OcrDateTimeValidator extends OcrValidator {
  ...
  protected compileValue(match: RegExpExecArray): string {
    if (match.groups) {
      let hours = +(this.removeSpaces(match.groups['hours'] ?? '') || '00');
      if (this.removeSpaces(match.groups['am'] ?? '')) {
        hours = hours >= 12 ? hours - 12 : hours;
      } else if (this.removeSpaces(match.groups['pm'] ?? '')) {
        hours = hours < 12 ? hours + 12 : hours;
      }
  ...
  validate(value: string): OcrValidationResult {
    ...
    for (const pattern of this.patterns) {
      ...
      if (match && match.length > 0) {
        ...
        const parsedValue = Moment(compiledValue, OcrDateTimeValidator._format, true);
        if (parsedValue.isValid() && DateHelper.validateDate(parsedValue)) {
          const formattedValue = this.formatDate(parsedValue);
          let validationResult: ValidationResult | null = null;
          if (this.normalize(formattedValue) != value) {
