...
export class OnlyOfficeEditorPreviewComponent extends React.PureComponent<OnlyOfficeEditorPreviewComponentProps> {
  ...
  public render(): React.ReactElement {
     ...
     */
    return (
      <div className="pdfViewerP7" ref={this._placeholderContainerRef}>
  ...
  private attachViewModelListeners = () => {
    ...
    const startDisposer = viewModel.previewManager.previewArea?.onResizeStart.add(
      this.resizerStartedHandler
    );
    if (startDisposer) {
      this._disposers.push(startDisposer);
    }

    const endDisposer = viewModel.previewManager.previewArea?.onResizeEnd.add(
      this.resizerFinishedHandler
    );
    if (endDisposer) {
      this._disposers.push(endDisposer);
    }
  };
