...
export class CreateAndSelectToolbarUIExtension extends CardUIExtension {
  public initialized(context: ICardUIExtensionContext): void {
    ...
    const uiContext = CreateAndSelectToolbarUIExtension.tryGetParentSelectionViewContext(
      UIContext.current
    );
    if (!uiContext || !uiContext.info) {
      return;
    }
    ...
    if (!toolbar.items.find(x => x.name === 'SaveAndSelect')) {
      ...
      toolbar.addItemIfNotExists(saveAndCloseItem, {
        name: 'Ctrl+Shift+S',
        key: 'KeyS',
