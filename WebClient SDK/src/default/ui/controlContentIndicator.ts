import { CardFieldChangedEventArgs, FieldMapStorage, CardControlTypes } from 'tessa/cards';
import { CardTypeEntryControl } from 'tessa/cards/types';
import { LocalizationManager } from 'tessa/localization';
import { IControlViewModel, IBlockViewModel, BreadthFirstControlVisitor } from 'tessa/ui/cards';
...
export class ControlContentIndicatorVisitor extends BreadthFirstControlVisitor {
  ...
  protected visitControl(control: IControlViewModel): void {
    ...
    if (
      ...
      cardTypeControl.type.name === CardControlTypes.StringControlType.name
    ) {
...
export abstract class ControlContentIndicator<T> {
  ...
  private readonly _cardFieldContainer: FieldMapStorage;
  ...
  protected abstract visitControl(visitor: ControlContentIndicatorVisitor, control: T): void;

  protected abstract getDisplayName(control: T): string;

  protected abstract setDisplayName(control: T, name: string): void;
  ...
  private checkContent(index: number): boolean {
    ...
    for (const field of this._controlFieldsMapping[index]) {
      if (!!this._cardFieldContainer.get(field)) {
        return true;
      }
    }
