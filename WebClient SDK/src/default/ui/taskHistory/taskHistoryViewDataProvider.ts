...
import { CardMetadataHelper, CardMetadataRuntimeType } from 'tessa/cards/metadata';
import { SchemeDbType } from 'tessa/platform';
import { compareDates } from 'tessa/platform/formatting';

export class TaskHistoryViewDataProvider implements IViewControlDataProvider {
  //#region ctor
  ...
  async getDataAsync(
    ...
    if (request.sortingColumns.length === 1) {
      ...
      const isDateTimeColumn = column
        ? CardMetadataHelper.getRuntimeTypeFromDbType(column[1].dbType) ===
          CardMetadataRuntimeType.DateTime
        : false;
