...
export class KrCardTasksEditorUIExtension extends CardUIExtension {
  ...
  private async doubleClickHandler(
    ...
    const token = param.criteriaValues[0].values[0]?.getValue<string>();
    if (!token) {
      return;
    }
    ...
    await LoadingOverlay.instance.show(async splashResolve => {
      ...
      try {
        ...
        await TaskAssignedRolesDialogHelper.showTaskAssignedRolesEditorDialog(
          ...
          async () => {
            const info = {
              '.TaskRowID': TypedField.createGuid(taskRowId),
              IgnoreButtonsInfoMark: TypedField.trueBoolean,
              '.IgnoreWorkflowEngineTiles': TypedField.trueBoolean
            };
