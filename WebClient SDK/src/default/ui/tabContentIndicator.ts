...
import { CardFieldChangedEventArgs, CardControlTypes } from 'tessa/cards';
...
export class TabContentIndicator {
  ...
  public fieldChangedAction = (e: CardFieldChangedEventArgs): void => {
...
class Visitor extends BreadthFirstControlVisitor {
  ...
  private _fieldIds: [string, string][];

  constructor(fieldTabMapping: Map<string, number>, fieldIds: [string, string][]) {
    super();
  ...
  protected visitControl(control: IControlViewModel): void {
    if (
      ...
      control.cardTypeControl.type === CardControlTypes.StringControlType
    ) {
      for (const colId of control.cardTypeControl.physicalColumnIdList) {
