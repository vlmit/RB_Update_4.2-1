...
import { createAppearanceString } from 'tessa/ui/uiHelper';
...
export class CardFilesDataProvider implements IViewControlDataProvider {
  ...
  protected addFieldDescriptions(result: ViewControlDataProviderResponse): void {
    result.columns.push(['GroupName', SchemeType.String]);
  ...
  protected populateDataRowsAsync(
    ...
    result.calculatedRowCount = rows.length;
    result.rows.push(...this.applyPaging(rows, requestParameters));
  }

  private applyPaging(
    rows: Array<IStorage>,
    requestParameters: ViewRequestParameter[]
  ): Array<IStorage> {
    if (requestParameters.length === 0) {
      return rows;
    }

    const pageOffsetParamName = 'PageOffset';
    const pageLimitParamName = 'PageLimit';
    const equalityOperatorName = 'Equality';
    ...
    for (const parameter of requestParameters) {
      switch (parameter.name) {
        case pageOffsetParamName:
          pageOffset = parameter.criteriaValues.find(x => x.criteriaName === equalityOperatorName)
            ?.values?.[0]?.value as number;
          break;
        case pageLimitParamName:
          pageLimit = parameter.criteriaValues.find(x => x.criteriaName === equalityOperatorName)
            ?.values?.[0]?.value as number;
          break;
      }
    }
    ...
    const begin = pageOffset - 1;
    return rows.slice(begin, begin + pageLimit);
  }
  ...
  protected mapFileToRow(file: FileViewModel): IStorage {
    ...
    return {
      ...
      Size: size,
      Appearance: file.isModified ? createAppearanceString(new Map([['FontWeight', 'Bold']])) : ''
    };
  }
}
    ...
    switch (criteria.criteriaName) {
      case 'Contains':
        filterBlock.criterialFilters.push(x =>
          x.description.toLocaleUpperCase().includes(valueAsString)
        );
        break;
      case 'Equality':
        filterBlock.criterialFilters.push(x => x.description.toLocaleUpperCase() === valueAsString);
        break;
      case 'StartWith':
        filterBlock.criterialFilters.push(x =>
          x.description.toLocaleUpperCase().startsWith(valueAsString)
        );
        break;
      case 'EndWith':
        filterBlock.criterialFilters.push(x =>
          x.description.toLocaleUpperCase().endsWith(valueAsString)
        );
        break;
    }
  }
