...
export class KrPermissionsMandatoryStoreExtension extends CardStoreExtension {
    ...
    if (
      ...
      const failRules: KrPermissionMandatoryRuleStorage[] = rules.map(x => {
        return {
          ...
          columnIds: (x['ColumnIDs'] as Array<TypedField<DotNetType.Guid, string>>).map(y =>
            getTypedFieldValue(y)
          )
        };
      });
      ...
      const taskItems =
        cardModel.mainForm instanceof DefaultFormTabWithTasksViewModel
          ? cardModel.mainForm.tasks
          : null;
      if (requestTasks.length > 0 && !!taskItems && taskItems.length > 0) {
        for (const task of requestTasks) {
          let taskItem: TaskViewModel | null = null;
          if (
            task.action === CardTaskAction.Complete &&
            (taskItem = taskItems.find(
              x => x instanceof TaskViewModel && x.taskModel.cardTask!.rowId === task.rowId
            )!)
          ) {
  ...
  private validateFromRules(cardModel: ICardModel, failRules: KrPermissionMandatoryRuleStorage[]) {
    for (const controlViewModel of cardModel.controlsBag) {
      ...
      if (
        ...
        failRules.some(
          x =>
            ...
            (x.columnIds.length === 0 ||
              sourceInfo.columnIds.some(y => x.columnIds.some(z => z === y)))
        )
      ) {
