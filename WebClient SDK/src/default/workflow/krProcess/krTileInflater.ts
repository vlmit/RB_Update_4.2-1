...
export class KrTileInflater {
  ...
  public inflate(
    ...
    for (const tileInfo of tileInfos) {
      tiles.push(this.inflateTile(contextSource, tileInfo, groups, true));
    }
  ...
  private inflateTile(
    ...
    let icon = getTessaIcon(tileInfo.icon);
    if (!icon && withDefaultIcon && !tileInfo.actionGrouping) {
      // Указываем иконку по умолчанию если она не была задана
      icon = getTessaIcon('Thin127');
    }
    ...
    if (tileInfo.nestedTiles.length !== 0) {
      ...
      for (const nestedTileInfo of tileInfo.nestedTiles) {
        nestedTiles.push(this.inflateTile(contextSource, nestedTileInfo, groups, false));
      }
