...
import { IRuntimeBlockViewModel, IRuntimeItemViewModel } from 'tessa/ui/formEditor/types';
...
export class WfResolutionTaskInfo {
  ...
  private static setControlVisibility(
    block: IBlockViewModel | IRuntimeBlockViewModel,
    suffix: string,
    isVisible: boolean
  ) {
    const setVisibility = (control: IRuntimeItemViewModel, isVisible: boolean) => {
      if (control instanceof ControlViewModelBase) {
    ...
    if (isRuntimeBlock(block)) {
      for (const control of block.iterate(child => child.alias?.endsWith(suffix) ?? false)) {
  ...
  private tryGetMainInfoBlock(): IBlockViewModel | IRuntimeBlockViewModel | null {
    const form = this.control.taskWorkspace.form;
    if (!form) {
      return this.control.taskWorkspace.wysiwygForm?.getRootBlock() ?? null;
    }
  ...
  private tryGetPerformersBlock(): IBlockViewModel | IRuntimeBlockViewModel | null {
    const form = this.control.taskWorkspace.form;
    if (!form) {
      return this.control.taskWorkspace.wysiwygForm?.getRootBlock() ?? null;
    }
  ...
  public subscribeToResolutionSectionAndUpdate(section: CardSection): void {
    this.unsubscribeFromResolutionSection();
  ...
  public unsubscribeFromResolutionSection(): void {
  ...
  private updateWithControl() {
    ...
    if (mainInfoBlock) {
      const withControl = !!this.resolutionSection.fields.tryGet('WithControl');
  ...
  private updateShowAdditionalControls() {
    ...
    const mainInfoBlock = this.tryGetMainInfoBlock();
    if (mainInfoBlock) {
      const showAdditional = !!this.resolutionSection.fields.tryGet('ShowAdditional');
  ...
  private updateMassCreation() {
    ...
    const massCreation = !!this.resolutionSection.fields.tryGet('MassCreation');
    if (!massCreation) {
  ...
  public subscribeToPerformersAndUpdate(performersRows: ArrayStorage<CardRow>): void {
    this.unsubscribeFromPerformers();
  ...
  public unsubscribeFromPerformers(): void {
