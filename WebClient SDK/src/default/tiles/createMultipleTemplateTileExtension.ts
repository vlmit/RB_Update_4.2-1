...
import { CardTypeFlags, CardTypeSealed } from 'tessa/cards/types';
import {
  tryGetFromInfo,
  UIContext,
  showNotEmpty,
...
import { Paging, ViewRowHelper } from 'tessa/views';
...
export class CreateMultipleTemplateTileExtension extends TileExtension {
  ...
  constructor(
    ...
    @inject(IViewSpecialParameters$) private _viewSpecialParameters: IViewSpecialParameters
  ) {
    super();
  }
    ...
    if (
      ...
      cardTools.tiles.push(
        new Tile({
          ...
          command: () => this.createMultipleCardsAction(),
  ...
  private static async getTypeInfoForMultipleCreation(cardType: CardTypeSealed): Promise<{
    hasPartner: boolean;
    hasAuthor: boolean;
  }> {
    await cardType.ensureDataLoaded();
  ...
  private async createMultipleCardsAction() {
    ...
    if (
      ...
      !(dialogType = cardMetadata.cardTypes.getCardTypeByName('Dialogs')!)
    ) {
      return;
    }

    await LoadingOverlayHelper.ensureCardTypeDataLoaded(dialogType);

    const dialogForm = dialogType.forms.find(x => x.name === 'CreateMultipleCards');
    if (!dialogForm) {
      return;
    }
    ...
    const model = await createCardModel(response.card, response.sectionRows);

    const mainFields = model.card.sections.get('Dialogs').fields;
    const { hasPartner, hasAuthor } =
      await CreateMultipleTemplateTileExtension.getTypeInfoForMultipleCreation(cardInTemplateType);
      ...
      [
        UIButton.create({
          ...
          buttonAction: async btn => {
            const cardCount = mainFields.get<number>('CardCount')!;
            ...
            if (
              ...
              const tuple = await this.tryCreateMultipleCardsAsync(
                cardCount,
                changePartner,
                changeAuthor,
                templateCard
          ...
          isEnabled: () => mainFields.get<number>('CardCount')! > 0
        }),
        UIButton.create({
  ...
  private async tryCreateMultipleCardsAsync(
    cardCount: number,
    ...
    await showLoadingOverlay(async () => {
      ...
      if (changeAuthor) {
        ...
        if (users) {
          const systemIndex = users.findIndex(x => x.id === '11111111-1111-1111-1111-111111111111'); // SystemID
          if (systemIndex >= 0) {
      ...
      try {
        for (let i = 0; i < cardCount; i++) {
          ...
          request.sourceCard = new Card(
            deserializeFromTypedToPlain<IStorage>(
              templateCard.sections.get('Templates').fields.get('Card')!
            )
          );
  ...
  private async tryLoadDirectory(
    viewAlias: string,
    entryPrefix: string
  ): Promise<DirectoryEntry[] | null> {
    const view = await this._viewRepository.getByName(viewAlias);
    if (!view) {
      await showError(
        LocalizationManager.instance.format(
    ...
    const viewRequest = new ViewRequest(viewMetadata);
    viewRequest.calculateRowCounting = false;

    let currentPage = 1;
    let pageLimit = viewMetadata.exportDataPageLimit;
    if (pageLimit <= 0) {
      pageLimit = ViewRowHelper.defaultExportDataPageLimit;
    }

    this._viewSpecialParameters.provideCurrentUserIdParameter(viewRequest.parameters);
    this._viewSpecialParameters.providePageLimitParameter(
      viewRequest.parameters,
      Paging.Always,
      pageLimit,
      false
    );
    ...
    while (true) {
      ...
      for (const rowValue of viewResult.rows) {
        const row = rowValue;
        if (
        ...
        const id = row[entryIdIndex] as string;
        const name = row[entryNameIndex] as string;
