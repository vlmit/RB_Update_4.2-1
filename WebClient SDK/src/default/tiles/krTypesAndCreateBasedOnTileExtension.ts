...
@extension({ name: 'KrTypesAndCreateBasedOnTileExtension' })
export class KrTypesAndCreateBasedOnTileExtension extends TileExtension {
  //#region ctor

  constructor(@inject(IKrTypesCache$) private readonly _typesCache: IKrTypesCache) {
    super();
  }
  ...
  async initializingGlobal(context: ITileGlobalExtensionContext): Promise<void> {
    await KrTypesAndCreateBasedOnTileExtension.initializeKrTilesGlobal(context, this._typesCache);
    await KrTypesAndCreateBasedOnTileExtension.initializeKrCreateBasedOnTiles(
      context,
      this._typesCache
    );
  }
  ...
  private static async initializeKrTilesGlobal(
    ...
    const unavailableTypes = typesCache.unavailableTypes;
    ...
    const types = (await typesCache.getTypes())
      .map(x => x)
    ...
    for (const groupTile of createCard.tiles) {
      ...
      const count = await KrTypesAndCreateBasedOnTileExtension.countTypes(
        typesCache,
        cardMetadata,
        groupTile.name
      );
      if (count > maxTilesCountInGroupTile) {
        ...
        for (const typeTile of tilesInGroup) {
          ...
          const cardType = cardMetadata.cardTypes.getCardTypeByName(typeTile.name);
          if (cardType) {
  ...
  private static async countTypes(
    cache: IKrTypesCache,
    ...
    let result = 0;
    const docTypes = await cache.getDocTypes();
    const unavailableTypes = cache.unavailableTypes;
    ...
    for (const cardType of metadata.cardTypes) {
      ...
      const components = await KrComponentsHelper.getKrComponentsTypesCache(cardType.id!, cache);
  ...
  private static async initializeKrCreateBasedOnTiles(
    context: ITileGlobalExtensionContext,
    typesCache: IKrTypesCache
  ): Promise<void> {
    ...
    if (count > maxTilesCountInGroupTile) {
      await KrTypesAndCreateBasedOnTileExtension.createBasedOnTilesAndGroup(
        context,
        typesCache,
        contextSource,
        createBasedOnTile,
      ...
      await KrTypesAndCreateBasedOnTileExtension.createBasedOnTilesWithoutGrouping(
        typesCache,
        contextSource,
        createBasedOnTile,
        false
  ...
  private static async createBasedOnTilesAndGroup(
    context: ITileGlobalExtensionContext,
    typesCache: IKrTypesCache,
    contextSource: TileContextSource,
    ...
    const cachedCardTypes = await typesCache.getCardTypes();
    const cachedDocTypes = await typesCache.getDocTypes();
    const unavailableTypes = typesCache.unavailableTypes;

    const types = cachedCardTypes.map(x => {
      return {
        krCardType: x,
    ...
    for (const key of orderedGroups) {
      ...
      for (const krCardType of grouping.map(x => x.krCardType)) {
        const metadataCardType = cardMetadata.cardTypes.getCardTypeById(krCardType.id);
  ...
  private static async createBasedOnTilesWithoutGrouping(
    typesCache: IKrTypesCache,
    contextSource: TileContextSource,
    createBasedOnTile: ITile,
    copyFiles: boolean
    ...
    const cachedTypes = await typesCache.getTypes();
    const unavailableTypes = typesCache.unavailableTypes;
    ...
    for (const type of types) {
      if (type instanceof KrCardType) {
        ...
        const metadataCardType = cardMetadata.cardTypes.getCardTypeById(cardType.id);
        if (!metadataCardType || hasFlag(metadataCardType.flags, CardTypeFlags.Hidden)) {
          continue;
        }
        ...
        const metadataCardType = cardMetadata.cardTypes.getCardTypeById(docType.cardTypeId);
        if (!metadataCardType || hasFlag(metadataCardType.flags, CardTypeFlags.Hidden)) {
          continue;
        }
  ...
  private static getCardTypeGroupLocalizedCaption(
    ...
    const cardMetadata = MetadataStorage.instance.cardMetadata;
    const cardType = cardMetadata.cardTypes.getCardTypeById(krCardType.id);
