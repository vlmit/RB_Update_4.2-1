...
import {
  canDeleteCardFromViewDefault,
  canExportCardFromViewDefault,
  canViewCardStorageFromViewDefault
} from './defaultViewAliases';
...
export class ProhibitTilesInViewsTileExtension extends TileExtension {
  ...
  private static setEnabledWithCollapsingInViewContext(
    ...
    if (viewContext) {
      ...
      const canExecute =
        !!view &&
        Array.from(view.metadata.references.values()).some(x => x.openOnDoubleClick && x.isCard) &&
  ...
  private static deleteEvaluating = (e: TileEvaluationEventArgs) =>
    ProhibitTilesInViewsTileExtension.setEnabledWithCollapsingInViewContext(e, view =>
      canDeleteCardFromViewDefault(view.metadata.alias)
    );

  private static exportEvaluating = (e: TileEvaluationEventArgs) =>
    ProhibitTilesInViewsTileExtension.setEnabledWithCollapsingInViewContext(e, view =>
      canExportCardFromViewDefault(view.metadata.alias)
    );

  private static viewCardStorageEvaluating = (e: TileEvaluationEventArgs) =>
    ProhibitTilesInViewsTileExtension.setEnabledWithCollapsingInViewContext(e, view =>
      canViewCardStorageFromViewDefault(view.metadata.alias)
    );
