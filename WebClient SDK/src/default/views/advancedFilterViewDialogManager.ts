...
import { ViewCriteriaHelper } from '@tessa/platform';
...
export class AdvancedFilterViewDialogManager implements IAdvancedFilterViewDialogManager {
  ...
  protected static fillField(
    ...
    const valueSection = sections.tryGet(parameterMapping.valueSectionName);
    ...
    valueSection.fields.set(
      ...
      value.value,
      parameter.metadata?.schemeType?.fieldType ?? DotNetType.String
    );
    ...
    const displayValueSection = sections.tryGet(parameterMapping.displayValueSectionName);
    ...
    const requestParameter = new RequestParameterBuilder()
      ...
      .addCriteria(
        parameterMapping.criteriaOperator ??
          ViewCriteriaHelper.getDefaultCriteria(parameterMetadata),
        displayValue,
        fieldValue
      )
      .asRequestParameter();
