 ...
 *      `customContentSuffix` добавляет, соответственно, логотипы данных компаний перед и
...
export class GridLayoutStylingExtension extends CardUIExtension {
  override async initialized(context: ICardUIExtensionContext): Promise<void> {
    ...
    const viewBag: IExampleViewBag = {
      ...
      shareColumnId: '$AbTest_CardTypes_Columns_Blocks_Share'
    };
...
const CustomHeaderRow: FC<IGridHeaderRowProps> = observer(({ style, ...otherProps }) => {
  ...
  const expandMenu = useMemo(() => {
    ...
    const expandAllButton = (
      <Button style={buttonStyle} onClick={handleExpandButtonClick}>
        Развернуть все
      </Button>
    );

    const collapseAllButton = (
      <Button style={buttonStyle} onClick={handleCollapseButtonClick}>
        Свернуть все
      </Button>
    );
