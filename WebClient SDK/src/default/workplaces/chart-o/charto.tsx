...
export class Charto extends React.Component<ChartoProps> {
  ...
  public render(): React.ReactElement {
    ...
    return (
      <StyledContainer>
        {Caption && (
          <div className="diagram-header">{LocalizationManager.instance.localize(Caption)}</div>
        )}
        {drawFunc ? (
          <DiagramsContainer className="diagram-container" columnCount={columnCount}>
  ...
  return (
    <ChartoStyled legendPosition={legendPosition}>
      ...
      <Legend marginLeft={Margin.left} marginRight={Margin.right}>
        {data?.length > 0 &&
          data.map((x, index) => (
            <LegendItem key={x.name} legendNotWrap={legendNotWrap}>
...
const SvgStyled = styled.svg`
  ...
  @media (max-width: ${PlatformSize.xl - 1}px) {
    .graph-caption {
      font-size: 0.7em;
    }
  }
  @media (max-width: ${PlatformSize.lg - 1}px) {
    .graph-caption {
      font-size: 0.9em;
    }
  }
  @media (max-width: ${PlatformSize.md - 1}px) {
    .graph-caption {
      font-size: 1em;
    }
  }
  @media (max-width: ${PlatformSize.sm - 1}px) {
    font-size: 1em;
    .graph-caption {
      font-size: 1em;
    }
...
const LegendItem = styled.div<{ legendNotWrap: boolean }>`
  display: flex;
  flex-direction: row;
  align-items: center;
