...
echo SELECT "ID", "Name" FROM "CalendarCalcMethods" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:Calendars" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - Calendar types
echo SELECT "ID", "Caption" FROM "CalendarTypes" ORDER BY "Caption"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:Calendars" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - Calendars
echo SELECT "ID", "Name" FROM "CalendarSettings" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:Calendars" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - Format settings
echo SELECT "ID", "Name" FROM "FormatSettings" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:Format settings" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail
...
echo SELECT "ID", "Name" FROM "RefGroupTypes" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:RefGroupTypes" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - Reference groups
echo SELECT "ID", "Name" FROM "RefGroups" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\Platform.jcardlib" "/o:RefGroups" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail
...
echo    - File templates
echo SELECT "ID", "Name" FROM "FileTemplates" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\PlatformWithFiles.jcardlib" "/o:File templates" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - VirtualFiles
echo SELECT "ID", "Name" FROM "KrVirtualFiles" ORDER BY "Name"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\PlatformWithFiles.jcardlib" "/o:VirtualFiles" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail

echo    - Help sections
echo SELECT "ID", CASE WHEN "LanguageCode" IS NULL THEN "Code" ELSE CONCAT("Code", '.', "LanguageCode") END AS "Digest" FROM "HelpSections" ORDER BY "Code", "LanguageCode"|tadmin Select %DbParam% "/cs:%Connection%" /q|tadmin ExportCards "/l:%Configuration%\Cards\PlatformWithFiles.jcardlib" "/o:HelpSections" "/a:%Address%" "/u:%Login%" "/p:%Password%" /localize:en /q
if not "%ErrorLevel%"=="0" goto :Fail
...
tadmin ExportForms "/o:%Configuration%\Forms" "/a:%Address%" "/u:%Login%" "/p:%Password%" /q
if not "%ErrorLevel%"=="0" goto :Fail

echo  ^> Exporting settings
tadmin ExportSettings "/o:%Configuration%\Settings" "/a:%Address%" "/u:%Login%" "/p:%Password%" /c /q
if not "%ErrorLevel%"=="0" goto :Fail

echo  ^> Exporting views
tadmin ExportViews "/o:%Configuration%\Views" "/a:%Address%" "/u:%Login%" "/p:%Password%" /s /c /q
if not "%ErrorLevel%"=="0" goto :Fail
...
echo;
echo Configuration has been exported to "%Configuration%"
