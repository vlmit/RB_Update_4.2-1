...
namespace Tessa.Extensions.Default.Client.AbTest
{
    ...
    /// </summary>
    public sealed class AbTaskEnableAttachFilesExampleUIExtension : CardUIExtension
    {
        ...
        public override async Task Saving(ICardUIExtensionContext context)
        {
            ...
            foreach (var taskViewModel in mainForm.Tasks.OfType<TaskViewModel>())
            {
                ...
                foreach (var fileCard in taskViewModel.TaskModel.CardTask.Card.Files)
                {
                    ...
                    if (fileCard.HasChanges()
                        || fileCard.Flags is not CardFileFlags.None
                        || (fileCard.Card.Sections.TryGetValue(CardSignatureHelper.SectionName, out var signatures)
