...
{
    public sealed class AbExternalFilesFileControlExtension(ISession session) : FileControlExtension
    {
        #region Fields
        ...
        public override async Task OpeningMenu(IFileControlExtensionContext context)
        {
            ...
            var isContainsExternalFiles = context.Control.Files.Any(p => p is AbExternalFile);

            // Добавляем группировку/фильтр
            if (context.Groupings.FirstOrDefault(p => p.Name == AbExternalFilesFileGroupingNames.Source) == null &&
                isContainsExternalFiles)
            {
                context.Groupings.AddRange(new AbFileSourceGrouping(AbExternalFilesFileGroupingNames.Source, "$AbTest_GroupingBySource"));
            }

            // Добавляем пункт меню для получения файлов
            context.Actions.AddRange(
                new MenuAction(
                    AbExternalFilesFileMenuActionNames.GetExternalFiles,
                    ...
                    new DelegateCommand(async o =>
                    {
                        if (!isContainsExternalFiles)
                        {
                            ...
                            await context.Control.SelectGroupingAsync(new AbFileSourceGrouping(AbExternalFilesFileGroupingNames.Source, "$AbTest_GroupingBySource"),
                                context.CancellationToken);

                            // Считываем имена и содержимое
                            var filesNamesContents = new List<Tuple<string, string>>
                            {
                                new("File1.txt", "File1_text.txt"),
                                new("File2.txt", "File2_text.txt"),
                                new("File3.txt", "File3_text.txt")
                            };
                        ...
                        else
                        {
                            ...
                            context.Control.Container.Files.RemoveRange(context.Control.Files.Where(p => p is AbExternalFile).ToArray());
                        }
                    }),
                    isSelectable: true,
        ...
        {
            IFileSource fileSource = new AbExternalFileSource(fileCache, this.session);
            ...
            fileToken.Name = name;
            fileToken.Type = new CardFileType(AbExternalFilesHelper.FileTypeAlias, AbExternalFilesHelper.FileTypeCaption);
            ((AbExternalFileCreationToken) fileToken).Description = content;
