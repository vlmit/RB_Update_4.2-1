...
namespace Tessa.Extensions.Default.Client.Workflow.Wf
{
    ...
    public sealed class WfResolutionVisualizationVisitor
    {
        ...
        {
            ThrowIfNull(nodeLayout);
            ThrowIfNull(nodeFactory);
            ThrowIfNull(card);
            ...
            for (int i = 0; i < items.Length; i++)
            {
                if (items[i].ParentRowID == parentRowID)
                {
                    if (children is null)
                    {
                        ...
                        children = historyItemListPool.Rent();
                        children.Clear();
                    }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            IExtensionExecutor executor =
                this.extensionContainer != null
                    ? await this.extensionContainer.ResolveExecutorAsync<IWfResolutionVisualizationExtension>()
                    : null;
