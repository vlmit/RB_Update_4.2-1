...
{
    public sealed class KrCardValidationManager(CardValidationManager platformValidationManager) : ICardValidationManager
    {
        #region Fields

        private readonly CardValidationManager platformValidationManager = NotNullOrThrow(platformValidationManager);
        ...
        public ValueTask<ICardValidationResult> ValidateCardAsync(
            ...
            Dictionary<string, object?>? externalContextInfo = null,
        ...
        {
            if (modifyContextActionAsync is null)
            {
        ...
        private static async ValueTask ModifyContextAsync(ICardValidationContext context)
        {
            ...
            if (token is null
                || token.TryGetExtendedCardSettings() is not { } extendedCardSettings
                || extendedCardSettings.TryGetSectionSettings() is not { } cardSettings)
            {
                return;
            }
                    ...
                    if (sectionSettings.IsMasked)
                    {
                        ...
                        else
                        {
                            foreach (var columnMeta in sectionMeta.Columns)
                            {
                                ...
                                if (checkColumnMeta is not null
