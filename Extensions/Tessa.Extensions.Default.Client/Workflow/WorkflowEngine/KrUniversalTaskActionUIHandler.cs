    ...
    public sealed class KrUniversalTaskActionUIHandler : WorkflowActionUIHandlerBase
    {
        ...
        protected override async Task AttachToCardCoreAsync(WorkflowEngineBindingContext bindingContext)
        {
            ...
            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                WorkflowActionTypes.TaskRolesSectionName,
                WorkflowActionTypes.TaskRolesSectionName);

            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                WorkflowActionTypes.TaskRoleRolesSectionName,
                WorkflowActionTypes.TaskRoleRolesSectionName);
            ...
            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                WorkflowActionTypes.CompletionNotificationsSectionName,
                WorkflowActionTypes.CompletionNotificationsSectionName);

            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                WorkflowActionTypes.NotificationTaskRolesSectionName,
                WorkflowActionTypes.NotificationTaskRolesSectionName);
            ...
            await this.AttachTableSectionAsync(
                bindingContext,
                KrUniversalTaskActionButtonTaskRolesVirtual.SectionName,
                KrUniversalTaskActionButtonTaskRolesVirtual.SectionName);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Подключим обработку контрола функциональных ролей
            WorkflowActionWithTaskRolesUIHelper.AttachTaskFunctionRolesControl(cardModel);
        }
        ...
        {
            if (sender is CardSection mainSectionInner)
            {
                switch (e.FieldName)
                {
        ...
        private void CompletionOptionsTable_RowEditorClosing(object? sender, GridRowEventArgs e)
        {
            ...
            if (optionID.HasValue)
            {
                ...
                if (CheckDuplicatesOptionID(rows, row.RowID, optionID.Value))
                {
                    ...
                    validationResult.AddError(
                        this,
                        LocalizeFormat("$KrActions_UniversalTask_CompletionOptionIDNotUnique"));
                    e.Cancel = true;
                }
            }
            else
            {
                ...
                validationResult.AddError(
                    this,
                    LocalizeFormat("$KrActions_UniversalTask_CompletionOptionIDEmpty"));
                e.Cancel = true;
            }
            ...
            if (string.IsNullOrEmpty(row.TryGet<string>(KrUniversalTaskActionButtonsVirtual.Caption)))
            {
                ...
                validationResult.AddError(
                    this,
                    LocalizeFormat("$KrActions_UniversalTask_CompletionOptionCaptionEmpty"));
                e.Cancel = true;
            }
