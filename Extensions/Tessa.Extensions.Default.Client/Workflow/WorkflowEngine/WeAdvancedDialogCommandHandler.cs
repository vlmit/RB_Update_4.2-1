...
namespace Tessa.Extensions.Default.Client.Workflow.WorkflowEngine
{
    ...
    public sealed class WeAdvancedDialogCommandHandler : AdvancedDialogCommandHandler
    {
            ...
            ICardEditorModel? parentCardEditor = null)
        {
            ...
            {
                throw new InvalidOperationException("No dialog completion option settings found in client command.");
            }

            var (request, requestSignature) = dialogSettings.GetProcessRequest();
            if (request is null)
            {
                throw new InvalidOperationException("No request found in dialog completion option settings.");
            }
            ...
            await using var uiScope = parentCardEditor is null ? (IAsyncDisposable?) null : UIContext.Create(parentCardEditor.Context);
