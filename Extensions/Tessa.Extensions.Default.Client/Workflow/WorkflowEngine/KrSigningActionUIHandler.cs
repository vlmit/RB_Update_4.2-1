...
namespace Tessa.Extensions.Default.Client.Workflow.WorkflowEngine
{
    ...
    public sealed class KrSigningActionUIHandler : WorkflowActionUIHandlerBase
    {
        ...
        private EventHandler<CardFieldChangedEventArgs> mainSectionFieldChangedEventHandler;
        ...
        protected override async Task AttachToCardCoreAsync(WorkflowEngineBindingContext bindingContext)
        {
            ...
            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                KrSigningActionHiddenFileCategoriesVirtual.SectionName,
                KrSigningActionHiddenFileCategoriesVirtual.SectionName);

            await this.AttachTableSectionToTemplateAsync(
                bindingContext,
                KrSigningActionFileCategoriesVirtual.SectionName,
                KrSigningActionFileCategoriesVirtual.SectionName);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isReadOnly)
            {
                this.mainSectionFieldChangedEventHandler = (sender, e) => MainSection_FieldChanged(sender, e, cardModel);
                var mainSection = card.Sections[KrSigningActionVirtual.SectionName];
                mainSection.FieldChanged += this.mainSectionFieldChangedEventHandler;
            ...
            ResolveSignFilesControlsVisibility(cardModel);
        ...
        protected override async Task InvalidateFormCore(ICardModel cardModel, CancellationToken cancellationToken = default)
        {
            ...
            if (this.mainSectionFieldChangedEventHandler is not null)
            {
                mainSection.FieldChanged -= this.mainSectionFieldChangedEventHandler;
                this.mainSectionFieldChangedEventHandler = null;
            }
        ...
        private static void MainSection_FieldChanged(object sender, CardFieldChangedEventArgs e, ICardModel cardModel)
        {
            ...
            switch (e.FieldName)
            {
                ...
                case KrSigningActionVirtual.Planned when e.FieldValue != null:
                    ...
                    break;
                case KrSigningActionVirtual.SignCardFiles:
                    ResolveSignFilesControlsVisibility(cardModel);
                    break;
            }
        }
        ...
        {
            var shouldRearrange = false;
            if (block.BlockVisibility != visibility)
            {
                block.BlockVisibility = visibility;
                shouldRearrange = true;
            }
        ...
        private static void ResolveSignFilesControlsVisibility(ICardModel cardModel)
        {
            var signFilesControl = cardModel.Controls["SignCardFiles"];
            var signFilesConcreteControl = (CheckBoxViewModel) ((ParametrizedControlViewModel) signFilesControl).InnerViewModel;

            ChangeVisibilityAndRearrangeChildren(
                cardModel.Blocks["StageSignCardFilesOptions"],
                signFilesControl.Block.Form,
                signFilesConcreteControl.IsChecked ? Visibility.Visible : Visibility.Collapsed);
        }
