...
namespace Tessa.Extensions.Default.Client.Workflow.WorkflowEngine
{
    ...
    public sealed class KrApprovalActionUIHandler : WorkflowActionUIHandlerBase
    {
        ...
        protected override async Task AttachToCardCoreAsync(WorkflowEngineBindingContext bindingContext)
        {
            ...
            this.AddLinkBinding(
                bindingContext,
                [KrApprovalActionOptionLinksVirtual.Link, Names.Table_ID],
                KrApprovalActionOptionLinksVirtual.SectionName);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isReadOnly)
            {
                ...
                mainSection.FieldChanged += this.MainSection_FieldChangedAsync;
                ...
                additionalPerformersSettingsSection.FieldChanged += (_, e) =>
                {
                    switch (e.FieldName)
                    {
                        ...
                        case KrApprovalActionAdditionalPerformersSettingsVirtual.IsAdditionalApprovalFirstResponsible when e.FieldValue is bool isAdditionalApprovalFirstResponsible:
                            ...
                            if (filteredAdditionalPerformerRows.Length > 0)
                            {
                ...
                await WorkflowHelper.InitializeTaskCompletionOptionsAsync(
                    this.cardMetadata,
                ...
                WorkflowHelper.InitializeActionCompletionOptions(
            ...
            void SelectedPerformerChanged(bool isSelectedPerformer)
            {
                ...
                if (additionalPerformersDisplayInfoSection.Rows.Count > 0)
                {
                    additionalPerformersDisplayInfoSection.Rows.Clear();
                }

                var currentMainApproverRowID = GetSelectedMainApproverRow()?.RowID;
                var overrideAdditionalPerformerRows = GetAdditionalPerformerRows(additionalPerformersSection.Rows, currentMainApproverRowID)
                ...
                var isAdditionalApprovalFirstResponsible = false;
                if (overrideAdditionalPerformerRows.Length > 0)
                {
                    var firstRow = overrideAdditionalPerformerRows[rowIndex];
                ...
                for (; rowIndex < overrideAdditionalPerformerRows.Length; rowIndex++)
                {
                    CreateDisplayInfo(
                        additionalPerformersDisplayInfoSection.Rows.Add(),
                        overrideAdditionalPerformerRows[rowIndex],
                        rowIndex);
                }
            }
        }
        ...
        private async void MainSection_FieldChangedAsync(object sender, CardFieldChangedEventArgs e)
        {
            var mainSectionInner = (CardSection) sender;
            switch (e.FieldName)
            {
                case KrApprovalActionVirtual.Period when e.FieldValue is not null:
                    mainSectionInner.Fields[KrApprovalActionVirtual.Planned] = null;
                    break;
                case KrApprovalActionVirtual.Planned when e.FieldValue is not null:
                    mainSectionInner.Fields[KrApprovalActionVirtual.Period] = null;
                    break;
                case KrApprovalActionVirtual.IsAdvisory when e.FieldValue is bool isAdvisory:
                    if (isAdvisory)
                    {
                        if (!mainSectionInner.Fields.TryGet<Guid?>(KrApprovalActionVirtual.Kind + Names.Table_ID).HasValue)
                        {
                            var kindID = KrConstants.AdvisoryTaskKindID;
                            var isFound = false;
                            string kindCaption = null;

                            try
                            {
                                (isFound, kindCaption) = await WorkflowCommonHelper.GetKindAsync(
                                    this.viewService,
                                    kindID);
                            }
                            ...
                            if (isFound)
                            {
                                mainSectionInner.Fields[KrApprovalActionVirtual.Kind + Names.Table_ID] = kindID;
                    ...
                    {
                        if (mainSectionInner.Fields.TryGet<Guid?>(KrApprovalActionVirtual.Kind + Names.Table_ID) == KrConstants.AdvisoryTaskKindID)
                        {
                            mainSectionInner.Fields[KrApprovalActionVirtual.Kind + Names.Table_ID] = null;
                            mainSectionInner.Fields[KrApprovalActionVirtual.Kind + Table_Field_Caption] = null;
                        }
                    }
        ...
        private void EditInterjectOptions_FieldChanged(object sender, CardFieldChangedEventArgs e)
        {
            ...
            switch (e.FieldName)
            {
                case KrWeEditInterjectOptionsVirtual.Period when e.FieldValue is not null:
                    mainSectionInner.Fields[KrWeEditInterjectOptionsVirtual.Planned] = null;
                    break;
                case KrWeEditInterjectOptionsVirtual.Planned when e.FieldValue is not null:
                    mainSectionInner.Fields[KrWeEditInterjectOptionsVirtual.Period] = null;
                    break;
            }
        }
