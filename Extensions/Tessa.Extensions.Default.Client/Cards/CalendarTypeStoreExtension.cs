        ...
        CardStoreExtension
    {
        ...
        public override async Task AfterRequest(ICardStoreExtensionContext context)
        {
                    ...
                    || calendarTypeSection.Fields.ContainsKey("WorkDaysInWeek")))
            {
                ...
                // Если поменял настройки другой пользователь, то мы об этом не узнаем до перезапуска клиента.
                await this.calendarTypeSettingsCache.InvalidateAsync(context.Request.Card.ID);
            }
        }
