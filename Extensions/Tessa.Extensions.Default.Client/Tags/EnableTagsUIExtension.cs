...
{
    /// <summary>
    /// Расширение на UI карточки, которое включает теги в карточке, если в <see cref="CardInfoStorageObject.Info"/> передаётся информация о метках карточки в объекте <see cref="TagsForCard"/>.
    /// </summary>
    public sealed class EnableTagsUIExtension : CardUIExtension
    {
        ...
        {
            if (context.Card is not { } card ||
                context.UIContext.CardEditor is not { } cardEditor)
            {
                return Task.CompletedTask;
            }

            var tagsAllowed = context.Model.CardType.Flags.Has(CardTypeFlags.AllowTags);
            
            cardEditor.Toolbar.CanHaveTags = tagsAllowed;
            cardEditor.Toolbar.TagsModel.Tags.Clear();
            cardEditor.Toolbar.TagsModel.MaxTagsDisplayed = this.userSettings.MaxTagsDisplayed;
            foreach (var tagInfo in TagsForCard.TryUnpack(card.Info)?.Tags ?? [])
            {
            ...
            if (cardEditor.Toolbar.RightTagControl is AddTagButtonViewModel addTagButtonViewModel)
            {
                ...
                {
                    if (cardEditor.CardModel is null)
                    {
                        return;
                    }
                    ...
                    {
                        if (await TessaDialog.ConfirmAsync(await LocalizeNameAsync("Tags_SaveCardToAddTag")))
                        {
        ...
        private void AddTagToToolbar(Card card, ICardEditorModel cardEditor, TagInfo tag)
        {
            ...
            {
                var tagInfoModel = this.tagInfoModelFactory.CreateModel(tag, cardEditor.Toolbar.TagsModel, card);
