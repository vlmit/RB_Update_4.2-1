...
namespace Tessa.Extensions.Default.Client.Tiles
{
    public static class KrTileHelper
    {
        ...
        private static volatile IReadOnlyList<Guid>? unavailableTypeIDList;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!sendTaskSucceeded || continuationOnSuccessFunc is null)
            {
        ...
        public static async ValueTask<(IReadOnlyList<Guid>, ValidationResult)> GetUnavailableTypesAsync(
            ICardRepository cardRepository,
            IKrTypesCache krTypesCache,
            CancellationToken cancellationToken = default)
        {
            if (unavailableTypeIDList is not null)
            {
                return (unavailableTypeIDList, ValidationResult.Empty);
            }

            (unavailableTypeIDList, ValidationResult result) = 
                await KrPermissionsHelper.GetUnavailableTypesAsync(cardRepository, krTypesCache, cancellationToken: cancellationToken).ConfigureAwait(false);
            return (unavailableTypeIDList, result);
        }
        ...
        public static void SetUnavailableTypes(ReadOnlyCollection<Guid> unavailableTypes) => unavailableTypeIDList = unavailableTypes;
