...
namespace Tessa.Extensions.Default.Client.Tiles
{
        ...
        : TileExtension
    {
        ...
        {
            if (context.Workspace.LeftPanel.Tiles.TryGet(TileNames.ViewsOther) is not { } otherTile
                || otherTile.Tiles.TryGet(TileNames.FilterView) is not { } filterViewTile)
            {
            ...
            filterViewTile.Command = new DelegateCommand(async o =>
                await ExecuteInViewContextAsync(
                    async viewContext =>
                    {
                        ...
                        {
                            await this.advancedFilterViewDialogManager.OpenAsync(descriptor, viewContext.Parameters, CancellationToken.None);
                        }
                        else
                        {
