...
namespace Tessa.Extensions.Default.Client.Tiles
{
    ...
    public sealed class KrTypesAndCreateBasedOnTileExtension : TileExtension
    {
            ...
            ISettingsProvider settingsProvider)
        {
            ...
            this.createBasedOnTypesLazy = new AsyncLazy<HashSet<Guid>>(
                async () =>
                {
                    ...
                    return 
                        krSettings is not null 
                            ? krSettings.CreateBasedOnTypes 
                            : new HashSet<Guid>();
                });
        }
        ...
        private static async ValueTask<int> CountTypesAsync(
            ...
            IReadOnlyCollection<Guid> unavailableTypes,
        ...
        private async Task InitializeKrTilesGlobalAsync(ITileGlobalExtensionContext context, IReadOnlyCollection<Guid> unavailableTypes)
        {
            ...
            Guid cardTypeID)
        {
            ...
            for (int i = 0; i < docTypesForCardType.Length; i++)
            {
                ...
                string docTypeTitle = docTypesForCardType[i].Name ?? string.Empty;
            ...
            ITile createBasedOnTile)
        {
                ...
                .OrderByLocalized(x => x.Caption))
            {
                if (type is KrCardType cardType)
                {
                    ...
                    tileCollection.Add(
                        this.CreateBasedOnCardTypeTile(
                            context,
                            contextSource,
                            ref order,
                            cardType));
            ...
            ITile createBasedOnTile)
        {
                ...
                .ThenBy(x => x.Key))
            {
                    ...
                    TileSize.Half)
                {
                    Info =
                    {
                        [CreateCardTileExtension.IsGroupKey] = BooleanBoxes.True
                    }
                };
                ...
                int tileOrder = 0;
                foreach (KrCardType krCardType in grouping.Select(x => x.krCardType).OrderByLocalized(x => x.Caption ?? string.Empty))
                {
                    CardTypeCollection metadataCardTypes = await this.cardMetadata.GetCardTypesAsync(context.CancellationToken).ConfigureAwait(false);
                    ...
                    if (krCardType.UseDocTypes)
                    {
                        ...
                        foreach (KrDocType docType in (await this.typesCache.GetDocTypesAsync(context.CancellationToken).ConfigureAwait(false))
                            .OrderByLocalized(x => x.Caption ?? string.Empty))
                        {
                            ...
                            ITile basedOnDocType = this.CreateBasedOnDocTypeTile(
                                context,
                                contextSource,
                                ref innerOrder,
                                docType);
            ...
            KrDocType docType)
        {
            var result = new Tile(
                ...
                new DelegateCommand(p =>
                {
                    ...
                    this.CreateBasedOnTileActionAsync(
                        tile, 
                        docType.CardTypeID,
                ...
                evaluating: EnableIfNotInSpecialModeAndNotCreating)
            {
                Info =
                {
                    [InitialCaptionKey] = docType.Caption
                }
            };
            ...
            Action<Dictionary<string, object?>>? initializeRequestAction = null)
        {
            ...
            if (saveBeforeCreatingCard && editor is not null)
            {
        ...
        {
            (IReadOnlyList<Guid> unavailableTypes, ValidationResult result) = await KrTileHelper.GetUnavailableTypesAsync(this.cardRepository, this.typesCache).ConfigureAwait(false);
            await TessaDialog.ShowNotEmptyAsync(result);
            await this.InitializeKrTilesGlobalAsync(context, unavailableTypes).ConfigureAwait(false);
            await this.InitializeKrCreateBasedOnTilesAsync(context, unavailableTypes).ConfigureAwait(false);
        }
