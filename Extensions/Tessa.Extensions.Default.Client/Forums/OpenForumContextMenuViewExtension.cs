...
namespace Tessa.Extensions.Default.Client.Forums
{
    ...
    public sealed class OpenForumContextMenuViewExtension : IWorkplaceViewComponentExtension
    {
        ...
        {
            if (this.session.Token?.ApplicationID == ApplicationIdentifiers.TessaAdmin || model.RefSection is not null)
            {
        ...
        private async Task RemoveParticipantsCommandAsync(IViewContext context, List<ForumOperationItem> items, CancellationToken cancellationToken = default)
        {
            ...
            if (!HasModeratorPermissions(context))
            {
                await TessaDialog.ShowErrorAsync(
                    string.Format(
                        await LocalizeAsync(
                            "$Forum_UI_Cards_RemoveParticipants_NoRequiredPermissions") ?? string.Empty,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (groupedItems.Count() > 1)
            {
                return new ValidationResultBuilder()
                    .Add(ForumValidationKeys.PermissionError, ValidationResultType.Error,
                        await LocalizeAsync("$Forum_UI_Cards_RemoveParticipants_CannotDeleteParticipantsAndRoles") ?? string.Empty)
                    .Build();
            }
            ...
            else
            {
                bool result = await TessaDialog.ConfirmAsync(
                    string.Format(
                        await LocalizeAsync("$Forum_UI_Cards_RemoveParticipant_ConfirmSingle") ?? string.Empty,
                        ForumHelper.ConcatUsersName(items.Select(u => u.RoleName).ToList(), ",")));
                if (result)
                {
                    (_, validationResult) = await this.forumProvider.RemoveParticipantsAsync(item.TopicID, participants, cancellationToken: cancellationToken).ConfigureAwait(false);
                }
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (result)
            {
                (bool isEnableSuperModeratorMode, _, ValidationResult vr) = await this.permissionsProvider.CheckElevatedPermissionsAsync(
                    cardID,
                    null,
                    cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (isSuperModerator)
            {
                ...
                if (confirmed)
                {
                    (ForumResponse _, ValidationResult validationResult) = await this.forumProvider.RemoveRolesAsync(items[0].TopicID, participants, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return validationResult;
                }
            ...
            if (result)
            {
                (bool isEnableSuperModeratorMode, _, ValidationResult validationResult) = await this.permissionsProvider.CheckElevatedPermissionsAsync(
                    cardID,
                    null,
                    cancellationToken);
                ...
                if (isEnableSuperModeratorMode)
                {
                    ...
                    if (result)
                    {
                        (ForumResponse _, ValidationResult removeRolesResult) = await this.forumProvider.RemoveRolesAsync(items[0].TopicID, participants, cancellationToken: cancellationToken).ConfigureAwait(false);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            ValidationResult validationResult = newItem.ParticipantType == ParticipantType.ParticipantFromRole
                ? await this.forumProvider.UpdateRolesAsync(newItem.TopicID, participants, newItem.IsReadOnly, newItem.CanEditMessages, newItem.IsSubscribed, cancellationToken).ConfigureAwait(false)
                : await this.forumProvider.UpdateParticipantsAsync(newItem.TopicID, participants, newItem.IsReadOnly, newItem.CanEditMessages, newItem.IsSubscribed, newItem.ParticipantType,
                        cancellationToken)
