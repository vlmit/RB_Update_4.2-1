...
namespace Tessa.Extensions.Default.Client.Forums
{
    ...
    /// </summary>
    public sealed class TopicsUIExtension(IForumDialogManager forumDialogManager) : CardUIExtension
    {
        ...
        public override async Task Initialized(ICardUIExtensionContext context)
        {
            ...
            // проверить что карточка входит в типовое решение и на ней включено "использовать систему форумов"
            foreach (ForumControlViewModel forumControl in GetAllForumControls(context))
            {
                ...
                forumControl.CheckElevatedPermissionsAsync(
                    ct => CheckElevatedPermissionsAsync(uiContext, ct));

                // Если в инфо есть topicID значит мы пришли из OpenTopicOnDoubleClickExtension. и Это ID выбранного топика
                    ...
                    context.Model.Card.Info.TryGetValue(ForumHelper.TopicTypeIDKey, out object type) && type is Guid typeID)
                {
                    if (forumControl.ForumViewModel.TopicTypeID == typeID)
                    {
                        ...
                        await forumControl.SelectTopicAsync(
                            topicID,
                            null,
                            data is not null && data.ReadTopicStatusList.TryFirst(
        ...
        private readonly IForumDialogManager forumDialogManager = NotNullOrThrow(forumDialogManager);
        ...
        private static async Task CheckElevatedPermissionsAsync(IUIContext uiContext, CancellationToken cancellationToken = default)
        {
            await OpenMarkedCardAsync(
                uiContext,
                KrPermissionsHelper.CalculateElevatedPermissions,
                null, //Не требуем подтверждения действия, если не было изменений
                cardIsNew => cardIsNew
                    ? TessaDialog.Confirm("$KrTiles_EditModeConfirmation") ? true : null
                    : TessaDialog.ConfirmWithCancel("$KrTiles_EditModeConfirmation"),
                async () => await ElevatedPermissionsMessageAsync(),
                cancellationToken: cancellationToken);
        }

        private async Task AddTopicShowDialogAsync(ForumControlViewModel forumControlViewModel, CancellationToken cancellationToken = default)
        {
        ...
        private static async Task ElevatedPermissionsMessageAsync()
        {
            var card = UIContext.Current.CardEditor?.CardModel?.Card;
            if (card is null)
            {
            ...
            var token = KrToken.TryGet(card.Info);
            var superModerator = token?.HasPermission(KrPermissionFlagDescriptors.SuperModeratorMode) == true;
            var canEditAllMessages = token?.HasPermission(KrPermissionFlagDescriptors.EditAllMessages) == true;
            if (superModerator && canEditAllMessages)
            {
                TessaDialog.ShowMessage("$Forum_Permission_ElevatedPermissions_All");
                return;
            }

            if (superModerator)
            {
                TessaDialog.ShowMessage("$Forum_Permission_ElevatedPermissions_SuperModerator");
                return;
            }

            if (canEditAllMessages)
            {
                TessaDialog.ShowMessage("$Forum_Permission_ElevatedPermissions_EditingAllMessages");
                return;
            }
        ...
        private static async Task OpenMarkedCardAsync(IUIContext context,
            ...
            Func<Task> continuationOnSuccessFunc = null,
