    ...
    public class WebChartColorPickerViewModel : ViewModel<EmptyModel>
    {
        ...
        private static Color? ParseColorFromUserInput(string text)
        {
            try
            {
                ...
                return UIHelper.TryGetColorFromArgbString(text) ?? Colors.Transparent;
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException(LocalizeName("UI_Controls_ColorSelector_InvalidText"), ex);
            }
        }
        ...
        /// <summary>
        /// Флаг, указывающий на то, что контрол работает с палитрой цветов.
        /// </summary>
        /// <remarks>Нужно указать <c>false</c> для того, чтобы привязанный к модели колорпикер не показывал кнопку выбора цвета из палитры.</remarks>
        public bool IsPaletteMode => false;
