...
namespace Tessa.Extensions.Default.Client.UI
{
    public sealed class KrExtendedPermissionsUIExtension : CardUIExtension
    {
        ...
        private class VisibilitySettings
        {
            ...
            {
                var alias = visibilitySetting.Alias;
                if (alias is null)
                {
                    return;
                }
                ...
                var wildStart = alias[0] == '*';
                var wildEnd = alias[length - 1] == '*';
        ...
        private class PermissionsControlsVisitor
        {
            ...
            private readonly ICardControlAdditionalInfoRegistry controlRegistry;
            private readonly HashSet<IFormWithBlocksViewModel> formsForRearrange = [];
            private readonly HashSet<Guid> hideSections = [];
            private readonly HashSet<Guid> hideFields = [];
            private readonly HashSet<Guid> showFields = [];
            private readonly HashSet<Guid> mandatorySections = [];
            private readonly HashSet<Guid> mandatoryFields = [];
            private readonly HashSet<Guid> disallowedSections = [];
            ...
            private ICollection<KrPermissionsFileConstraints>? fileConstraints;
            ...
            public void Fill(
                KrToken token,
                IReadOnlyCollection<IKrPermissionSectionSettings>? sectionSettings,
                ...
                ICollection<KrPermissionsFileConstraints>? fileConstraints)
            {
                ...
                ICardModel cardModel)
            {
                    ...
                    || this.otherFilesSettings is not null)
                {
                    ...
                    if (control is CardViewControlViewModel { Name: not null } viewControl
                        && CardFilesHelper.TryGetFileControl(cardModel.Info, viewControl.Name) is { } fileControl2)
                    {
            ...
            {
                var filesSettings = this.SelectFilesSettings(context.ReplaceFile);
                ...
                if (context.ReplaceFile is null)
                {
                    if (!this.canAddFiles
                        ...
                        && (filesSettings.TryGetGlobalSettings() is not { } globalSettings
                            || !globalSettings.AddAllowed
                            && globalSettings.AllowedCategories is not { Count: > 0 }))
                    {
                ...
                else
                {
                    ...
                    if (filesSettings?.TryGetGlobalSettings() is { } globalSettings)
                    {
                        if (globalSettings.AddAllowed)
                        {
                            needFilter = categoryID is not null
                                && globalSettings.DisallowedCategories?.Contains(categoryID.Value) == true;
                        }
                        else if (globalSettings.AddDisallowed)
                        {
                            needFilter = categoryID is null
                                || globalSettings.AllowedCategories?.Contains(categoryID.Value) != true;
                        }
                        else
                        {
                            needFilter = categoryID is null
            ...
            private async ValueTask<IEnumerable<IFileCategory>> CategoryFilterAsync(IFileCategoryFilterContext context)
            {
                    ...
                    && ownFiles.Count > 0)
                {
                    ...
                    if (this.ownFilesSettings.TryGetGlobalSettings() is { } globalSettings)
                    {
                        checkExtensionSettings.Add(globalSettings);
                    }
                }
                    ...
                    && otherFiles.Count > 0)
                {
                    ...
                    if (this.otherFilesSettings.TryGetGlobalSettings() is { } globalSettings)
                    {
                        checkExtensionSettings.Add(globalSettings);
                    }
                }
            ...
            private async ValueTask ValidateFileCategoryAsync(IFileValidateCategoryContext context)
            {
                var file = context.FileInfo.File;
                var filesSettings = this.SelectFilesSettings(file);
                ...
                var isOwnFile = this.IsOwnFile(file);
                ...
                else if (this.CheckFileSize(fileExtension, context.Category, context.FileInfo.FileSize, isOwnFile) is { } fileSizeConstraint)
                {
                    KrPermissionsHelper.AddFileValidationError(
                        context.ValidationResult,
                        this,
                        ...
                        fileExtension: fileExtension,
                        categoryCaption: context.Category?.Caption,
                        sizeLimit: fileSizeConstraint.Limit);
                }
                else if (this.CheckMaxCount(file?.ID, fileExtension, context.Category, isOwnFile, context.Files, context.ValidatedFiles, context.ValidatedFilesCategories) is { } maxCountConstraint)
                {
                    await KrPermissionsHelper.AddMaxFileCountExceededOrMandatoryErrorAsync(
                        ...
                        maxCountConstraint.Categories,
                        maxCountConstraint.Categories is null || context.Category is null
                            ? null
                            : [context.Category.Caption],
                        maxCountConstraint.MaxCount!.Value,
                        null,
            ...
            private KrPermissionsFileConstraints? CheckMaxCount(
                Guid? checkFileID,
                string extension,
                IFileCategory? category,
                bool isOwnFile,
                IFileCollection files,
                IReadOnlyList<FileInfo>? validatedFiles = null,
                IReadOnlyList<IFileCategory>? validatedFilesCategories = null)
            {
                if (this.fileConstraints is null)
                {
                    return null;
                }
                ...
                foreach (var fileConstraint in this.fileConstraints)
                {
                    if (fileConstraint.MaxCount is null
                        || !IsFileConstraintAppliable(fileConstraint, categoryID, extension, isOwnFile))
                    {
                        continue;
                    }

                    var counter = 1; // текущий файл уже посчитали
                    if (validatedFiles is not null)
                    {
                        for (var i = 0; i < validatedFiles.Count; i++)
                        {
                            var validatedFile = validatedFiles[i];
                            var fileCategoryID = GetCategoryID(validatedFilesCategories?.ElementAtOrDefault(i) ?? category);
                            var fileExtension = GetExtension(validatedFile.FileName);
                            var fileIsOwnFile = this.IsOwnFile(validatedFile.File);

                            if (!IsFileConstraintAppliable(fileConstraint, fileCategoryID, fileExtension, fileIsOwnFile))
                            {
                                continue;
                            }
                            ...
                            if (counter > fileConstraint.MaxCount.Value)
                            {
                    ...
                    foreach (var file in files)
                    {
                        ...
                        var fileIsOwnFile = this.IsOwnFile(file);
            ...
            private async ValueTask ValidateFileContentAsync(
                IFileValidateContentContext context,
                bool checkOnFileReplace)
            {
                var filesSettings = this.SelectFilesSettings(context.FileInfo.File);
                if (filesSettings is null)
                {
                    return;
                }
                ...
                // Если файл новый и категория не известна, значит проверяем, что есть хотя бы одна возможная настройка добавления файла.
                ...
                else
                {
                    ...
                    // Категория доступна для добавления по настройкам или, если настроек для категории нет, добавление доступно глобально.
                    hasAccess = categoryAllowed ?? this.canAddFiles;
                }

                if (!hasAccess)
                {
                    KrPermissionsHelper.KrPermissionsErrorAction errorAction = checkOnFileReplace
                        ? KrPermissionsHelper.KrPermissionsErrorAction.ReplaceFile
                        : KrPermissionsHelper.KrPermissionsErrorAction.AddFile;
                ...
                else if (replaceFile is not null)
                {
                    var isOwnFile = this.IsOwnFile(replaceFile);
                    if (this.CheckFileSize(fileExtension, replaceFile.Category, context.FileInfo.FileSize, this.IsOwnFile(replaceFile)) is { } fileSizeConstraint)
                    {
                        KrPermissionsHelper.AddFileValidationError(
                            context.ValidationResult,
                            this,
                    ...
                    else if (this.CheckMaxCount(replaceFile.ID, fileExtension, replaceFile.Category, isOwnFile, context.Files) is { } maxCountConstraint)
                    {
                        await KrPermissionsHelper.AddMaxFileCountExceededOrMandatoryErrorAsync(
                            context.ValidationResult,
                            this,
                            ...
                            maxCountConstraint.FileCheckRule,
                            maxCountConstraint.Categories,
                            maxCountConstraint.Categories is null || replaceFile.Category is null
                                ? null
                                : [replaceFile.Category.Caption],
                            maxCountConstraint.MaxCount!.Value,
                            null,
            ...
            private KrPermissionsFileConstraints? CheckFileSize(
                string extension,
                IFileCategory? category,
                long fileSize,
                bool isOwnFile)
            {
                ...
                var categoryID = GetCategoryID(category);
                KrPermissionsFileConstraints? checkFileConstraint = null;
                foreach (var fileConstraint in this.fileConstraints)
                {
                    if (!fileConstraint.Limit.HasValue
                        || !IsFileConstraintAppliable(fileConstraint, categoryID, extension, isOwnFile))
                    {
                        continue;
                    }

                    if (checkFileConstraint is null
                        || checkFileConstraint.Limit!.Value > fileConstraint.Limit.Value)
                    {
                        checkFileConstraint = fileConstraint;
                    }
                    else if (checkFileConstraint.Priority != fileConstraint.Priority)
                    {
                        break;
                    }
                }

                return checkFileConstraint is not null && fileSize > checkFileConstraint.Limit!.Value
                    ? checkFileConstraint
                    : null;
            }

            private static bool IsFileConstraintAppliable(
                KrPermissionsFileConstraints fileConstraint,
                Guid? categoryID,
                string extension,
                bool isOwnFile)
            {
                return fileConstraint.FileCheckRule != (isOwnFile ? KrPermissionsHelper.FileCheckRules.FilesOfOtherUsers : KrPermissionsHelper.FileCheckRules.OwnFiles)
                    && (fileConstraint.Categories is not { } categories
                        || (categoryID.HasValue && categories.Contains(categoryID.Value)))
                    && (fileConstraint.Extensions is not { } extensions
            ...
            private static void MakeControlMandatory(IControlViewModel controlViewModel)
            {
                ...
                controlViewModel.RequiredText = LocalizeFormat("$KrPermissions_MandatoryControlTemplate", controlViewModel.Caption);
            }
        ...
        public override async Task Initialized(ICardUIExtensionContext context)
        {
            ...
            Dictionary<GridViewModel, PermissionsControlsVisitor> visitors = [];
            ...
            var extendedSettings = token.TryGetExtendedCardSettings();
            var sectionSettings = extendedSettings?.TryGetSectionSettings();
            var tasksSettings = extendedSettings?.TryGetTasksSettings();
            var visibilitySettings = extendedSettings?.TryGetVisibilitySettings();
            var fileSettings = extendedSettings?.TryGetFileSettings();
            ...
            var fileConstraints = extendedSettings?.TryGetFileConstraints()?.OrderByDescending(x => x.Priority).ToArray();
