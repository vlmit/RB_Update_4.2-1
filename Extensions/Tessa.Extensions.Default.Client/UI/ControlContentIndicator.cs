...
namespace Tessa.Extensions.Default.Client.UI
{
    ...
    /// </summary>
    /// <typeparam name="T">Тип контролируемых элементов управления. Тип должен быть одним из поддерживаемых объектом <see cref="BreadthFirstControlVisitor"/>
    /// выполняющим обход дерева элементов управления. Поддерживаемый тип соответствует аргументу типа одной из перегрузок метода
    /// <see cref="BreadthFirstControlVisitor.Visit(ICardModel)"/>.</typeparam>
        ...
        IDisposable
    {
        ...
        {
            ThrowIfNull(controls);
            ThrowIfNull(cardFieldContainer);
            ThrowIfNull(fieldIDs);
            ThrowIfNull(indicatorFormat);
            ...
            this.controlFieldsMapping =
                fieldControlsMapping
                    .GroupBy(p => p.Value)
                    .Select(p => (order: p.Key, fields: p.Select(q => q.Key).ToArray()))
                    .OrderBy(p => p.order)
                    .Select(p => p.fields)
                    .ToArray();
        ...
        private string GetNewName(string originalName) =>
            LocalizeFormat(this.indicatorFormat, originalName);
