...
namespace Tessa.Extensions.Default.Client.UI.KrProcess
{
        ...
        : CardUIExtension
    {
        ...
        private sealed class VisibilityViaTagsVisitor(Guid typeID) :
            BreadthFirstControlVisitor
        {
            /// <inheritdoc />
            protected override void VisitControl(
                IControlViewModel controlViewModel) => SetControlSettings(typeID, controlViewModel);

            /// <inheritdoc />
            protected override void VisitBlock(
                IBlockViewModel blockViewModel) => SetBlockControlsSettings(typeID, blockViewModel);
        }
        ...
        private readonly IKrProcessUIContainer uiContainer = NotNullOrThrow(uiContainer);

        private readonly ICardMetadata cardMetadata = NotNullOrThrow(cardMetadata);

        private readonly Dictionary<Guid, List<IStageTypeUIHandler>> handlersCache = new();
        ...
        private async void AddStageDialogHandlerAsync(object? sender, GridRowAddingEventArgs e)
        {
            ...
            var viewModel = new StageSelectorViewModel(
                ...
                async (groupVm, _, typeId, ct) =>
                {
                    ...
                    var result = await stageGroupsView.GetDataAsync(
                        new TessaViewRequest(stageGroupsView.Alias)
                        {
                            new RequestParameter("TypeId").Add(EqualsToCriteriaOperator.Instance, typeId)
                ...
                async (groupType, _, typeId, ct) =>
                {
                    ...
                    bool designTimeCard = KrProcessSharedHelper.DesignTimeCard(typeId);
                    if (designTimeCard)
                    {
                        typeIdParam = null;
                        ...
                        if (typeRows.Count > 0)
                        {
                            ...
                            {
                                typeIdParam = new RequestParameter("TypeId");
                                foreach (var id in typeIdSet)
                                {
                                    typeIdParam.Add(EqualsToCriteriaOperator.Instance, id);
                                }
                            }
                        }
                    }
                    ...
                    {
                        typeIdParam = new RequestParameter("TypeId").Add(EqualsToCriteriaOperator.Instance, typeId);
                    }

                    var result = await stageTypesView.GetDataAsync(
                        new TessaViewRequest(stageTypesView.Alias)
                        {
                            new RequestParameter("StageGroupIDParam").Add(EqualsToCriteriaOperator.Instance, groupType),
                            designTimeCard ? new RequestParameter("IsTemplate") { IsTrueCriteriaOperator.Instance } : null,
                            typeIdParam
                        }, ct).ConfigureAwait(false);
            ...
            GridRowEventArgs e)
        {
            ...
            var title = row.State == CardRowState.Inserted && KrProcessSharedHelper.RuntimeCard(e.CardModel.Card.TypeID)
                ? LocalizeFormat("$UI_StageRowWindowTitle", stageTypeCaption, row.TryGet<string>(KrConstants.KrStages.StageGroupName))
                : Localize(stageTypeCaption);

            // Изменение заголовка первого блока на форме таблицы ApprovalStagesTable для формирования правильных заголовков окон развёрнутых текстовых полей.
            // Подробнее про формирование заголовка окна текстового поля см. в TextBoxViewModelBase.GetFieldDigest.
            if (e.RowModel.Blocks.TryGet(KrConstants.Ui.KrStageCommonInfoBlock) is { } stageCommonInfoBlock)
            {
                stageCommonInfoBlock.Caption = title;
            }
            ...
            GridRowEventArgs e)
        {
            ...
            if (e.RowModel.Blocks.TryGet(KrConstants.Ui.KrStageSettingsBlockAlias, out var block))
            {
                ...
                {
                    var stageHandlerIDs = control
                        .CardTypeControl
                        .ControlSettings
                        .TryGet<IList?>(KrConstants.Ui.StageHandlerDescriptorIDSetting);

                    if (stageHandlerIDs is null)
                    {
                        continue;
                    }

                    if (stageTypeID.HasValue
                        && stageHandlerIDs.Contains(stageTypeID.Value))
                    {
            ...
            IControlViewModel controlViewModel)
        {
            ...
            if (!controlSettings.TryGetValue(TagParserCardTypeVisitor.TagsListSettingKey, out var tagsObj)
            ...
            IBlockViewModel blockViewModel)
        {
            ...
            if (!blockSettings.TryGetValue(TagParserCardTypeVisitor.TagsListSettingKey, out var tagsObj)
                || tagsObj is not IList tags)
            {
                return;
            }
        ...
        public override async Task Initialized(ICardUIExtensionContext context)
        {
            ...
            if (!KrProcessSharedHelper.DesignTimeCard(context.Model.CardType.ID))
            {
                ...
                approvalStagesTable.LeftButtons.Add(
                    new("$CardTypes_Buttons_ActivateStage",
