...
namespace Tessa.Extensions.Default.Client.UI.KrProcess.StageHandlers
{
        ...
        StageTypeUIHandlerBase
    {
        ...
        private async ValueTask<(Guid, string)> GetKindAsync(Guid id, CancellationToken cancellationToken = default)
        {
            var taskKindsView = await this.viewService.GetByNameAsync(KrConstants.Views.TaskKinds, cancellationToken);
            if (taskKindsView is null)
            {
                return default;
            }

            var result = await taskKindsView.GetDataAsync(
                new TessaViewRequest(taskKindsView.Alias) { new RequestParameter("ID").Add(EqualsToCriteriaOperator.Instance, id) },
