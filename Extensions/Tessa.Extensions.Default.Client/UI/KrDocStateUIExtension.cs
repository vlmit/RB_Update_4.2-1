...
namespace Tessa.Extensions.Default.Client.UI
{
        ...
        CardUIExtension
    {
        ...
        {
            context.Model.Info[CardUIHelper.MoveToTabModifierFuncAsyncKey] =
                static ValueTask<bool> (ICardEditorModel editor, OpenCardOptions options) =>
                {
                    var fields = editor.CardModel.Card.Sections["KrDocStateVirtual"].RawFields;
                    (options.Info ??= new())[DefaultExtensionHelper.StateIDKey] = fields.TryGet<int>("StateID");
                    return new(true);
                };
