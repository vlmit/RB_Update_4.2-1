...
namespace Tessa.Extensions.Default.Console.ExportSettings
{
    ...
    {
        [Verb("ExportSettings")]
        [LocalizableDescription("Common_CLI_ExportSettings")]
        public static async Task ExportSettings(
            [Output] TextWriter stdOut,
            [Error] TextWriter stdErr,
            [Argument, LocalizableDescription("Common_CLI_SettingNames")] IEnumerable<string>? name = null,
            ...
            await using (var companion = new UnityContainerCompanion { UseConfiguration = true })
            {
                result = await companion.ProcessAndGetAsync(
                    ...
                    async (c, ct) =>
                    {
                        ...
                        var context = new OperationContext(name?.AsReadOnlyCollection(), outputFolder, clearOutputFolder);
