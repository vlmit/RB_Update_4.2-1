            ...
            try
            {
                ...
                (_, CardNewResponse newResponse) = await this.cardManager.CreateFromTemplateAsync(
                    ...
                    async p =>
                    {
                        ...
                        foreach (IFile file in c.FileContainer.Files)
                        {
                            if (file.ID == p.FileID)
                            {
                ...
                var c = container[0] ??= await this.cardFileManager.CreateContainerAsync(newResponse.Card, cancellationToken: cancellationToken);

                storeResponse = await c.StoreAsync(cancellationToken: cancellationToken);
                storeResponse.ValidationResult.Add(result);
            }
            catch (OperationCanceledException)
            {
