...
namespace Tessa.Extensions.Default.Console.TimeZone
{
    public sealed class Operation : ConsoleOperation<OperationContext>
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var absUtcOffset = TimeSpan.FromMinutes(Math.Abs(context.ZoneOffset.Value));
            zoneToModifyRow.Fields["OffsetTime"] = CardHelper.DefaultDateTime.Date.Add(absUtcOffset);
            if (context.ZoneOffset >= 0)
            {
                zoneToModifyRow.Fields["IsNegativeOffsetDirection"] = false;
                zoneToModifyRow.Fields["ShortName"] = $"UTC+{TimeSpan.FromMinutes(Math.Abs(context.ZoneOffset.Value)):hh\\:mm}";
            }
            else
            {
        ...
        public override async Task<int> ExecuteAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (OperationFunction operation in context.OperationFunction ?? Enumerable.Empty<OperationFunction>())
                {
                    if (!asyncActions.TryGetValue(operation, out var asyncAction))
                    {
