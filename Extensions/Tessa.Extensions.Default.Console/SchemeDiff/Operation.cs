...
namespace Tessa.Extensions.Default.Console.SchemeDiff
{
    public static class Operation
    {
            ...
            IEnumerable<string>? excludedPartitions)
        {
            ...
            FileSchemeService fileSchemeService = new(
                fileFullPath,
            ...
            IEnumerable<string>? excludedPartitionsB)
        {
            ...
            await logger.InfoAsync("Difference is evaluated between \"{0}\" and \"{1}\", printing it to output", sourceA, sourceB);

            if (difference is null)
            {
                await stdOut.WriteLineAsync(await LocalizeNameAsync("Common_CLI_Scheme_DatabasesAreEqual"));
            }
            else
            {
                await using IndentedTextWriter writer = new(stdOut);
