...
namespace Tessa.Extensions.Default.Console.PackageWebApp
{
    public static class Operation
    {
        ...
        private static async Task<bool> LoadInfoAsync(IConsoleLogger logger, WebApplication app, string? exePath, string? languageCode)
        {
            ...
            string? appFolder = string.IsNullOrEmpty(exePath) ? exePath : Path.GetDirectoryName(Path.GetFullPath(exePath));
            ...
            var ignoredPathList = new HashSet<string>(await ignoredProvider.GetIgnoredFileNamesAsync(appFolder));

            if (ignoredVersionFilePath is not null)
            {
            ...
            bool client64Bit)
        {
            ...
            try
            {
                ...
                bool success = await LoadInfoAsync(logger, app, exePath, languageCode);
                if (!success)
                {
                    return -3;
                }
                ...
                if (app.Files.Count > 0)
                {
                    ...
                    foreach (WebApplicationFile file in app.Files)
                    {
                        ...
                        if (file.Content is { } content)
                        {
                            ...
                            cardFile.Hash = hashAlgorithm.ComputeHash(content);
                ...
                else if (outputPath.EndsWith('/') || outputPath.EndsWith('\\'))
                {
                    outputPath = Path.Combine(
                        outputPath[..^1],
                        $"{GetDefaultAppFileNameWithoutExtension(app)}.jcard");
                }
