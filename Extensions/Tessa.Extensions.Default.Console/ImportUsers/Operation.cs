...
namespace Tessa.Extensions.Default.Console.ImportUsers
{
    public sealed class Operation(
        ...
        [OptionalDependency] IDbConnectionStringCleaner? connectionStringCleaner = null)
        : ConsoleOperation<OperationContext>(logger, sessionManager, extendedInitialization: true)
    {
        #region Fields

        private readonly OperationSettings settings = new();
        ...
        private Task<OperationData> GetDataFromSourceAsync(
            ...
            importType switch
            {
                ImportType.Excel => this.GetFromExcelAsync(contextPathToUserFile, cancellationToken),
                ImportType.Csv => this.GetFromCsvAsync(contextPathToUserFile, contextPathToDepartmentFile, cancellationToken),
                _ => throw ArgumentOutOfRange(importType)
            };
            ...
            try
            {
                ...
                using (var reader = new StreamReader(contextPathToUserFile, Encoding.Default))
                {
                    ...
                    while (await csv.ReadAsync())
                    {
                        ...
                        if (csv.TryGetField<string>(++index, out var userDepartments) && !string.IsNullOrWhiteSpace(userDepartments) &&
                            int.TryParse(userDepartments, out var userDepartmentId))
                        {
                        ...
                        if (user.LoginType.Flags.Has(UserLoginTypeFlags.UseLogin)
        ...
        private async Task<OperationData> GetFromExcelAsync(string contextExcelFile, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    SharedStringTablePart sstpart = NotNullOrThrow(doc.WorkbookPart).GetPartsOfType<SharedStringTablePart>().First();
                    ...
                    if (departmentsRows is null || usersRows is null || miscRows is null)
                    {
                        return importData;
                    }
        ...
        private async Task<(int TimeZoneID, string? TimeZoneShortName, int TimeZoneUtcOffsetMinutes, string? TimeZoneCodeName)?>
            TryGetTimeZoneFromDictionaryAsync(
                ...
                CancellationToken cancellationToken = default)
        {
            Debug.Assert(this.builderFactory is not null);
            Debug.Assert(this.db is not null);
            ...
            else if (objectWithTimeZone.ZoneOffset is not null)
            {
                this.db.SetCommand(
                        selectFactory
                ...
                Guid? departmentId =
                    await this.db.SetCommand(
                            this.builderFactory
                                .Select().C("ID")
                                .From("Roles").NoLock()
                                .Where().C("TypeID").Equals().V((int) RoleType.Department)
                                .And().C("ExternalID").Equals().P("ExternalID")
                                .And().C("ExternalID").IsNotNull()
                                .Build(),
                            this.db.Parameter("ExternalID", externalID, DataType.NVarChar))
                ...
                return await this.db
                    .SetCommand(
                        this.builderFactory
                            ...
                            .Build(),
                        this.db.Parameter("Name", name, DataType.NVarChar))
                    .ExecuteAsync<Guid?>(cancellationToken);
            }
            ...
            if (!string.IsNullOrEmpty(externalID))
            {
                Guid? userID = await this.db.SetCommand(
                        this.builderFactory
                            ...
                            .Where().C("ExternalID").Equals().P("ExternalID")
                            .And().C("ExternalID").IsNotNull()
                            .Build(),
        ...
        private async Task<bool> ImportDataAsync(OperationData data, CancellationToken cancellationToken = default)
        {
                         ...
                         (t, r) => t))
            {
                ...
                var departmentCard = departmentID is not null
        ...
        /// <summary>
        /// Обновляет карточку департамента данными
        /// </summary>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!dep.TessaID.HasValue)
            {
                throw new InvalidOperationException(
                    $"Parent department isn't created in TESSA. Maybe, specified parent role creates cyclic coupling? Check data.{Environment.NewLine}Parent department ID: {id}{Environment.NewLine}Parent department ExternalID: {dep.ExternalID}{Environment.NewLine}Parent department Name: {dep.Name}");
            }
            ...
            CancellationToken cancellationToken = default)
        {
            foreach (var user in users)
            {
                ...
                var userCard = userId is not null
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            int loginTypeID = user.LoginType?.ID ?? UserLoginTypes.Forbidden.ID;
            string? loginType =
                await this.db
                    .SetCommand(
                        this.builderFactory
                            ...
                            .Build(),
                        this.db.Parameter("LoginTypeID", Int16Boxes.Box((short) loginTypeID), DataType.Int16))
            ...
            userCard.Sections[RoleStrings.PersonalRoles].Fields["FullName"] = user.GetFullName();
            ...
            userCard.Sections[RoleStrings.PersonalRoles].Fields["LoginTypeID"] = Int32Boxes.Box(loginTypeID);
            userCard.Sections[RoleStrings.PersonalRoles].Fields["LoginTypeName"] = loginType;

            if (user.LoginType?.Flags.Has(UserLoginTypeFlags.UseInternalPassword) == true)
            {
                var password = string.IsNullOrWhiteSpace(user.Password) ? OperationHelper.GetUniqueKey(16) : user.Password;
                userCard.Sections[InitializationExtensionHelper.PersonalRoleVirtualSection].Fields["Password"] =  Convert.ToBase64String(Encoding.UTF8.GetBytes(password));
            }
        ...
        private async Task<bool> StoreCardAsync(Card card, CancellationToken cancellationToken = default)
        {
            ...
            CardStoreRequest storeRequest = new() { Card = card };
        ...
        public override async Task<int> ExecuteAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                this.db = await ConsoleAppHelper.CreateDbManagerAsync(
                    this.Logger,
                    this.configurationManager,
                    context.ConfigurationString,
                    context.DatabaseName,
                    connectionStringCleaner,
                    cancellationToken);

                this.builderFactory = new QueryBuilderFactory(this.db.Dbms);

                OperationData data = await this.GetDataFromSourceAsync(
                    context.ImportType,
                    NotNullOrThrow(context.PathToUserFile),
                    context.PathToDepartmentFile,
                    cancellationToken);
