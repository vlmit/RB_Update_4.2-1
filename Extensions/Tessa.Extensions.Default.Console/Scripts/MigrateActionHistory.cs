...
namespace Tessa.Extensions.Default.Console.Scripts
{
        ...
        ServerConsoleScriptBase
    {
        ...
        private static readonly Dictionary<string, DataType> columns = new()
        {
            ...
            ["HostIP"] = DataType.NVarChar,
            ["HostName"] = DataType.NVarChar,
        ...
        protected override async ValueTask ShowHelpCoreAsync(CancellationToken cancellationToken)
        {
            ...
            await this.Logger.WriteLineAsync(
                "[-pp:target=NAME] - Specifies the connection string to the target database to where action history is copied to. \"migration\" if unspecified.");
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using var toDb = await ConsoleAppHelper.CreateDbManagerAsync(
                this.Logger,
                this.Container.Resolve<IConfigurationManager>(),
                configurationString: target,
                databaseNameOverride: null,
                connectionStringCleaner: this.Container.TryResolve<IDbConnectionStringCleaner>(),
                cancellationToken: cancellationToken);

            Dbms toDbms = toDb.Dbms;
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                for (int i = 0; i < reader.FieldCount; i++)
                {
                    parameters[i].Value = i == reader.FieldCount - 1
                        ? await reader.GetSequentialStringAsync(i, fromDbms, cancellationToken)
                        : reader.GetValue(i);
                }
                ...
                catch (Exception ex)
                {
                    ...
                    DbmsErrorCode errorCode = await errorCodeProvider.GetErrorCodeAsync(ex);
