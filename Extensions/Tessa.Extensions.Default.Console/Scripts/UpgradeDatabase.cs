...
namespace Tessa.Extensions.Default.Console.Scripts
{
        ...
        : ServerConsoleScriptBase
    {
        ...
        protected override async ValueTask ExecuteCoreAsync(CancellationToken cancellationToken)
        {
            ...
            var schemeService = new DatabaseSchemeService(
                dbScope,
                new ServerConfigurationVersionProvider(dbScope));

            var dbms = await dbScope.GetDbmsAsync();
            var schemeDbmsVersion = await schemeService.ReadSchemeDbmsVersionAsync(cancellationToken);

            var currentDbmsVersion = await dbScope.GetDbmsVersionAsync(cancellationToken);
            if (currentDbmsVersion == RuntimeHelper.ZeroVersion)
            {
