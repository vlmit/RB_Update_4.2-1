    ...
    public sealed class MigrationTask
    {
        ...
        public async Task RunAsync(CancellationToken cancellationToken = default)
        {
            ...
            await using (this.dbScope.Create())
            {
                ...
                parameters[1] = db.Parameter("SourceID", Int16Boxes.Box((short) this.toSourceTypeID.ID), DataType.Int16);

                string updateSourceActualText = GenerateUpdateSourceQuery(this.dbScope.BuilderFactory, "FileVersions");
                string updateSourceDeletedText = GenerateUpdateSourceQuery(this.dbScope.BuilderFactory, "DeletedFileVersions");
                ...
                foreach (FileVersionInfo versionInfo in this.versionInfoList)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (contentStream is not null && validationResult.IsSuccessful())
                            {
                        ...
                        if (validationResult.IsSuccessful())
                        {
                            ...
                            var updateSourceText = versionInfo.IsDeleted ? updateSourceDeletedText : updateSourceActualText;
        ...
        private static string GenerateUpdateSourceQuery(IQueryBuilderFactory builderFactory, string fileVersionsTable) =>
            builderFactory
