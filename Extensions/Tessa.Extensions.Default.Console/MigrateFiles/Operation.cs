...
namespace Tessa.Extensions.Default.Console.MigrateFiles
{
    public static class Operation
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Func<CancellationToken, Task<DbManager>>?[] createDbManagerFuncAsyncClosure = [null];
            ...
            return await companion.ProcessAndGetAsync(
                (c, ct) => c.Container
                    .RegisterServerForConsoleAsync(
                        () =>
                        {
                            var funcAsync = createDbManagerFuncAsyncClosure[0];
                            ThrowIfNull(funcAsync);

                            return funcAsync(ct).GetAwaiter().GetResult();
                        },
                        cancellationToken: ct),
                async (c, ct) =>
                {
                    createDbManagerFuncAsyncClosure[0] = await ConsoleAppHelper.CreateAsyncDbManagerFuncAsync(
                        logger,
                        c.ConfigurationManager!,
                        configurationString,
                        databaseName,
                        c.Container.TryResolve<IDbConnectionStringCleaner>(),
                        ct);

                    IExtensionAssemblyInfo assemblyInfo = c.Container.ResolveAssemblyInfo();
                    ...
                    await using (dbScope.Create())
                    {
                        ...
                        var fileSources = await GetFileSourcesAsync(dbScope, fromSourceTypeID, toSourceTypeID, ct);
                        ...
                        var sourceParameter = DataParameter.Int32("SourceID", fromSourceID);
                        await AddFileVersionsAsync(isDeleted: true, dbScope, sourceParameter, versionInfoList, ct);
                        await AddFileVersionsAsync(isDeleted: false, dbScope, sourceParameter, versionInfoList, ct);
                    ...
                    var contentStrategy = c.Container.Resolve<CardSourceContentStrategy>();
        ...
        private static async Task AddFileVersionsAsync(
            bool isDeleted,
            IDbScope dbScope,
            DataParameter sourceParameter,
            List<FileVersionInfo> versionInfoList,
            CancellationToken cancellationToken)
        {
            var filesTable = isDeleted ? Names.DeletedFiles : Names.Files;
            var fileVersionsTable = isDeleted ? Names.DeletedFileVersions : Names.FileVersions;
            ...
            var query = dbScope.BuilderFactory
                ...
                .C("f", "RowID")
                .C("fv", "RowID")
                .From(filesTable, "f").NoLock()
                .InnerJoin(fileVersionsTable, "fv").NoLock()
                .On().C("fv", "ID").Equals().C("f", "RowID")
                .Where().C("fv", "SourceID").Equals().P(sourceParameter.Name!)
                .Build();

            await using var reader = await dbScope.Db
                .SetCommand(query, sourceParameter)
                .LogCommand()
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                var versionRowID = reader.GetGuid(2);

                versionInfoList.Add(new(cardID, fileID, versionRowID, isDeleted));
            }
        }

        private static async Task<(ICardFileSource? FromSource, ICardFileSource? ToSource)> GetFileSourcesAsync(
            IDbScope dbScope,
            CardFileSourceType fromSourceType,
            CardFileSourceType toSourceType,
            CancellationToken cancellationToken)
        {
