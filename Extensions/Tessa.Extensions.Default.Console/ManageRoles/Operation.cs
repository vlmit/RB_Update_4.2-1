    ...
    public sealed class Operation : ConsoleOperation<OperationContext>
    {
        ...
        public override async Task<int> ExecuteAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                foreach (CommandType commandType in context.Commands ?? Enumerable.Empty<CommandType>())
                {
                    switch (commandType)
                    {
                        case CommandType.SyncAllDeputies:
                            ...
                            validationResult.Add(
                                await this.roleManagerService.SyncAllDeputiesAsync(context.BulkSize, context.SyncChangedOnly, cancellationToken));
                            break;
                        ...
                        case CommandType.RecalcAllSmartRoleGenerators:
                            await this.Logger.InfoAsync("Recalculating all smart role generators...");
                            validationResult.Add(await this.roleManagerService.RecalcSmartRoleGeneratorsAsync(cancellationToken: cancellationToken));
                            break;

                        case CommandType.RecalcDynamicRoles:
                            await this.Logger.InfoAsync("Recalculating dynamic role(s): {0}...",
                                string.Join(", ", context.Identifiers?.Select(x => $"\"{x}\"") ?? Enumerable.Empty<string>()));
                            foreach (Guid identifier in context.Identifiers ?? Enumerable.Empty<Guid>())
                            {
                                ...
                                if (itemResult.Items.Count > 0)
                                {
                                    validationResult.AddError(
                                        ...
                                        itemResult.HasErrors
                                            ? "Errors are occurred when recalculating dynamic role {0:B}"
                        ...
                        case CommandType.RecalcRoleGenerators:
                            await this.Logger.InfoAsync("Recalculating role generator(s): {0}...",
                                string.Join(", ", context.Identifiers?.Select(x => $"\"{x}\"") ?? Enumerable.Empty<string>()));
                            foreach (Guid identifier in context.Identifiers ?? Enumerable.Empty<Guid>())
                            {
                                ...
                                if (itemResult.Items.Count > 0)
                                {
                                    validationResult.AddError(
                                        typeof(Operation),
                                        itemResult.HasErrors
                                            ? "Errors are occurred when recalculating role generator {0:B}"
                        ...
                        case CommandType.RecalcSmartRoleGenerators:
                            await this.Logger.InfoAsync("Recalculating smart role generator(s): {0}...",
                                string.Join(", ", context.Identifiers?.Select(x => $"\"{x}\"") ?? Enumerable.Empty<string>()));

                            if (context.Identifiers is null || context.Identifiers.Length == 0)
                            {
                                break;
                            }

                            var recalcSmartRoleGeneratorsResults =
                                await this.roleManagerService.RecalcSmartRoleGeneratorsAsync(
                                    context.Identifiers.ToList(),
                                    cancellationToken);

                            validationResult.Add(recalcSmartRoleGeneratorsResults);
