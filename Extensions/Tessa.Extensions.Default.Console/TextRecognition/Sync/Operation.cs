    ...
    public sealed class Operation : Base.Operation<OperationContext, OcrSyncRequest, OcrSyncResponse>
    {
        ...
        public override async Task<int> ExecuteAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var (operationID, token) = await this.CreateOperationAsync(context, cancellationToken);
                ...
                cancellationToken.ThrowIfCancellationRequested();
                var isSuccessful = await this.WaitOperationAsync(operationID.Value, context, token, cancellationToken);
                if (!isSuccessful)
                {
                    return -1;
                }
                ...
                cancellationToken.ThrowIfCancellationRequested();
                await using var result = await this.GetOperationResultAsync(operationID.Value, context, token, cancellationToken);
                if (result is null)
                {
                    return -1;
                }
                ...
                var contentPath = Path.Combine(context.TargetFolderPath, $"{fileNameWithoutExtension}{OcrCommon.PdfContentFileExtension}");
        ...
        /// <inheritdoc/>
        protected override async Task<(Guid? OperationID, JinniBalancingToken? Token)> CreateOperationAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            ...
            var result = await this.ocrService.CreateOperationAsync(request, context.ValidationResult, token: null, cancellationToken);
