...
namespace Tessa.Extensions.Default.Console.RebuildCalendar
{
    public sealed class Operation : ConsoleOperation<OperationContext>
    {
        ...
        private async Task<int> RebuildCalendarAsync(Guid calendarID, CancellationToken cancellationToken = default)
        {
            ...
            var errorCardID = storeResponse.Info.Get<Guid?>(BusinessCalendarHelper.RebuildOperationErrorCardIDKey);
            if (errorCardID.HasValue)
            {
                ...
                var errorCardRequestResult =
                    await this.cardRepository.GetAsync(errorCardRequest, cancellationToken);
                ...
                if (errorCardGetResult.IsSuccessful &&
                    ...
                    errorCard.Sections.TryGetValue("Errors", out CardSection? errorsSection))
                {
                    await this.Logger.LogResultAsync(
                        ValidationResult.FromText(errorsSection.RawFields.Get<string>("Text"),
                ...
                if (errorCardGetResult.Items.Count != 1 ||
                    errorCardGetResult.Items[0].Key != CardValidationKeys.InstanceNotFound)
                {
                    await this.Logger.LogResultAsync(errorCardGetResult);
                }
            }
        ...
        public override async Task<int> ExecuteAsync(OperationContext context, CancellationToken cancellationToken = default)
        {
            ...
            var state = await this.operationRepository.GetStateAsync(OperationTypes.CalculatingCalendar, cancellationToken);
            ...
            if (context.CalendarIntID is null)
            {
