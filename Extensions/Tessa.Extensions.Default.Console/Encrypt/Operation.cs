...
namespace Tessa.Extensions.Default.Console.Encrypt
{
    public static class Operation
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                await logger.ErrorAsync("The file to encrypt does not exist: {0}", filePath);
                return -1;
            }
            ...
            encryptedFilePath = DefaultConsoleHelper.NormalizeFilePath(encryptedFilePath);
            ...
            return await companion.ProcessAndGetAsync(
                ...
                async (c, ct) =>
                {
                    ...
                    try
                    {
                        // encryption uses public key, so there is no need to supply the private key file
                        ...
                        await using var outputStream = FileHelper.Create(string.IsNullOrEmpty(encryptedFilePath) ? filePath : encryptedFilePath);

                        await encryptionService.EncryptDataAsync(certificate, inputStream, outputStream, ct);
                    }
                    catch (Exception e) when (e is not OperationCanceledException)
                    {
                        await logger.LogExceptionAsync("Error encrypting file", e);
                    ...
                    await logger.InfoAsync("File encrypted successfully.");
                    return 0;
                }, cancellationToken);
        }
