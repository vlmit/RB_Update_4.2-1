...
namespace Tessa.Extensions.Default.Console.MigrateDatabase
{
    public static class Command
    {
            ...
            [Argument("nologo")] [LocalizableDescription("CLI_NoLogo")] bool nologo = false)
        {
            ...
            var logger = new ConsoleLogger(LogManager.GetLogger(nameof(MigrateDatabase)), TextWriter.Synchronized(stdOut), TextWriter.Synchronized(stdErr), quiet);

            int result = await Operation.ExecuteAsync(logger, configurationString, databaseName, targetCS, targetDatabaseName, threadCount, bulkSize);
