...
DEALLOCATE query_cursor;";
        ...
        private static ValueTask<string?> CleanConnectionAsync(
            string connectionString,
            DbProviderFactory dbProviderFactory,
            IDbConnectionStringCleaner? connectionStringCleaner = null,
            CancellationToken cancellationToken = default) =>
            connectionStringCleaner is not null
                ? connectionStringCleaner.CleanConnectionSafeAsync(dbProviderFactory.GetDbms(), connectionString, cancellationToken)
                : new(connectionString);

        private static (string ConnectionString, DbProviderFactory DbProviderFactory, ConfigurationDataProvider ConfigurationDataProvider)
            ResolveConnectionParameters(IConfigurationManager configurationManager, string? sourceConfigurationString, string? sourceDatabaseName)
        {
            var configuration = configurationManager.Configuration;
            var (dataProvider, configurationConnection) =
                configuration.GetConfigurationDataProvider(sourceConfigurationString);

            var configurationDataProvider = dataProvider;

            var dbProviderFactory = configuration
            ...
            {
                var connectionBuilder = NotNullOrThrow(dbProviderFactory.CreateConnectionStringBuilder());
        ...
        {
            var connection = sourceDbProviderFactory.CreateConnection()
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                var context = NotNullOrThrow(contextArray[threadIndex]);
        ...
        private static async Task MigrateDataTableAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var columns = table.Columns.GetPhysicalColumns().Where(x => !x.IsDbVirtual()).ToArray();
            if (columns.Length == 0)
            {
                return;
            }
            ...
            try
            {
                ...
                {
                    var dataTypes = columns
                        .Select(x => NotNullOrThrow(x.Type).DbType)
                        .ToArray();

                    var bulkInsertParameters = context.InsertExecutor.PrepareBulkInsert(
                        context.TargetDb,
                    ...
                    for (var i = 0; i < parameterValueArrays.Length; i++)
                    {
                        parameterValueArrays[i] = context.ValueArrayPool.Rent();
                    }
                    ...
                    for (var i = 0; i < taskParameterValueArrays.Length; i++)
                    {
                        taskParameterValueArrays[i] = context.ValueArrayPool.Rent();
                    }

                    var insertTask = Task.CompletedTask;
                    var row = 0;

                    await using (var reader = await context.SourceDb.ExecuteReaderAsync(CommandBehavior.SequentialAccess, cancellationToken))
                    {
                        while (await reader.ReadAsync(cancellationToken))
                        {
                            ...
                            if (row == context.BulkSize)
                            {
                                ...
                                var currentTaskParameterValueArrays = parameterValueArrays;
                    ...
                    // если есть несколько строк, ожидающих вставки (индексы от 0 до row-1), а таблица уже закончилась
                    if (row > 0)
                    {
                        // освободить массивы
                        foreach (var arr in parameterValueArrays)
                        {
                            context.ValueArrayPool.Return(arr);
                        }
                ...
                else
                {
                    ...
                    var targetCommandParameters =
                        columns
                            .Select(x => context.TargetDb.Parameter(x.Name, NotNullOrThrow(x.Type).DbType.ToDataType()))
                            .ToArray();
                    ...
                    await using var reader = await context.SourceDb.ExecuteReaderAsync(cancellationToken);
                    while (await reader.ReadAsync(cancellationToken))
                    {
                        for (var i = 0; i < targetCommandParameters.Length; i++)
                        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            switch (dbms)
            {
                ...
                case Dbms.PostgreSql:
                    ...
                    // только с системными не отключаемыми FK (остальные мы уже прибили);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await companion.ProcessAndGetAsync(
                ...
                async (c, ct) =>
                {
                    var assemblyInfo = c.Container.ResolveAssemblyInfo();
                    await ConsoleAppHelper.LogLoadedExtensionsAsync(logger, assemblyInfo, SessionType.Server);
                    var connectionStringCleaner = c.Container.TryResolve<IDbConnectionStringCleaner>();

                    await logger.InfoAsync("Connecting to the source database");

                    var (sourceConnectionString, sourceDbProviderFactory, sourceConfigurationDataProvider) =
                        ResolveConnectionParameters(c.ConfigurationManager!, sourceConfigurationString, sourceDatabaseName);

                    await logger.InfoAsync(
                        await CleanConnectionAsync(sourceConnectionString, sourceDbProviderFactory, connectionStringCleaner, ct));

                    var sourceDbScope = new DbScope(c.ConfigurationManager!, () =>
                    ...
                    if (!await sourceDatabaseSchemeService.IsStorageExistsAsync(ct))
                    {
                        await logger.ErrorAsync("Scheme doesn't exists in the source database");
                        return -1;
                    }

                    if (!await sourceDatabaseSchemeService.IsStorageUpToDateAsync(ct))
                    {
                    ...
                    SchemeDatabase database = new(SchemeDatabaseNames.Original);
                    await database.RefreshAsync(sourceDatabaseSchemeService, ct);

                    SchemeDatabase databaseFinal = new(SchemeDatabaseNames.Original);
                    await databaseFinal.RefreshAsync(database, ct);
                    ...
                    var tables = database.Tables.OrderBy(x => x.Name).ToList();
                    ...
                    foreach (var table in tables)
                    {
                        if (!table.IsSealed)
                        {
                            // отключаем внешние ключи во всех комплексных колонках, в которых можем;
                    ...
                    var (targetConnectionString, targetDbProviderFactory, targetConfigurationDataProvider) =
                        ResolveConnectionParameters(c.ConfigurationManager!, targetConfigurationString, targetDatabaseName);

                    await logger.InfoAsync(
                        await CleanConnectionAsync(targetConnectionString, targetDbProviderFactory, connectionStringCleaner, ct));

                    var targetDbScope = new DbScope(c.ConfigurationManager!, () =>
                    ...
                    if (!await targetDatabaseSchemeService.IsStorageExistsAsync(ct))
                    {
                        await logger.InfoAsync("Scheme doesn't exists in the target database, creating it");
                        await targetDatabaseSchemeService.CreateStorageAsync(ct);
                    }

                    if (!await targetDatabaseSchemeService.IsStorageUpToDateAsync(ct))
                    {
                        await logger.InfoAsync("Scheme isn't up-to-date in the target database, upgrading it");
                        await targetDatabaseSchemeService.UpdateStorageAsync(ct);
                    }

                    await logger.InfoAsync("Importing initial scheme to the target database");
                    var sqInitial = new SchemeSubmittingQueue(database, targetDatabaseSchemeService);
                    try
                    {
                        await sqInitial.RefreshAsync(ct);
                        await sqInitial.SubmitAsync(ct);
                    }
                    catch (Exception ex) when (ex is not OperationCanceledException && sqInitial.FaultedItem is not null)
                    {
                        // ignored
                    }

                    if (sqInitial.FaultedItem is { } faultedItemInitial)
                    {
                        var message = SchemeHelper.GetFaultedItemMessage(sqInitial.FaultedItem);
                        if (faultedItemInitial.Exception is { } ex)
                        {
                    ...
                    try
                    {
                        for (var threadIndex = 0; threadIndex < contextArray.Length; threadIndex++)
                        {
                            ...
                            if (threadIndex == 0)
                            {
                                ...
                                await SetupIndicesForeignKeysAndTableLoggingAsync(false, logger, targetDbms, targetDb, targetBuilderFactory, tables, ct);
                            }
                            else
                            {
                                targetBuilderFactory = new QueryBuilderFactory(targetDbms);
                            ...
                            var insertExecutor = c.Container.Resolve<IBulkInsertExecutor>();
                        ...
                        await MigrateDataAsync(logger, contextArray, tables, ct);

                        if (contextArray.Length > 0)
                        {
                            // включаем внешние ключи на таблицах, которые были отключены выше перед миграцией
                            var context = NotNullOrThrow(contextArray[0]);
                            await SetupIndicesForeignKeysAndTableLoggingAsync(true, logger, targetDbms, context.TargetDb, context.TargetBuilderFactory, tables, ct);

                            // асинхронно закрываем соединения
                            foreach (var dbContext in contextArray)
                            {
                    ...
                    var sqFinal = new SchemeSubmittingQueue(databaseFinal, targetDatabaseSchemeService);
                    try
                    {
                        await sqFinal.RefreshAsync(ct);
                        await sqFinal.SubmitAsync(ct);
                    }
                    catch (Exception ex) when (ex is not OperationCanceledException && sqFinal.FaultedItem is not null)
                    {
                        // ignored
                    }

                    if (sqFinal.FaultedItem is { } faultedItemFinal)
                    {
                        var message = SchemeHelper.GetFaultedItemMessage(sqFinal.FaultedItem);
                        if (faultedItemFinal.Exception is { } ex)
                        {
                            await logger.LogExceptionAsync(message, ex);
                        }
                        else
