...
namespace Tessa.Extensions.Default.Console.SchemeCompact
{
    public static class Operation
    {
            ...
            IEnumerable<string>? excludedPartitions)
        {
            ...
            var database = new SchemeDatabase(SchemeDatabaseNames.Original);
            ...
            try
            {
                ...
                var schemeVersion = await fileSchemeService.GetStorageVersionAsync(System.Threading.CancellationToken.None);
                scope = new();

                database.WriteXmlWithVersion(xmlWriter, schemeVersion);
            }
            finally
            {
                scope?.Dispose();
