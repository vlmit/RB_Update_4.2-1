    ...
    public static class Operation
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (string.IsNullOrWhiteSpace(app.Version))
            {
                ...
                app.Version = version?.ToString() ?? BuildInfo.VersionObjectString;
            }
            ...
            if (string.IsNullOrWhiteSpace(icoPath))
            {
                ...
                icoPath = Path.Combine(appFolder, $"{iconFileNameWithoutExtension}.png");

                if (iconFileNameWithoutExtension.EndsWith("32", StringComparison.Ordinal) && !File.Exists(icoPath))
                {
                    // для TessaClient32.exe - ищем иконку в TessaClient.png, если не нашли TessaClient32.png
                    icoPath = Path.Combine(appFolder, $"{iconFileNameWithoutExtension[..^2]}.png");
                }
            }
            ...
            if (File.Exists(icoPath))
            {
                try
                {
                    app.Icon = await File.ReadAllBytesAsync(icoPath, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    outputPath = $"{GetDefaultAppFileNameWithoutExtension(app)}.jcard";
                }
                else if (outputPath.EndsWith('/') || outputPath.EndsWith('\\'))
                {
                    outputPath = Path.Combine(outputPath[..^1], $"{GetDefaultAppFileNameWithoutExtension(app)}.jcard");
                }
                else if (!outputPath.EndsWith(".jcard", StringComparison.OrdinalIgnoreCase)
                         && Directory.Exists(outputPath))
                {
                    outputPath = Path.Combine(outputPath, $"{GetDefaultAppFileNameWithoutExtension(app)}.jcard");
                }
                ...
                await using var writer = new StreamWriter(targetStream, Encoding.UTF8, FileHelper.DefaultFileBufferSize, leaveOpen: true);
                writer.NewLine = "\n";
                await using var jsonWriter = new OptimizedJsonTextWriter(writer);
