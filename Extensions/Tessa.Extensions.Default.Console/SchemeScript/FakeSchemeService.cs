    ...
    public sealed class FakeSchemeService : SystemSchemeService
    {
            ...
            : base(SchemeServiceOptions.None)
        {
            ...
            FakeConnection connection = CreateConnection(dbms, output, outputTransactions, executor => executor
                .Query(SqlHelper.GetDbmsVersionQuery(dbms), this.GetDbmsVersion)
            ...
            var dbScope = new DbScope(ConfigurationManager.Default, () => connection.AsDbManager());
        ...
        private FakeDataReader GetDbmsVersion(FakeCommand command) => new(new object[,] { { this.dbmsVersion.ToString() } });
        ...
        protected override ValueTask<SchemeDatabaseInfo> GetDatabaseInfoOverrideAsync(CancellationToken cancellationToken = default)
        {
            return this.service.GetDatabaseInfoAsync(cancellationToken);
        }
        ...
        /// <inheritdoc/>
        public override ValueTask InvalidateCacheAsync() =>
            this.service.InvalidateCacheAsync();

        /// <inheritdoc/>
        public override ValueTask<bool> InvalidateCacheIfChangedAsync() =>
            this.service.InvalidateCacheIfChangedAsync();

        /// <inheritdoc/>
        public override ValueTask EnsureInvalidateCacheSubscribedAsync(CancellationToken cancellationToken = default) =>
            ValueTask.CompletedTask;
        ...
        private static DatabaseSchemeService.KnownTables ToTable(Guid? idValue)
        {
            if (idValue is not { } id)
            {
