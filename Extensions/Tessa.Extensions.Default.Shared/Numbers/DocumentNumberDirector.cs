...
namespace Tessa.Extensions.Default.Shared.Numbers
{
        ...
        NumberDirector
    {
        ...
        protected override async ValueTask<string?> OnGettingDigestAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            INumberObject number = context.NumberObject;
            string? fullNumber = number.IsEmpty()
                ? await this.TryGetFullNumberAsync(context, NumberTypes.Primary, cancellationToken)
                : number.FullNumber;

            string? result = string.IsNullOrWhiteSpace(fullNumber)
                ? null
                : fullNumber.Trim();
        ...
        protected override async ValueTask<bool> OnCreatingCardAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            ...
            NumberQueue? numberQueue = await this.TryGetNumberQueueCoreAsync(context, createIfNotExists: true, cancellationToken: cancellationToken)
            ...
            numberQueue.Add(
                ...
                context.GetOverriddenServiceType() == CardServiceType.Client
        ...
        protected override async ValueTask<bool> OnSavingNewCardAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            ...
            INumberObject reservedNumber = await context.Builder
                .ReserveAndCommitAtServerAsync(context, NumberTypes.Primary, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        private static async ValueTask<bool> ReleaseOnDeletingAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            ...
            if (primaryNumberIsSequential && secondaryNumberIsSequential)
            {
                ...
                {
                    // если номер один, то или это проектный номер, или это регистрационный без проектного
                    if (type is { DocNumberRegularAutoAssignmentID: not KrDocNumberRegularAutoAssignmentID.None })
                    {
                    ...
                    {
                        result = type is not { UseRegistration: true, ReleaseRegistrationNumberOnFinalDeletion: true }
                            || await context.Composer.ReleaseNumberAsync(context, primaryNumber, cancellationToken).ConfigureAwait(false);
                    }
                }
                else
        ...
        protected override async ValueTask<bool> OnReleasingNumberFromControlAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            if (context.Session.Type == SessionType.Server)
            {
                (bool releaseIsQueued, _) = await context.Builder.ReleaseAndCommitAtServerAsync(context, context.NumberObject, cancellationToken)
                    .ConfigureAwait(false);
                if (!releaseIsQueued)
                {
                    return false;
        ...
        private static async ValueTask<KrState?> GetCoreForKrStateAsync(INumberContext context, CancellationToken cancellationToken = default)
        {
            ...
            KrState stateFromDb = await KrProcessSharedHelper.GetKrStateAsync(context.Card.ID, context.DbScope, cancellationToken).ConfigureAwait(false) ??
                KrState.Draft;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (eventType == NumberEventTypes.DeletingBackupCard)
            {
                ...
                return type is null or { UseRegistration: true, ReleaseRegistrationNumberOnFinalDeletion: true }
                    || type.ReleaseRegularNumberOnFinalDeletion;
            }
