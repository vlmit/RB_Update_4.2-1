    ...
    public class DefaultEDSManager : CAdESManager
    {
            ...
            CancellationToken cancellationToken = default)
        {
            if (certificate != null)
            {
                ...
                string serialNumber;
                using (var certObj = SignatureHelper.LoadCertificate(certificate))
                {
                ...
                using (X509Store store = new X509Store(StoreName.My, StoreLocation.CurrentUser))
                {
                    ...
                    {
                        if (string.Equals(certItem.SerialNumber, serialNumber, StringComparison.OrdinalIgnoreCase))
                        {
                            cert = certItem;
                            break;
                        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!success)
            {
                errorText = LocalizeName("Platform_EDSCheckErrors");
