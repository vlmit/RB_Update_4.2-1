...
namespace Tessa.Extensions.Default.Shared.EDS
{
    public abstract class CAdESManager : ICAdESManager
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var (digestOid, encryptOid, _) = SignatureHelper.GetDigestAndEncryptOid(settingsCard, certificate);
            ...
            byte[] signatureValue = await this.GenerateSignatureAsync(certificate, new MemorySignatureFile(toBeSigned), digestOid, cancellationToken).ConfigureAwait(false);

            byte[] signature = await this.MakeBesSignaturePkcs7Async(certificate, file, signingTime, signatureValue, digestOid, encryptOid, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<byte[]> GetBesSignatureFromExtendedAsync(byte[] signature, CancellationToken cancellationToken = default)
        {
            ...
            return CAdESSignatureHelper.GetSignedData(response).Signature;
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            CAdESSignatureHelper.SetToBeSignedInfo(
                request,
                certificate,
                await file.GetBase64Async(cancellationToken).ConfigureAwait(false),
                signingTime,
                digestAlgorithmOid,
        ...
        public async Task<byte[]> GetToBeSignedDocumentWithHashAsync(
            byte[] certificate,
            ISignatureFile hash,
            DateTime signingTime,
            string digestAlgorithmOid = null,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            CAdESSignatureHelper.SetToBeSignedWithHashInfo(
                request,
                certificate,
                await hash.GetBase64Async(cancellationToken).ConfigureAwait(false),
                signingTime,
                digestAlgorithmOid,
                encryptionAlgorithmOid
            );
        ...
        public async Task<byte[]> MakeBesSignaturePkcs7WithSignedAttributesAsync(
            byte[] certificate,
            ISignatureFile signedAttributes,
            DateTime signingTime,
            byte[] signature,
            string digestAlgorithmOid = null,
            string encryptionAlgorithmOid = null,
            CancellationToken cancellationToken = default)
        {
            ...
            CAdESSignatureHelper.SetBesSignaturePkcs7InfoWithSignedAttributes(
                request,
                certificate,
                await signedAttributes.GetBase64Async(cancellationToken).ConfigureAwait(false),
                signingTime,
                signature,
                digestAlgorithmOid,
                encryptionAlgorithmOid
