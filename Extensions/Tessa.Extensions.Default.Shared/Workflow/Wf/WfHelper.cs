...
namespace Tessa.Extensions.Default.Shared.Workflow.Wf
{
    ...
    public static class WfHelper
    {
        ...
        /// <summary>
        /// Ключ с количеством дней на которое срок исполнения дочерней резолюции может превышать родительскую.
        /// </summary>
        public static string CheckSafeLimitKey = StorageHelper.SystemKeyPrefix + "CheckSafeLimit";

        /// <summary>
        /// Ключ для плановой даты родителя при проверке SafeLimit для дочерней резолюции.
        /// </summary>
        public static string ParentPlannedKey = StorageHelper.SystemKeyPrefix + "ParentPlanned";

        /// <summary>
        /// Ключ для времени сохранения для проверки SafeLimit для дочерней резолюции.
        /// </summary>
        public static string StoreDateTimeKey = StorageHelper.SystemKeyPrefix + "StoreDateTime";
        ...
        /// <summary>
        /// Имя формы в задании резолюций, используемой для отправки задания.
        /// </summary>
        public const string SendToPerformerFormName = "SendToPerformer";
        ...
        public const string ResettingFieldsAfterTaskIsCompletedAsModifiedKey =
            StorageHelper.SystemKeyPrefix + "resettingFields";
            ...
            bool limitLength = true)
        {
            ...
            if (completionOptionID.HasValue)
            {
                state = completionOptionID.Value == DefaultCompletionOptions.Revoke
                    ? string.Format(LocalizeName("WfResolution_State_Revoked"), Localize(userName))
                    : string.Format(LocalizeName("Cards_TaskState_Completed"), Localize(userName));
            }
        ...
        public static void SetResponseCard(CardResponseBase response, Card card)
        {
            ...
            if (card is not null)
            {
                response.Info[ResponseCardKey] = card.GetStorage();
            }
            else
            {
                response.TryGetInfo()?.Remove(ResponseCardKey);
            }
        }
        ...
        public static Card TryGetResponseCard(CardResponseBase response)
        {
            ...
            return response.TryGetInfo()?.TryGet<Dictionary<string, object>>(ResponseCardKey) is { } cardStorage
                ? new Card(cardStorage)
                : null;
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using (dbScope.Create())
            {
                ...
                {
                    await FillRolesAsync(db, builderFactory, roleGetStrategy, taskPerformersRoles, roleRecord.Item1, cancellationToken).ConfigureAwait(false);
                }
            }
        ...
        private static async Task FillRolesAsync(
            DbManager db,
            IQueryBuilderFactory builderFactory,
            IRoleGetStrategy roleGetStrategy,
