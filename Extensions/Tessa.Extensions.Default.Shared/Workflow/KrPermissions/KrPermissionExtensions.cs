...
namespace Tessa.Extensions.Default.Shared.Workflow.KrPermissions
{
    ...
    public static class KrPermissionExtensions
    {
        ...
        /// <returns>Асинхронная задача.</returns>
        public static async Task SetCardAccessAsync(
            this KrPermissionExtendedCardSettingsStorage extendedCardSettings,
            bool isAllowed,
        ...
        {
            ThrowIfNull(extendedCardSettings);
        ...
        /// <param name="fields">Идентификаторы полей, для которых устанавливаются права. Если не заданы, то доступ устанавливается для всей секции.</param>
        public static void SetCardAccess(
            this KrPermissionExtendedCardSettingsStorage extendedCardSettings,
            bool isAllowed,
            Guid sectionID,
            ICollection<Guid>? fields)
        {
            ...
            var sectionSettings = extendedCardSettings.SectionSettings.FirstOrDefault(x => x.ID == sectionID);
            if (sectionSettings is null)
            {
                sectionSettings = extendedCardSettings.SectionSettings.Add();
                    ...
                    && fields.Count > 0)
                {
                    if (isAllowed)
                    {
                        sectionSettings.AllowedFields = [.. fields];
                    }
                    else
                    {
                        sectionSettings.DisallowedFields = [.. fields];
                    }
                }
                else
                {
            ...
            else
            {
                    ...
                    && fields.Count > 0)
                {
                    ...
                    fromFields.RemoveWhere(toFields.Contains);
