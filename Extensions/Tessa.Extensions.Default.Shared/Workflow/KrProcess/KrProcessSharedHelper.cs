...
namespace Tessa.Extensions.Default.Shared.Workflow.KrProcess
{
    public static class KrProcessSharedHelper
    {
        ...
        /// <summary>
        /// Пытается получить идентификатор типа документа для заданной карточки.
        /// </summary>
        /// <param name="card">Карточка, для которой требуется получить идентификатор типа документа.</param>
        /// <param name="docTypeId">Найденный идентификатор типа документа или значение <see langword="null"/>, если его не удалось получить.</param>
        /// <param name="checkKrToken">
        /// Определяет, нужно ли проверять тип документа по <see cref="KrToken"/> карточки. Следует использовать только в ситуации,
        /// когда токен был проверен или доподлинно известно, что он будет проверен.
        /// </param>
        /// <returns><c>true</c>, если идентификатор типа был найден, иначе <c>false</c>.</returns>
        /// <remarks>Метод не выполняет запросов к базе данных.</remarks>
        public static bool TryGetDocTypeID(
            Card card,
            out Guid? docTypeId,
            bool checkKrToken = false)
        {
            ThrowIfNull(card);
            ...
            {
                docTypeId = docTypeIDObj as Guid?;
                return true;
            }


            if (card.TryGetInfo()?.TryGetValue(Keys.DocTypeID, out var docTypeIDCached) == true)
            {
                // Тип был закэширован в Card.Info.
                docTypeId = docTypeIDCached as Guid?;
                return true;
            }

            if (checkKrToken
                && card.TryGetInfo() is { } info
                && KrToken.TryGet(info) is { } token
                && token.TryGetDocTypeID(out var tokenDocTypeID))
            {
                docTypeId = tokenDocTypeID;
                return true;
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // В ещё не созданных карточках тип документа либо находится в атрибутах, либо же не используется.
            // Даже если это не так, запрос ниже для таких карточек не имеет смысла т.к. карточка ещё не создана.
            if (TryGetDocTypeID(card, out var docTypeID) || card.StoreMode == CardStoreMode.Insert)
            {
                return docTypeID;
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Карточка не содержит секции или карточка могла быть не полностью инициализирована.
            if (!result.HasValue || result.Value == KrState.Draft)
            {
                // возможно удалённая карточка
                var satelliteCard = CardSatelliteHelper.TryGetSingleSatelliteCardFromList(
                    card,
                    CardSatelliteHelper.SatellitesKey,
                    DefaultCardTypes.KrSatelliteTypeID);

                if (satelliteCard is not null)
                {
                    result = (KrState?) satelliteCard.Sections[KrApprovalCommonInfo.Name].RawFields.Get<int?>(KrApprovalCommonInfo.StateID);
                }
            }
        ...
        /// </param>
        /// <param name="dbScope"><see cref="IDbScope"/></param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (krCardType.UseDocTypes)
            {
                ...
                if (dbScope is null)
                {
                    TryGetDocTypeID(card, out docTypeID);
                }
                else
                {
                    docTypeID = await GetDocTypeIDAsync(card, dbScope, cancellationToken);
                }
            ...
            string orderFieldName)
        {
            ...
            if (rows.Count == 1)
            {
                ...
                if (row.Get<int>(orderFieldName) != 0)
                {
                    row[orderFieldName] = Int32Boxes.Zero;
                }
            ...
            {
                var row = rows[i];
                var iRowT = (IDictionary<string, object>) row;

                var j = i - 1;

                while (j >= 0)
                {
                    var jRowT = (IDictionary<string, object>) rows[j];

                    if (jRowT.Get<int>(orderFieldName) > iRowT.Get<int>(orderFieldName))
                    {
                        rows[j + 1] = rows[j];
                        j--;
                    }
            ...
            for (var i = 0; i < rows.Count; i++)
            {
                ...
                if (row.Get<int>(orderFieldName) != i)
                {
                    row[orderFieldName] = Int32Boxes.Box(i);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="rows">Список сортируемых строк. Тип значений: <c>IDictionary&lt;string, object&gt;</c>.</param>
        /// <param name="orderFieldName">Имя поля, содержащего порядок сортировки (тип значения: <see cref="int"/>).</param>
        /// <param name="parentRowIDFieldName">Имя поля, содержащего идентификатор (тип значения: <see cref="Guid"/>) родительской строки.</param>
        /// <remarks>
        /// Метод выполняет следующие действия:
        ///
        /// <list type="number">
            ...
            string parentRowIDFieldName)
        {
            ThrowIfNull(rows);
            ThrowIfNullOrEmpty(orderFieldName);
            ThrowIfNullOrEmpty(parentRowIDFieldName);
            ...
            {
                var rowObj = rows[i];
                var iRowT = (IDictionary<string, object>) rowObj;

                Guid parentRowID = iRowT.Get<Guid>(parentRowIDFieldName);
                ...
                rows[jPlusOne] = rowObj;
            }

            // Восстановление последовательности порядка строк.
            var orders = new Dictionary<Guid, int>(1);

            foreach (var rowObj in rows)
            {
                var row = (IDictionary<string, object>) rowObj;

                var parentRowID = row.Get<Guid>(parentRowIDFieldName);

                if (!orders.TryGetValue(parentRowID, out var i))
                {
                    orders.Add(parentRowID, 0);
                }
                ...
                string parentRowIDFieldName)
            {
                for (int i = startIndex; i < rows.Count; i++)
                {
                    ...
                    if (row.Get<Guid>(parentRowIDFieldName) == parentRowID)
                    {
                        return i;
                    }
                }
