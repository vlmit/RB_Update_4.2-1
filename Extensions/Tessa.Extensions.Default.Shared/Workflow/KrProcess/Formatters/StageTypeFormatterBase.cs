...
{
    /// <summary>
    /// Базовая абстрактная реализация <see cref="IStageTypeFormatter"/>.
    /// </summary>
    public abstract class StageTypeFormatterBase :
        IStageTypeFormatter
    {
        ...
        {
            var containsTimeLimit = context.StageRow.TryGetValue(KrConstants.KrStages.TimeLimit, out var timeLimit);
            var containsPlanned = context.StageRow.TryGetValue(KrConstants.KrStages.Planned, out var planned);
            ...
            if (context.StageRow.TryGetValue(KrConstants.KrSinglePerformerVirtual.PerformerName,
            ...
            else if (context.Card.Sections.TryGetValue(KrConstants.KrPerformersVirtual.Synthetic, out var appSec))
            {
                    ...
                    ListStorage<CardRow> rows)
                {
                    foreach (var row in rows)
                    {
                        if (row.State != CardRowState.Deleted
                            && row.TryGet<Guid?>(KrConstants.KrPerformersVirtual.StageRowID)?.Equals(context.StageRow.RowID) == true
                            && row.TryGetValue(KrConstants.KrPerformersVirtual.PerformerName, out var nameObj)
            ...
            IStageTypeFormatterContext context)
        {
            ...
            if (context.Settings!.TryGetValue(KrConstants.KrSinglePerformerVirtual.PerformerName, out var performerNameObj)
                && performerNameObj is string performerName
                && !string.IsNullOrWhiteSpace(performerName))
            {
                DefaultSinglePerformerFormatting(performerName, context);
            }
            else if (context.Settings.TryGetValue(KrConstants.KrPerformersVirtual.Synthetic, out var performers)
                    ...
                    IList rows)
                {
                    foreach (var row in rows)
                    {
                        if (row is IDictionary<string, object?> rowStorage
                            && rowStorage.TryGetValue(KrConstants.KrPerformersVirtual.PerformerName, out var nameObj)
                            && nameObj is string name)
                        {
                            yield return RoleHelper.EscapeRoleNameForLocalization(name);
                        }
            ...
            IStageTypeFormatterContext context)
        {
            if (planned.HasValue)
            {
                context.DisplayTimeLimit =
                    FormatDateTimeWithoutSeconds(planned.Value.ToUniversalTime() + context.Session.ClientUtcOffset, convertToLocal: false)
                    + " UTC" + FormatUtcOffset(context.Session.ClientUtcOffset);
            }
            else if (timeLimit.HasValue)
            {
                context.DisplayTimeLimit = timeLimit.Value.ToString(CultureInfo.InvariantCulture)
                    + LocalizeName("KrProcess_WorkingDaysSuffix");
            }
            else
            {
