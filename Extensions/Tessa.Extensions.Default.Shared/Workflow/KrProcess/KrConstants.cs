...
namespace Tessa.Extensions.Default.Shared.Workflow.KrProcess
{
    ...
    public static class KrConstants
    {
        ...
        /// </summary>
        /// <remarks>
        /// Параметр (необязательный): <see cref="KrProcessID"/>.
        /// </remarks>
        ...
        /// <summary>
        /// Ключ, по которому содержится идентификатор экземпляра запускаемого или запущенного процесса. Тип: <see cref="Guid"/>.
        /// </summary>
        public const string KrProcessID = nameof(KrProcessID);
        ...
        /// <summary>
        /// Возвращает число типовых этапов в маршруте.
        /// </summary>
        /// <param name="hasApprovalProcess">Значение <see langword="true"/>, если карточка содержит процесс согласования, иначе - <see langword="false"/>.</param>
        /// <returns>Число типовых этапов в маршруте.</returns>
        /// <remarks>
        /// Для получения названий типовых этапов можно использовать метод <see cref="GetRouteStageNamesWithDefaultStages(IEnumerable{string}, bool)"/> и его перегрузки.
        /// </remarks>
        public static int GetDefaultStagesCount(
            bool hasApprovalProcess = false) =>
            GetRouteStageNamesWithDefaultStages(hasApprovalProcess: hasApprovalProcess).Count();

        /// <summary>
        /// Возвращает список названий этапов маршрута, включающий названия типовых этапов.
        /// </summary>
        /// <param name="customStageNames">Названия этапов из кастомного процесса или значение <see langword="null"/>, если выводятся названия только типовых этапов.</param>
        /// <param name="hasApprovalProcess">Значение <see langword="true"/>, если карточка содержит процесс согласования, иначе - <see langword="false"/>.</param>
        /// <returns>Названия этапов маршрута.</returns>
        /// <remarks>Число типовых этапов содержится в константе <see cref="GetDefaultStagesCount"/>.</remarks>
        public static IEnumerable<string> GetRouteStageNamesWithDefaultStages(
            IEnumerable<string> customStageNames = null,
            bool hasApprovalProcess = false)
        {
            yield return StageTypeDescriptors.HistoryManagementDescriptor.DefaultStageName;
            yield return StageTypeDescriptors.EditDescriptor.DefaultStageName;
            yield return StageTypeDescriptors.ApprovalProcessManagementDescriptor.DefaultStageName;

            if (hasApprovalProcess)
            {
                yield return StageTypeDescriptors.ApprovalProcessDescriptor.DefaultStageName;
            }

            if (customStageNames is not null)
            {
                foreach (var customStageName in customStageNames)
                {
                    yield return customStageName;
                }
            }

            yield return StageTypeDescriptors.ProcessManagementDescriptor.DefaultStageName;
        }

        /// <inheritdoc cref="GetRouteStageNamesWithDefaultStages(IEnumerable{string}, bool)"/>
        public static IEnumerable<string> GetRouteStageNamesWithDefaultStages(
            params string[] customStageNames) =>
            GetRouteStageNamesWithDefaultStages((IEnumerable<string>) customStageNames);
        ...
        /// </summary>
        public const string RaiseErrorWhenExecutionIsForbidden = StorageHelper.SystemKeyPrefix + nameof(RaiseErrorWhenExecutionIsForbidden);

        /// <summary>
        /// Коллекция идентификаторов типов карточек, определяющая порядок сохранения карточек заданных типов.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Максимальный уровень рекурсии, используемый по умолчанию в обобщённых табличных выражениях.
        /// </summary>
        public const int MaxRecursion = 500;

        /// <summary>
        /// Имя типа сигнала, по которому выполняется завершение процесса согласования в этапе "Процесс согласования".
        /// </summary>
        /// <remarks>Параметр: <see cref="ApprovalProcessSignalInstanceIDParam"/>.</remarks>
        public const string ApprovalProcessCompletedSignal = nameof(ApprovalProcessCompletedSignal);

        /// <summary>
        /// Имя типа сигнала, по которому запускается продолжение выполнения процесса согласования в этапе "Процесс согласования".
        /// </summary>
        /// <remarks>Параметр: <see cref="ApprovalProcessSignalInstanceIDParam"/>.</remarks>
        public const string ApprovalProcessContinueSignal = nameof(ApprovalProcessContinueSignal);

        /// <summary>
        /// Имя типа сигнала, по которому выполняется остановка процесса согласования в этапе "Процесс согласования".
        /// </summary>
        /// <remarks>Параметр: <see cref="ApprovalProcessSignalInstanceIDParam"/>.</remarks>
        public const string ApprovalProcessStopSignal = nameof(ApprovalProcessStopSignal);

        /// <summary>
        /// Имя типа сигнала, по которому выполняется уведомление этапа "Процесс согласования" о завершении отзыва.
        /// </summary>
        /// <remarks>
        /// Отдельное уведомление необходимо, т.к. вторичный процесс отзыва мог не выполнить отмену этапа.<para/>
        ///
        /// Параметр: <see cref="ApprovalProcessSignalInstanceIDParam"/>.
        /// </remarks>
        public const string ApprovalProcessStopCompletedSignal = nameof(ApprovalProcessStopCompletedSignal);

        /// <summary>
        /// Ключ, по которому в <see cref="IWorkflowSignal.Info"/> содержится идентификатор процесса согласования. Тип значения: <see cref="Guid"/>.
        /// </summary>
        public const string ApprovalProcessSignalInstanceIDParam = "InstanceID";

        /// <summary>
        /// Ключ, по которому в параметрах этапа <see cref="StageTypeDescriptors.ApprovalProcessManagementDescriptor"/> содержится список идентификаторов процессов согласования,
        /// которые должен обработать этап. Тип значения: <see cref="List{T}"/>, где <c>T</c> - <see cref="Guid"/>.
        /// </summary>
        public const string ApprovalProcessManagementInstanceIDsParam = "InstanceIDs";

        /// <summary>
        /// Идентификаторы типов стандартных этапов согласования.
        /// </summary>
        public static readonly IReadOnlyList<Guid> DefaultApprovalStageTypeIDList = [
            StageTypeDescriptors.ApprovalDescriptor.ID,
            StageTypeDescriptors.ApprovalProcessDescriptor.ID];

        /// <summary>
        /// Ключ, по которому содержится флаг, запрещающий обновление информации о внешнем процессе, к которому относится запускаемый процесс согласования. Тип: <see cref="bool"/>.
        /// </summary>
        public const string IsNotUpdateExternalProcess = nameof(IsNotUpdateExternalProcess);

        public static class Keys
        {
            /// <summary>
            /// Имя ключа, по которому в <see cref="CardInfoStorageObject.Info"/> объекта <see cref="Card"/> содержится информация о подписях этапов. Тип значения: <c>Dictionary&lt;string, object&gt;</c>.
            /// </summary>
            ...
            public const string Compile = StorageHelper.SystemKeyPrefix + "Compile";

            public const string CompileWithValidationResult = StorageHelper.SystemKeyPrefix + "CompileWithValidationResult";

            public const string CompileAll = StorageHelper.SystemKeyPrefix + "CompileAll";

            public const string CompileAllWithValidationResult = StorageHelper.SystemKeyPrefix + "CompileAllWithValidationResult";

            /// <summary>
            /// Имя ключа, по которому в поле KrUniversalTaskOptions.Settings может сохраняться информация о функциональных ролях, для которых доступен вариант завершения универсального задания.
            /// </summary>
            public const string OptionFunctionRoles = StorageHelper.SystemKeyPrefix + nameof(OptionFunctionRoles);

            /// <summary>
            /// Имя ключа, по которому через <see cref="CardInfoStorageObject.Info"/> в <see cref="CardTask"/> предаётся информация о наличии в настраиваемом задании скрытых по умолчанию вариантов завершения.
            /// </summary>
            public const string HiddenByDefaultKey = StorageHelper.SystemKeyPrefix + "hiddenByDefault";
            ...
            /// <summary>
            /// Ключ, по которому в <see cref="T:Tessa.Extensions.Default.Server.Workflow.KrObjectModel.Stage.InfoStorage"/> содержится дополнительная информация по вложенному процессу. Тип значения: <see cref="IDictionary{TKey, TValue}"/>, где TKey - <see cref="string"/>, TValue - <see cref="object"/>.
            /// </summary>
            ...
            /// <summary>
            /// Ключ, по которому в <see cref="IWorkflowProcessInfo.ProcessParameters"/> содержится значение переопределяющее параметр "Не возвращать на доработку". Если значение - <see langword="true"/>, то проверяется значение параметра в этапе, иначе - <see langword="false"/> - нет. Тип значения: <see cref="Nullable{T}"/>, где T - <see cref="bool"/>.
            /// </summary>
            ...
            /// <summary>
            /// Ключ, по которому содержится идентификатор строки этапа. Тип значения: <see cref="Guid"/>.
            /// </summary>
            public const string StageRowID = StorageHelper.SystemKeyPrefix + "stageRowID";

            /// <summary>
            /// Ключ, по которому в <see cref="T:Tessa.Extensions.Default.Server.Workflow.KrObjectModel.Stage.InfoStorage"/> содержится значение, показывающее, что процесс был отозван. Тип значения: <see cref="bool"/>.
            /// </summary>
            public const string Revoked = nameof(Revoked);

            /// <summary>
            /// Ключ, по которому в <see cref="ApprovalProcessInstanceSettings.Info"/> содержится идентификатор вторичного процесса, выполняемого при отзыве процесса согласования. Тип значения: <see cref="Guid"/>.
            /// </summary>
            public const string ApprovalProcessSecondaryRevokeProcessID = nameof(ApprovalProcessSecondaryRevokeProcessID);

            /// <summary>
            /// Ключ, по которому содержится флаг отключения диалога выбора файла для подписания ЭП в задании подписания. Тип значения: <see cref="bool"/>.
            /// </summary>
            public const string NoSignFileDialog = StorageHelper.SystemKeyPrefix + nameof(NoSignFileDialog);

            /// <summary>
            /// Ключ, по которому содержится флаг отключения подписи и отображения копий файлов для подписания ЭП в задании подписания. Тип значения: <see cref="bool"/>.
            /// </summary>
            public const string DoNotSignFileCopies = StorageHelper.SystemKeyPrefix + nameof(DoNotSignFileCopies);

            /// <summary>
            /// Ключ, по которому содержится флаг отключения диалога добавления комментария ЭП. Тип значения: <see cref="bool"/>.
            /// </summary>
            public const string NoCommentDialog = StorageHelper.SystemKeyPrefix + nameof(NoCommentDialog);

            /// <summary>
            /// Ключ, по которому содержится список идентификаторов категорий, файлы которых будут подписаны или автовыбраны в диалоге для подписания ЭП в задании подписания.
            /// Тип значения: <see cref="System.Collections.IList"/>, где тип элемента - <see cref="Guid"/>.
            /// </summary>
            public const string FileCategories = StorageHelper.SystemKeyPrefix + nameof(FileCategories);

            /// <summary>
            /// Ключ, по которому содержится список идентификаторов категорий, файлы которых не подлежат подписи и скрыты в диалоге выбора файлов для подписания ЭП в задании подписания.
            /// Тип значения: <see cref="System.Collections.IList"/>, где тип элемента - <see cref="Guid"/>.
            /// </summary>
            public const string HiddenFileCategories = StorageHelper.SystemKeyPrefix + nameof(HiddenFileCategories);

            /// <summary>
            /// Ключ, по которому содержится список идентификаторов файлов для подписи или автовыбора в диалоге для подписания ЭП в задании подписания.
            /// Тип значения: <see cref="System.Collections.IList"/>, где тип элемента - <see cref="Guid"/>.
            /// </summary>
            public const string SelectedFiles = nameof(SelectedFiles);

            /// <summary>
            /// Ключ, по которому содержится список идентификаторов файлов, неподлежащих подписи и скрытых в диалоге выбора файлов для подписания ЭП в задании подписания.
            /// Тип значения: <see cref="System.Collections.IList"/>, где тип элемента - <see cref="Guid"/>.
            /// </summary>
            public const string HiddenFiles = nameof(HiddenFiles);
        }
        ...
        public static class Ui
        {
            ...
            /// <summary>
            /// Ключ, по которому в <see cref="CardTypeControl.ControlSettings"/> содержится список идентификаторов типов этапов. Тип значения: <see cref="IList{T}"/>, где T - <see cref="Guid"/>.
            /// </summary>
            ...
            public const string ReturnAfterDisapproval = nameof(ReturnAfterDisapproval);
        }
        ...
        public static class TaskHistorySettingsKeys
        {
            ...
            /// </summary>
            public const string ProcessID = StorageHelper.SystemKeyPrefix + nameof(ProcessID);

            /// <summary>
            /// Имя ключа, по которому в <see cref="CardTaskHistoryItem.Settings"/> хранится информация об отображаемом имя бизнес-процесса.
            /// </summary>
            public const string ProcessName = StorageHelper.SystemKeyPrefix + nameof(ProcessName);

            /// <summary>
            /// Имя ключа, по которому в <see cref="CardTaskHistoryItem.Settings"/> хранится информация о типе бизнес-процесса.
            /// </summary>
            public const string ProcessKind = StorageHelper.SystemKeyPrefix + nameof(ProcessKind);
            ...
            /// </summary>
            public const string PerformerRoleTypeID = StorageHelper.SystemKeyPrefix + nameof(PerformerRoleTypeID);
            ...
            /// Для записи из <see cref="CardTask.TaskAssignedRoles"/> выбирается первая запись по RowID с <see cref="CardFunctionRoles.PerformerID"/>
            /// </summary>
            public const string PerformerID = StorageHelper.SystemKeyPrefix + nameof(PerformerID);

            /// <summary>
            /// Имя ключа, по которому в <see cref="CardTaskHistoryItem.Settings"/> хранится информация о имени роли исполнителя задания.
            /// Для записи из <see cref="CardTask.TaskAssignedRoles"/> выбирается первая запись по RowID с <see cref="CardFunctionRoles.PerformerID"/>
            /// </summary>
            public const string PerformerName = StorageHelper.SystemKeyPrefix + nameof(PerformerName);
        }
        ...
        public static class KrAddFromTemplateSettingsVirtual
        {
            ...
            public static readonly string FileCategoryID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(FileCategoryID));
            public static readonly string FileCategoryName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(FileCategoryName));
        }
        ...
        public static class KrApprovalSettingsVirtual
        {
            ...
            public static readonly string NotCreateReturnEditTaskHistoryRecord = StageTypeSettingsNaming.PlainColumnName(Name, nameof(NotCreateReturnEditTaskHistoryRecord));
        }
        ...
        public static class KrDialogStageTypeSettingsVirtual
        {
            ...
            public static readonly string NotDisplayTabs =
                StageTypeSettingsNaming.PlainColumnName(Name, nameof(NotDisplayTabs));
        }
        ...
        public static class KrEditSettingsVirtual
        {
            ...
            public static readonly string HasEditApprovalSchemeAccess = StageTypeSettingsNaming.PlainColumnName(Name, nameof(HasEditApprovalSchemeAccess));
        }
        ...
        public static class KrSigningStageSettingsVirtual
        {
            ...
            public static readonly string NotCreateReturnEditTaskHistoryRecord = StageTypeSettingsNaming.PlainColumnName(Name, nameof(NotCreateReturnEditTaskHistoryRecord));
            public static readonly string SignCardFiles = StageTypeSettingsNaming.PlainColumnName(Name, nameof(SignCardFiles));
            public static readonly string NoSignFilesDialog = StageTypeSettingsNaming.PlainColumnName(Name, nameof(NoSignFilesDialog));
            public static readonly string DoNotSignFileCopies = StageTypeSettingsNaming.PlainColumnName(Name, nameof(DoNotSignFileCopies));
            public static readonly string NoCommentDialog = StageTypeSettingsNaming.PlainColumnName(Name, nameof(NoCommentDialog));
        }

        public static class KrSigningStageSettingsFileCategoriesVirtual
        {
            public const string Name = nameof(KrSigningStageSettingsFileCategoriesVirtual);
            public static readonly string Synthetic = StageTypeSettingsNaming.SectionName(Name);

            public static readonly string FileCategoryID = nameof(FileCategoryID);
            public static readonly string FileCategoryName = nameof(FileCategoryName);
        }

        public static class KrSigningStageSettingsHiddenFileCategoriesVirtual
        {
            public const string Name = nameof(KrSigningStageSettingsHiddenFileCategoriesVirtual);
            public static readonly string Synthetic = StageTypeSettingsNaming.SectionName(Name);
        ...
        public static class KrSigningTaskOptions
        {
            ...
            public static readonly string SignCardFiles = nameof(SignCardFiles);
        }
        ...
        public static class KrApprovalProcessSettingsVirtual
        {
            public const string SectionName = nameof(KrApprovalProcessSettingsVirtual);
            public static readonly string TemplateID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(TemplateID));
            public static readonly string TemplateName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(TemplateName));
            public static readonly string ReturnAfterDisapproval = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ReturnAfterDisapproval));
            public static readonly string ChangeState = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ChangeState));
            public static readonly string UseProcessFromCard = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(UseProcessFromCard));
            public static readonly string ShowRevokeButton = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ShowRevokeButton));
            public static readonly string NotReturnEdit = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(NotReturnEdit));
            public static readonly string SecondaryRevokeProcessID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(SecondaryRevokeProcessID));
            public static readonly string SecondaryRevokeProcessName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(SecondaryRevokeProcessName));
            public static readonly string InfoModeID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(InfoModeID));
            public static readonly string InfoModeName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(InfoModeName));
        }

        public static class KrApprovalProcessManagementSettingsVirtual
        {
            public const string SectionName = nameof(KrApprovalProcessManagementSettingsVirtual);
            public static readonly string ControlTypeID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ControlTypeID));
            public static readonly string ControlTypeName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ControlTypeName));
            public static readonly string StateID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(StateID));
            public static readonly string StateName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(StateName));
            public static readonly string UpdateHistoryGroup = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(UpdateHistoryGroup));
            public static readonly string ShowRevokeButton = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(ShowRevokeButton));
            public static readonly string SecondaryRevokeProcessID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(SecondaryRevokeProcessID));
            public static readonly string SecondaryRevokeProcessName = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(SecondaryRevokeProcessName));
            public static readonly string InfoModeID = StageTypeSettingsNaming.PlainColumnName(SectionName, nameof(InfoModeID));
        ...
        {
            public const string SectionName = nameof(Partners);

            public const string ID = nameof(ID);
            public const string Name = nameof(Name);
            public const string FullName = nameof(FullName);
            public const string LegalAddress = nameof(LegalAddress);
            public const string Phone = nameof(Phone);
            public const string Head = nameof(Head);
            public const string ChiefAccountant = nameof(ChiefAccountant);
            public const string ContactPerson = nameof(ContactPerson);
            public const string Email = nameof(Email);
            public const string ContactAddress = nameof(ContactAddress);
            public const string INN = nameof(INN);
            public const string KPP = nameof(KPP);
            public const string OGRN = nameof(OGRN);
            public const string OKPO = nameof(OKPO);
            public const string OKVED = nameof(OKVED);
            public const string Comment = nameof(Comment);
            public const string Bank = nameof(Bank);
            public const string SettlementAccount = nameof(SettlementAccount);
            public const string BIK = nameof(BIK);
            public const string CorrAccount = nameof(CorrAccount);
            public const string TypeID = nameof(TypeID);
            public const string TypeName = nameof(TypeName);
            public const string VatTypeID = nameof(VatTypeID);
            public const string VatTypeName = nameof(VatTypeName);
        }

        public static class PartnersTypes
        {
            public const string SectionName = nameof(PartnersTypes);

            public const string ID = nameof(ID);
            public const string Name = nameof(Name);
        }

        public static class PartnersContacts
        {
            public const string SectionName = nameof(PartnersContacts);
        ...
        {
            public const string SectionName = nameof(Recipients);
        ...
        public static class KrSecondaryProcesses
        {
            ...
            public const string ID = nameof(ID);
            public const string NotMessageHasNoActiveStages = nameof(NotMessageHasNoActiveStages);
            public const string TileHandlerID = nameof(TileHandlerID);
            public const string TileHandlerName = nameof(TileHandlerName);
            public const string NameField = "Name";
            public const string ToolbarVisibilityModeID = nameof(ToolbarVisibilityModeID);
            public const string ToolbarVisibilityModeName = nameof(ToolbarVisibilityModeName);
