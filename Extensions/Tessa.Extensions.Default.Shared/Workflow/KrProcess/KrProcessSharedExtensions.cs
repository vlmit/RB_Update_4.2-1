...
namespace Tessa.Extensions.Default.Shared.Workflow.KrProcess
{
    ...
    public static class KrProcessSharedExtensions
    {
        ...
        {
            ThrowIfNull(cardInfoStorageObject);
            ...
            bool value = true)
        {
            ...
            IgnoreButtons(request.Info, value);
        }

        /// <summary>
        /// Устанавливает значение, показывающее, что при загрузке карточки не надо добавлять в ответ информацию по тайлам вторичных процессов.
        /// </summary>
        /// <param name="info">Хранилище в котором устанавливается флаг.</param>
        /// <param name="value">Значение <see langword="true"/>, если при загрузке карточки не надо добавлять в ответ информацию по тайлам вторичных процессов, иначе - <see langword="false"/>.</param>
        public static void IgnoreButtons(
            Dictionary<string, object> info,
            bool value = true)
        {
            ...
            info[IgnoreButtonsInfoMark] = BooleanBoxes.Box(value);
        }
        ...
        public static void RemoveLocalTiles(
            this CardInfoStorageObject storage) => storage?.TryGetInfo()?.Remove(LocalTilesInfoMark);
        ...
        {
            ThrowIfNull(storage);
            ThrowIfNull(launchResult);

            storage.Info[KrProcessLaunchResult] = launchResult.GetStorage();
        }

        /// <summary>
        /// Возвращает объект, содержащий результат запуска процесса или значение <see langword="null"/>, если указанный объект его не содержит.
        /// </summary>
        /// <param name="storage">Хранилище содержащее результаты запуска процесса.</param>
        /// <returns>Объект, содержащий результат запуска процесса или значение <see langword="null"/>, если указанный объект его не содержит.</returns>
        ...
        private const string StartingKrProcessParametersKey = StorageHelper.SystemKeyPrefix + "startKrProcessParameters";
        ...
        private const string RecalcInfoMark = StorageHelper.SystemKeyPrefix + "Recalc";

        private const string InfoAboutChangesInfoMark = StorageHelper.SystemKeyPrefix + "InfoAboutChanges";

        private const string HasChangesInRoute = StorageHelper.SystemKeyPrefix + "HasChangesInRoute";

        private const string RouteChanges = StorageHelper.SystemKeyPrefix + "RouteChanges";
        ...
        /// <returns>Название состояния согласования документа или значение <see langword="null"/>, если заданное состояние не найдено.</returns>
        public static async ValueTask<string> TryGetDocumentStateNameAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return (string) record?[KrConstants.KrDocState.Name];
        }
        ...
        public static async ValueTask<string> GetDocumentStateNameAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return await metadata.TryGetDocumentStateNameAsync(state, cancellationToken)
                ?? throw new InvalidOperationException($"State with ID = {state.ID} doesn't exist in {KrConstants.KrDocState.SectionName} enumeration table.");
        }
        ...
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Название состояния этапа или значение <see langword="null"/>, если заданное состояние не найдено.</returns>
        public static async ValueTask<string> TryGetStageStateNameAsync(
            this ICardMetadata metadata,
            KrStageState state,
            CancellationToken cancellationToken = default)
        {
            ...
            return (string) record?[KrConstants.KrStageState.Name];
        }
        ...
        public static async ValueTask<string> GetStageStateNameAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return await metadata.TryGetStageStateNameAsync(state, cancellationToken)
                ?? throw new InvalidOperationException($"State with ID = {state.ID} doesn't exist in {KrConstants.KrStageState.Name} enumeration table.");
        }
