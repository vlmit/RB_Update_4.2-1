    ...
    public sealed class KrVirtualFileConvertGetContentExtension : CardGetFileContentExtension
    {
        ...
        public override async Task BeforeRequest(ICardGetFileContentExtensionContext context)
        {
            ...
            if (context.Response != null
                ...
                || context.Session.Token is not { } sessionToken)
            {
                return;
            }
            ...
            var converterRequest = new FileConverterRequest
            {
                ...
                Flags = FileConverterRequestFlags.IgnoreCacheBeforeConversion
                    | FileConverterRequestFlags.ExecuteSynchronously,
                Parameters = { ["KrVirtualFileSpecialID"] = Guid.NewGuid() }
            };

            SignedSessionToken signedToken = await this.sessionTokenProvider.CreateTokenAsync(sessionToken,
                modifyTokenAction: token => (token.Info ??= new())[FileTemplateHelper.SkipFileTemplatePermissionsCheck] = BooleanBoxes.True,
