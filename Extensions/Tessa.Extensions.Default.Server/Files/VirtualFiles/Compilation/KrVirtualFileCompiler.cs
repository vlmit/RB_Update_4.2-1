...
namespace Tessa.Extensions.Default.Server.Files.VirtualFiles.Compilation
{
        ...
        IKrVirtualFileCompiler
    {
        ...
        /// Параметры скрипта.
        /// </summary>
        private static readonly IReadOnlyList<(string Type, string Name)> parameters =
        [
            (nameof(IKrVirtualFileScriptContext), "context")
        ];
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var compilerOutputItem in compilationResult.CompilerOutput)
            {
                ...
                var errorText = await LocalizeFormatAsync(
                    "$KrVirtualFiles_CompilationErrorTemplate",
                    await LocalizeAsync(source.Name),
