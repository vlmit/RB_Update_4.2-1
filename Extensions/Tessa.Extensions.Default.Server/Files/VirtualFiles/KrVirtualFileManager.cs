...
namespace Tessa.Extensions.Default.Server.Files.VirtualFiles
{
    ...
    public sealed class KrVirtualFileManager : IKrVirtualFileManager
    {
        ...
        public async Task<ValidationResult> FillCardWithFilesAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            // Если для данного типа нет виртуальных файлов, прерываем выполнение.
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (virtualFile is null)
            {
                ...
                return ValidationResult.FromText(
                    await LocalizeFormatAsync("$KrVirtualFiles_UnknownVirtualFile", fileID),
            ...
            await using (this.dbScope.Create())
            {
                ...
                var userRoles = await this.GetUserRolesAsync(cancellationToken);

                var hasAccess = virtualFile.Types.Contains(typeID) &&
                                virtualFile.DocumentStates.Contains((KrState) stateID) &&
                                virtualFile.Roles.Intersect(userRoles).Any();
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // Устанавливаем все права на карточку.
            ...
            CancellationToken cancellationToken = default)
        {
            await using (this.dbScope.Create())
            {
                var allFiles = await this.virtualFileCache.GetAllAsync(cancellationToken);
                var documentState = (KrState) (card.Sections.TryGet(KrConstants.KrApprovalCommonInfo.Virtual)?
                                        .RawFields.TryGet<int?>(KrConstants.KrApprovalCommonInfo.StateID) ??
                                    KrState.Draft.ID);
                var userRoles = await this.GetUserRolesAsync(cancellationToken);
                var virtualFiles = allFiles.Where(
                    x => x.Types.Contains(card.TypeID) &&
                    x.DocumentStates.Contains(documentState) &&
                    x.Roles.Intersect(userRoles).Any())
                    .ToList();
                ...
                var result = new List<IKrVirtualFile>(virtualFiles.Count);
                ...
                foreach (var virtualFile in virtualFiles)
                {
                    if (await this.CheckConditionsAsync(conditionContext, virtualFile))
                    {
        ...
        private async ValueTask<IList<Guid>> GetUserRolesAsync(CancellationToken cancellationToken)
        {
            await using (this.dbScope.Create())
            {
                var db = this.dbScope.Db;
                ...
                return await db.SetCommand(
                        builder
                            ...
                            .And().C("ru", "TypeID").NotEquals().P("RoleID")
                            .Build(),
                        db.Parameter("UserID", this.session.User.ID),
                        db.Parameter("RoleID", (int) RoleType.Task))
                    .LogCommand()
