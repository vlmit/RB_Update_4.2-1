...
namespace Tessa.Extensions.Default.Server.Acl
{
    ...
    public class KrAclGenerationRuleExecutor : AclGenerationRuleExecutor
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                if (krRule.CardTypes.Contains(typeID.Value))
                {
                    result = true;
                }
                else if (docTypes.TryFirst(x => x.ID == typeID.Value, out var docType))
                {
                    result = krRule.DocTypes.Contains(docType.ID) || krRule.CardTypes.Contains(docType.CardTypeID);
                }
                else
                {
                    var docTypeID = await KrProcessSharedHelper.GetDocTypeIDAsync(cardID, this.DbScope, cancellationToken);
                    if (docTypeID.HasValue
                        && docTypes.TryFirst(x => x.ID == docTypeID.Value, out var docType2))
                    {
                        result = krRule.DocTypes.Contains(docType2.ID) || krRule.CardTypes.Contains(docType2.CardTypeID);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    result = cardTypeID is not null && krRule.CardTypes.Contains(cardTypeID.Value);
