...
namespace Tessa.Extensions.Default.Server.Acl
{
    ...
    /// </summary>
    public class KrAclGenerationRuleCacheObject(IDbScope dbScope, IKrTypesCache typesCache) : AclGenerationRuleCacheObject
    {
        #region Fields
        ...
        /// <inheritdoc/>
        public override async ValueTask<IReadOnlyList<IAclGenerationRule>> GetRulesByTriggerCardAsync(
            Card triggerCard,
            CancellationToken cancellationToken = default)
        {
            if (this.RulesByTriggerDocType.Count > 0
        ...
        public override async ValueTask InitializeAsync(IList<IAclGenerationRule> allRules, CancellationToken cancellationToken = default)
        {
            ...
            if (this.RulesByTriggerCardType is not null)
            {
            ...
            if (docTypesToMove is not null)
            {
                ...
                {
                    if (this.RulesByTriggerCardType?.Remove(docType.ID, out var aclRules) == true)
                    {
