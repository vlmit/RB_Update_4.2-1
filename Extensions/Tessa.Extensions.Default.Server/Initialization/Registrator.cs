...
namespace Tessa.Extensions.Default.Server.Initialization
{
    ...
    public sealed class Registrator : RegistratorBase
    {
        ...
        {
            if (this.UnityContainer.TryResolve<ICardMetadata>() is not { } cardMetadata
                || this.UnityContainer.TryResolve<IClientEnumerationRegistry>() is not { } enumerationRegistry)
            {
                return;
            }

            enumerationRegistry
                .RegisterFromMetadata(cardMetadata, "KrPermissionsFileCheckRules")
                .RegisterFromMetadata(cardMetadata, "KrWeActionCompletionOptions");
        }
    }
}
