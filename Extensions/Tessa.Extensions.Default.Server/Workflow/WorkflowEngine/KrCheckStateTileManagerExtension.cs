...
namespace Tessa.Extensions.Default.Server.Workflow.WorkflowEngine
{
    ...
    public sealed class KrCheckStateTileManagerExtension : IWorkflowEngineTileManagerExtension
    {
        ...
        /// </summary>
        public static readonly Guid KrCheckStateWorkflowTileExtensionTypeID = new(0x9fce6311, 0x1746, 0x412a, 0x93, 0x46, 0x77, 0x39, 0x4c, 0xae, 0xbe, 0x90);
        ...
        /// <inheritdoc />
        public async ValueTask<IList<Guid>> CheckTileAccessForVisibilityAsync(
            IList<Guid> tileIDs,
            Card? card,
            CancellationToken cancellationToken = default)
        {
            ...
            return await this.GetAccessedTilesAsync(card, card.ID, null, tileIDs, cancellationToken);
        }
        ...
        public async ValueTask<ValidationResult> CheckTileAccessForExecuteAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ThrowIfNull(tileInfo);

            var result = await this.GetAccessedTilesAsync(null, cardID, tileInfo.TileID, null, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            await using (this.dbScope.Create())
            {
                ...
                DataParameter? dpTileIDs = singleTileID is null
                    ? null
                    : db.Parameter("TileID", singleTileID, LinqToDB.DataType.Guid);
                if (card is not null
                ...
                {
                    var stateID = stateIDObj is null ? 0 : (int) stateIDObj;
                    ...
                    return await db.SetCommand(
                        this.dbScope.BuilderFactory
                            ...
                            .Where().C("s", "StateID").Equals().P("StateID")
                                .And().C("bpbe", "ButtonRowID")
                                    .If(singleTileID.HasValue, b => b.Equals().P("TileID"))
                                    .If(tileIDs is not null, b => b.InArray(tileIDs!, "TileIDs", out dpTileIDs))
                            .Build(),
                        DataParameters.Get(
