...
namespace Tessa.Extensions.Default.Server.Workflow.Wf
{
        ...
        WorkflowTaskWorker<WfWorkflowManager>
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (roleID.HasValue)
            {
                taskRoles = [(roleID.Value, roleName)];
            }
            else
            {
            ...
            if (taskInfo is not null)
            {
                ...
                if (flags.Has(WfResolutionSendingFlags.UseParentPlanned)
                ...
                task.Info[CardHelper.TaskProcessIDKey] = processInfo.ProcessID;
                task.Info[CardHelper.TaskProcessNameKey] = processInfo.ProcessTypeName;
                task.Info[CardHelper.TaskProcessKindKey] = WfHelper.ResolutionProcessName;
                                ...
                                | WfResolutionSendingFlags.CreatingChildResolution))
                        {
                                    ...
                                    | WfResolutionSendingFlags.CreatingChildResolution))
                            {
                                ...
                                if (flags.Has(WfResolutionSendingFlags.UseMajorPerformer))
                                {
                                    task.Digest = string.IsNullOrEmpty(parentComment)
                                        ? string.Format(
                                            await LocalizeAndEscapeFormatAsync("$WfResolution_Digest_MajorPerformerWithoutComment") ?? string.Empty,
                                            parentUserName)
                                        : string.Format(
                                            await LocalizeAndEscapeFormatAsync("$WfResolution_Digest_MajorPerformer") ?? string.Empty,
                                            parentUserName,
                                            LocalizationManager.EscapeIfLocalizationString(parentComment));
                                }

                                // Кейсы, когда несколько исполнителей.
                                if (flags.HasAny(WfResolutionSendingFlags.CreatingChildResolution | WfResolutionSendingFlags.UseMajorPerformer))
                                {
                                    ...
                                    task.Result = string.IsNullOrEmpty(parentComment)
                                        ? null
                                        : string.Format(
                                            await LocalizeAndEscapeFormatAsync("$WfResolution_Result_ParentComment") ?? string.Empty,
                                            LocalizationManager.EscapeIfLocalizationString(parentComment.ReplaceLineEndings(" ")));
                                }
                            }
                        ...
                        if (massCreationPerformers is not null && massCreationPerformers.Length > 1)
                        {
                            ...
                            if (flags.Has(WfResolutionSendingFlags.UseMajorPerformer))
                            {
                                ...
                                coperformerDigest = string.IsNullOrEmpty(parentComment)
                                    ? string.Format(
                                        await LocalizeAndEscapeFormatAsync("$WfResolution_Digest_CoperformerWithoutComment") ?? string.Empty,
                                        parentUserName,
                                        taskRoles[0].performerRoleName)
                                    : string.Format(
                                        await LocalizeAndEscapeFormatAsync("$WfResolution_Digest_Coperformer") ?? string.Empty,
                                        parentUserName,
                                        taskRoles[0].performerRoleName,
                                        LocalizationManager.EscapeIfLocalizationString(parentComment));
            ...
            await using (dbScope.Create())
            {
                    ...
                    && completedTaskSections.TryGetValue(WfHelper.ResolutionSection, out CardSection? completedTaskSection))
                {
                    ...
                    controlTask.Digest = string.IsNullOrEmpty(parentComment)
                        ? string.Format(
                            await LocalizeAndEscapeFormatAsync("$WfResolution_Digest_ControlWithoutComment") ?? string.Empty,
                            userName)
                        : string.Format(
                            await LocalizeAndEscapeFormatAsync(
                                "$WfResolution_Digest_ControlWithComment") ?? string.Empty,
                            userName,
                            LocalizationManager.EscapeIfLocalizationString(parentComment));
                }
                        ...
                        db1
                            .SetCommand(
                                builderFactory1
                                    ...
                                    .Limit(1)
                                    .OptionMaxRecursion(MaxRecursion)
                                    .Build(),
                ...
                this.Manager.ValidationResult.Add(
                    await this.Manager.NotificationManager.SendAsync(
                        ...
                        new NotificationSendContext
                        {
                            ...
                            ModifyEmailActionAsync = async (email, ct) => { DefaultNotificationHelper.ModifyTaskCaption(email, task); },
        ...
        protected string UpdateModifiedByAuthorDate(string result)
        {
            ...
            string modified = FormatDateTimeWithoutSeconds(
        ...
        /// </summary>
        private const string RevokedByParentKey = StorageHelper.SystemKeyPrefix + "revokedByParent";
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using (DbDataReader reader = await db.ExecuteReaderAsync(CommandBehavior.SequentialAccess, cancellationToken))
            {
                if (await reader.ReadAsync(cancellationToken))
                {
                    ...
                    if (((settings.TryConvertEnum<RoleType>(TaskHistorySettingsKeys.PerformerRoleTypeID) is not { } performerRoleType ||
                        CardComponentHelper.IsTemporaryTaskRole(performerRoleType)) &&
                        (settings.TryConvertGuid(TaskHistorySettingsKeys.PerformerRoleTypeID) is not { } performerRoleTypeID || // Для поддержки Legacy PerformerRoleTypeID (Guid)
                        CardComponentHelper.IsTemporaryTaskRole(performerRoleTypeID))) ||
                        settings.TryConvertGuid(TaskHistorySettingsKeys.PerformerID) is not { } performerID ||
            ...
            CancellationToken cancellationToken = default)
        {
            switch (processInfo.ProcessTypeName)
            {
                case WfHelper.ResolutionSubProcessName:
                    ...
                    return this.NotificationParentProcessAsync(
                        processInfo,
                        cancellationToken);

                default:
                    throw ArgumentOutOfRange(processInfo.ProcessTypeName);
            }
        }
                ...
                await this.Manager.NotificationManager
                    .SendAsync(
                        ...
                        new NotificationSendContext
                        {
                            ...
                            {
                                DefaultNotificationHelper.ModifyTaskCaption(email, task);
                                await using (dbScope.Create())
                                {
                                    var db = dbScope.Db;
                                    var builder = dbScope.BuilderFactory;
        ...
        /// <summary>
        /// Уведомляет родительский процесс о завершении процесса WfResolution.
        /// </summary>
        /// <param name="processInfo"><inheritdoc cref="IWorkflowProcessInfo" path="/summary"/></param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Асинхронная задача.</returns>
        private async Task NotificationParentProcessAsync(
            IWorkflowProcessInfo processInfo,
            CancellationToken cancellationToken = default)
        {
            var taskInfo = processInfo.ToTaskInfo();
            var optionID = taskInfo.Task.OptionID;

            if ((optionID != DefaultCompletionOptions.Complete && optionID != DefaultCompletionOptions.Revoke)
                || !this.Manager.ValidationResult.IsSuccessful()
                || taskInfo.Task.Info.TryGet<bool>(WorkflowEngineHelper.TaskCompletedByWorkflowEngineKey))
            {
                return;
            }

            var (parentProcessKind, parentProcessID, parentTaskID) = await this.TryGetParentProcessInfoAsync(
                taskInfo.Task.RowID,
                cancellationToken);

            await this.SendSignalToParentProcessAsync(
                parentProcessKind,
                parentProcessID,
                parentTaskID,
                cancellationToken);
        ...
        /// <summary>
        /// Возвращает информацию о родительском процессе.
        /// </summary>
        /// <param name="currentTaskRowID">Идентификатор текущего задания.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Кортеж: &lt;Тип процесса; Идентификатор процесса; Идентификатор родительского задания процесса WfResolution&gt;.</returns>
        private async Task<(string? ParentProcessKind, Guid ParentProcessID, Guid ParentTaskID)> TryGetParentProcessInfoAsync(
            Guid currentTaskRowID,
            CancellationToken cancellationToken)
        {
            await using var _ = this.Manager.DbScope.Create();

            var db = this.Manager.DbScope.Db;

            db
                .SetCommand(this.Manager.DbScope.BuilderFactory
                        ...
                        .OptionMaxRecursion(MaxRecursion)
                        .Build(),
                    db.Parameter("RowID", currentTaskRowID, DataType.Guid),
                    db.Parameter("ProjectTypeID", DefaultTaskTypes.WfResolutionProjectTypeID, DataType.Guid))
                .LogCommand();

            await using var rootTaskReader = await db.ExecuteReaderAsync(
                CommandBehavior.SequentialAccess,
                cancellationToken);

            if (!await rootTaskReader.ReadAsync(cancellationToken))
            {
                return default;
            }

            var settingsJson = await rootTaskReader.GetSequentialNullableStringAsync(
                0,
                db.Dbms,
                cancellationToken);
            ...
            if (settings is null
                || settings.TryGet<Guid?>(TaskHistorySettingsKeys.ProcessID) is not { } processID
                || settings.TryGet<string>(TaskHistorySettingsKeys.ProcessKind) is not { Length: > 0 } processKind)
            {
                return default;
            }

            var parentTaskID = rootTaskReader.GetGuid(1);

            return (processKind, processID, parentTaskID);
        }

        /// <summary>
        /// Отправляет сигнал о завершении текущего процесса в родительский процесс.
        /// </summary>
        /// <param name="parentProcessKind">Тип родительского процесса.</param>
        /// <param name="parentProcessID">Идентификатор родительского процесса.</param>
        /// <param name="parentTaskID">Идентификатор родительского задания процесса WfResolution.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Асинхронная задача.</returns>
        private async Task SendSignalToParentProcessAsync(
            string? parentProcessKind,
            Guid parentProcessID,
            Guid parentTaskID,
            CancellationToken cancellationToken)
        {
            switch (parentProcessKind)
            {
                case KrProcessName:
                ...
                case KrNestedProcessName:
                    this.Manager
                        ...
                        .GetWorkflowQueue()
                        .AddSignal(
                            processTypeName: parentProcessKind,
                            name: KrPerformSignal,
                ...
                case WorkflowEngineHelper.WorkflowEngineProcessName:
                    var signal = new WorkflowEngineTaskSignal(
                        ...
                        [parentTaskID])
                    {
                        // Значение не используется при завершении процесса WfResolution.
                        OptionID = null,
                    };
                    signal.Hash[WorkflowConstants.NamesKeys.ResolutionFinished] = BooleanBoxes.True;
                    ...
                    var result =
                        await this.workflowProcessor.SendSignalToAllSubscribersAsync(
                            ...
                            storeCard: this.Manager.Request.Card,
                            cancellationToken: cancellationToken);

                    this.Manager.ValidationResult.Add(result.ValidationResult);
