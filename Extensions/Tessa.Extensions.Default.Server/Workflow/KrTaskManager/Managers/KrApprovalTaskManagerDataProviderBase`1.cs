...
{
    /// <summary>
    /// Реализация <see cref="IKrApprovalTaskManagerDataProvider"/>, предоставляющая связь с внешней подсистемой через <see cref="IExternalContextProvider{T}.ExternalContext"/>.
    /// </summary>
    /// <param name="dataProviderFactory"><inheritdoc cref="IKrTaskManagerDataProviderFactory" path="/summary"/></param>
    public abstract class KrApprovalTaskManagerDataProviderBase<TExternalContext>(
        IKrTaskManagerDataProviderFactory dataProviderFactory) :
        KrTaskWithActionsManagerDataProviderBase<TExternalContext, IRoleUser>,
        IKrApprovalTaskManagerDataProvider<TExternalContext>
    {
        #region Properties
            ...
            Action<TDataProvider>? configureAction = null) where TDataProvider : IKrTaskManagerDataProvider
        {
            ...
            if (providerType == typeof(IKrApprovalCoreTaskManagerDataProvider))
            {
                return (TDataProvider) dataProviderFactory.Create<IKrApprovalCoreTaskManagerDataProvider<TExternalContext>, TExternalContext>(
