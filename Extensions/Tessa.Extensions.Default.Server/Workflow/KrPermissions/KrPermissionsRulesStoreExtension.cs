...
namespace Tessa.Extensions.Default.Server.Workflow.KrPermissions
{
        ...
        CardStoreExtension
    {
            ...
            bool withAcl)
        {
            ...
            foreach (var row in permissionTypes.Rows)
            {
                if (row.State != CardRowState.Deleted)
                {
                    ...
                    if (typeCaption?.StartsWith('$') == true)
                    {
            ...
            var statesText = text.ToStringAndClear().TrimEnd();
            if (!hasCreating && statesText.Length == 0)
            {
                validationResult.AddWarning(this, "$KrMessages_StatesNotSpecified");
            }

            permissionsSection.Fields["States"] = statesText;
            ...
            permissionsSection.Fields["Roles"] = text.ToStringAndClear().TrimEnd();
            ...
            if (withAcl)
            {
                ...
                permissionsSection.Fields["AclGenerationRules"] = text.ToStringAndClear().TrimEnd();
            }
        ...
        private static async Task FillTypeIsDocTypeAsync(ICardStoreExtensionContext context)
        {
            var sections = context.Request.Card.TryGetSections();
            if (sections is not null &&
                sections.TryGetValue("KrPermissionTypes", out var krPermissionTypesSection))
            {
                foreach (var row in krPermissionTypesSection.Rows)
                {
                    if (row.TryGetValue("TypeIsDocType", out var typeIsDocType) && typeIsDocType is null)
                    {
                        row["TypeIsDocType"] = BooleanBoxes.False;
                    }
                }
            }
        }
        ...
        public override async Task BeforeRequestWhenTypeResolved(ICardStoreExtensionContext context)
        {
            ...
            // Заполняем поле "TypeIsDocType".
            await FillTypeIsDocTypeAsync(context);
        }
        ...
        public override async Task BeforeCommitTransaction(ICardStoreExtensionContext context)
        {
            ...
            if (context.Request.Info.TryGet<bool>(SkipPermissionsStoreKey)
                ...
                || card.TryGetSections() is not { Count: > 0 })
            {
                return;
            }
            ...
            if (updatedCard.HasChanges())
            {
                ...
                storeRequest.SetDigest(context.Request.TryGetDigest());
