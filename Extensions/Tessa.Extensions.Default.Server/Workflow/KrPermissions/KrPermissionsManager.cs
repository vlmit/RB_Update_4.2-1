...
namespace Tessa.Extensions.Default.Server.Workflow.KrPermissions
{
    ...
    public sealed class KrPermissionsManager(
        ...
        IKrFileOwnershipChecker krFileOwnershipChecker) : IKrPermissionsManager
    {
        #region Nested Types
        ...
        private class InnerContext(IKrPermissionsManagerContext context, bool checkPermissions) : IAsyncDisposable
        {
            #region Fields
            ...
            public IKrPermissionsManagerContext Context { get; } = NotNullOrThrow(context);

            /// <summary>
            /// Флаг определяет, что происходит проверка прав.
            /// Если флег имеет значение false, значеит идет расчет прав.
            /// </summary>
            public bool CheckPermissions { get; } = checkPermissions;
            ...
            /// <summary>
            /// Флаг определяет, что расширенные настройки на обязательность уже были рассчитаны.
            /// </summary>
            public bool ExtendedMandatorySettingsCalculated { get; set; }

            /// <summary>
            /// Флаг определяет, что расширенные настройки на файлы рассчитаны.
            /// </summary>
            public bool ExtendedFileSettingsCalculated { get; set; }

            public List<Guid> StaticRolesCheckedRules { get; } = new();
            public List<Guid> AclGenerationRolesCheckedRules { get; } = new();
            public HashSet<Guid> SubmittedRules { get; } = new();
            public HashSet<Guid> RejectedRules { get; } = new();
            public HashSet<Guid> SubmittedRoles { get; } = new();
            public HashSet<Guid> RejectedRoles { get; } = new();
            ...
            public Dictionary<string, object> ResultInfo => this.resultInfo ??= new(StringComparer.Ordinal);
        ...
        private readonly ICardContextRoleCache roleCache = NotNullOrThrow(roleCache);
        ...
        private readonly IKrPermissionsFilesManager krPermissionsFilesManager = NotNullOrThrow(krPermissionsFilesManager);
        ...
        private readonly CalcAction[] mandatoryCalcAction =
        [
            CalcExtendedMandatoryFromRule
        ];

        private readonly FilterFunc mandatoryFilterFunc = x => x.MandatoryRules.Count > 0;

        private readonly CalcAction[] fileRulesCalcAction =
        [
            CalcExtendedFileRulesFromRule
        ];

        private readonly FilterFunc fileRulesFilterFunc = x => x.FileRules.Count > 0;

        private readonly CalcAction[] allCalcActionsWithEdit =
        [
            CalcExtendedPermissionsFromRuleWithEdit,
            CalcExtendedPermissionsFromRuleForTasks,
            CalcExtendedMandatoryFromRule,
            CalcVisibilityFromRule,
            CalcExtendedFileRulesFromRule
        ];

        private readonly CalcAction[] allCalcActionsWithoutEdit =
        [
        ...
        /// <inheritdoc />
        public async Task<KrPermissionsCreateContextResult> TryCreateContextAsync(
            KrPermissionsCreateContextParams param,
            CancellationToken cancellationToken = default)
        {
            // Последовательность генерации контекста:
            // 1) Определяем режим проверки прав по отношению к карточке и тип карточки.
            // 2) Проверяем принадлежность типа карточки к типовому решению
            // 3) При наличии предыдущего токена, проверяем его валидность. Если он не валиден, возвращаем ошибку. Если валиден - используем его данные.
            // 4) Определяем тип документа, если он требуется для данного типа карточки. Если типа документа нужен, но определить его не удалось, возвращаем ошибку.
            // 5) Загружаем актуальный слепок кэша правил доступа для проверки прав по нему.
            ...
            {
                // Тип карточки неизвестен или не задан, тогда проверку прав доступа не выполняем
                return KrPermissionsCreateContextResult.NotAllowed;
            }

            var components = await KrComponentsHelper.GetKrComponentsAsync(cardType.ID, this.krTypesCache, cancellationToken);
            ...
            var createContextValidationResult = new ValidationResultBuilder();
            KrToken? previousToken = null;
            KrTokenValidationResult? validateTokenResult = null;

            if (param.PrevToken is { } token)
            {
                if (!token.ServerOnly || param.ServiceType == CardServiceType.Default)
                {
                    previousToken = token;
                    ...
                    if (card is null)
                    {
                        if (param.CardID is null || previousToken.CardVersion == CardComponentHelper.DoNotCheckVersion)
                        {
                            card = new() { ID = param.CardID ?? cardType.ID, TypeID = cardType.ID };
                        }
                        else
                        {
                            card = await this.GetCardForTokenCheckAsync(param.CardID.Value, createContextValidationResult, cancellationToken);

                            if (!createContextValidationResult.IsSuccessful())
                            {
                                param.ValidationResult?.Add(createContextValidationResult);
                                return new()
                                {
                                    Status = KrPermissionsCreateContextStatus.Fail,
                                    ValidationResult = createContextValidationResult.Build()
                                };
                            }
                        }
                    }

                    if (previousToken.TryGetDocTypeID(out var docTypeID))
                    {
                        param.DocTypeID = docTypeID;
                        if (mode == KrPermissionsCheckMode.WithStoreCard)
                        {
                            param.Card.Info[Keys.DocTypeID] = docTypeID;
                        }
                    }

                    validateTokenResult = await this.krTokenProvider.ValidateTokenAsync(
                        card,
                        previousToken,
                        createContextValidationResult,
                        param.CustomValidationAction,
                        cancellationToken);

                    if (validateTokenResult == KrTokenValidationResult.Fail)
                    {
                        param.ValidationResult?.Add(createContextValidationResult);
                        return new()
                        {
                ...
                {
                    createContextValidationResult.AddError(this, "$KrMessages_ServerTokenIsUsedForClientRequest");
                    param.ValidationResult?.Add(createContextValidationResult);
                    return new()
                    {
                        Status = KrPermissionsCreateContextStatus.Fail,
            ...
            if (components.Has(KrComponents.DocTypes))
            {
                if (!param.DocTypeID.HasValue)
                {
                    ...
                    if (!param.DocTypeID.HasValue)
                    {
                        createContextValidationResult.AddError(
                            this,
                            "$KrMessages_DocTypeNotSpecified",
                            cardType.Name,
                            cardType.ID,
                            param.Card?.ID ?? param.CardID ?? Guid.Empty);
            ...
            {
                param.PermissionsCache = await this.permissionsCacheContainer.TryGetCacheAsync(createContextValidationResult, cancellationToken);
                if (!createContextValidationResult.IsSuccessful())
                {
                    param.ValidationResult?.Add(createContextValidationResult);
                    return new()
            ...
            return new()
            {
                Context = new KrPermissionsManagerContext(
                    this.dbScope,
                    this.session,
                    ...
                    param.ServiceType == CardServiceType.Default,
                    ...
                    cancellationToken),
                Status = KrPermissionsCreateContextStatus.Success,
            ...
            params KrPermissionFlagDescriptor[] requiredPermissions)
        {
            ...
            if (CheckFullTokenAccess(context))
            {
                return this.CreateFullResult(context, requiredPermissions);
            }

            context.Descriptor = new(requiredPermissions);
            context.Method = nameof(this.GetEffectivePermissionsAsync);
            ...
            params KrPermissionFlagDescriptor[] requiredPermissions)
        {
            ...
            context.Descriptor = new(requiredPermissions);
            context.Method = nameof(this.CheckRequiredPermissionsAsync);
            ...
            await using (var innerContext = new InnerContext(context, true))
            {
                ...
                if (context.WithExtendedPermissions)
                {
                    if (context.Mode == KrPermissionsCheckMode.WithStoreCard)
                    {
                        if (requiredPermissions.Contains(KrPermissionFlagDescriptors.EditCard)
                            ...
                            || requiredPermissions.Contains(KrPermissionFlagDescriptors.CreateFileLink)
                        ...
                        await this.CheckExtendedFilesConstraintsAsync(innerContext);
                        if (!context.ValidationResult.IsSuccessful())
                        {
                            if (innerContext.HasResultInfo)
                            {
                                return new(false, innerContext.ResultInfo);
                            }
                            else
                            {
                                return false;
                    ...
                    {
                        var checkReedCard = requiredPermissions.Contains(KrPermissionFlagDescriptors.ReadCard);
                        var checkCreateFileLink = requiredPermissions.Contains(KrPermissionFlagDescriptors.CreateFileLink);
                        if (checkReedCard || checkCreateFileLink)
                        {
                            await this.CheckFileReadAccessAsync(innerContext, context.FileID.Value, context.FileVersionID,
                                isCopy: false,
                                checkReadFile: checkReedCard,
                                checkCreateFileLink: checkCreateFileLink);
                            if (!context.ValidationResult.IsSuccessful())
                            {
                                return false;
                            }
                ...
                if (context.Descriptor.AllChecked())
                {
                    if (innerContext.HasResultInfo)
                    {
                        return new(true, innerContext.ResultInfo);
                    }
                    else
                    {
                        return true;
        ...
        private async Task CheckPermissionsWithCardAndTokenAsync(InnerContext innerContext)
        {
            ...
            try
            {
                await using var extensions =
                    await this.extensionContainer.ResolveExecutorAsync<ICardPermissionsExtension>();

                await this.ExtendPermissionsWithCardAsync(context, extensions, nameof(ICardPermissionsExtension.ExtendPermissionsBeforeTokenCheckAsync));
                ...
                await this.ExtendPermissionsWithCardAsync(context, extensions, nameof(ICardPermissionsExtension.ExtendPermissionsAfterTokenCheckAsync));
            }
            ...
            InnerContext innerContext)
        {
            ...
            if (innerContext.CheckPermissions
                ...
                && serverToken.TryGetExtendedCardSettings() is { } extendedCardSettings)
            {
                if (extendedCardSettings.TryGetSectionSettings() is { } cardSettings)
                {
                ...
                if (extendedCardSettings.TryGetTasksSettings() is { Count: > 0 } tasksSettings)
                {
                    foreach (var taskSettings in tasksSettings)
                    {
                            ...
                            || existedTaskSettings is null)
                        {
                            descriptor.ExtendedTasksSettings[taskType]
                                = existedTaskSettings
                                    = new HashSet<Guid, IKrPermissionSectionSettingsBuilder>(x => x.SectionID);
                        }
        ...
        private static bool CheckFullTokenAccess(
            IKrPermissionsManagerContext context)
        {
            return context.PreviousTokenValidationResult is KrTokenValidationResult.Success
                && context.PreviousToken is { ServerOnly: true, FullAccess: true };
        }
        ...
        {
            var context = innerContext.Context;
            if (context.PreviousTokenValidationResult != KrTokenValidationResult.Success)
            {
                return;
            }
            ...
            {
                var token = context.PreviousToken;
                ...
                if (token.TryGetSubmittedRules() is { } submittedRules)
                {
                    innerContext.SubmittedRules.AddRange(submittedRules);
                }

                if (token.TryGetRejectedRules() is { } rejectedRules)
                {
                    innerContext.RejectedRules.AddRange(rejectedRules);
                }
                    ...
                    && context.WithExtendedPermissions)
                {
                    ...
                    if (token.TryGetExtendedCardSettings() is { } extendedCardSettings)
                    {
                        innerContext.ExtendedSettingsCalculated = true;
                        innerContext.ExtendedFileSettingsCalculated = true;
                        // Переносим настройки из токена в дескриптор
                        if (extendedCardSettings.TryGetSectionSettings() is { } cardSettings)
                        {
                            context.Descriptor.ExtendedCardSettings.AddRange(
                                cardSettings.Select(KrPermissionSectionSettings.ConvertFrom));
                        }
                        ...
                        if (extendedCardSettings.TryGetTasksSettings() is { Count: > 0 } tasksSettings)
                        {
                            foreach (var taskSettings in tasksSettings)
                            {
                                ...
                                context.Descriptor.ExtendedTasksSettings[taskType]
                                    = new(
                                        x => x.SectionID,
                                        taskSettings.Value.Select(KrPermissionSectionSettings.ConvertFrom));
                            }
                        }

                        // Если идет проверка по файлу, то пытаемся сформировать правило доступа для этого файла
                        if (extendedCardSettings.TryGetFileSettings() is { Count: > 0 } fileSettings)
                        {
                            foreach (var fileSetting in fileSettings)
                            {
                                context.Descriptor.FileRules.Add(
                                    new KrPermissionsDirectFileRule
                                    {
                                        ...
                                        CreateLinkAccessSetting = fileSetting.CreateLinkAccessSetting,
                                        FileSizeLimit = fileSetting.FileSizeLimit
                                    });
                            }
                        }

                        if (extendedCardSettings.TryGetOwnFilesSettings() is { } ownFilesSettings)
                        {
                            if (ownFilesSettings.TryGetGlobalSettings() is { } globalSettings)
                            {
                            ...
                            {
                                foreach ((_, KrPermissionsFileExtensionSettings extensionSettings) in extensionsSettings)
                                {
                        ...
                        if (extendedCardSettings.TryGetOtherFilesSettings() is { } otherFilesSettings)
                        {
                            if (otherFilesSettings.TryGetGlobalSettings() is { } globalSettings)
                            {
                        ...
                        if (extendedCardSettings.TryGetFileConstraints() is { Count: > 0 } fileConstraints)
                        {
                            foreach (var fileConstraint in fileConstraints)
                            {
                                context.Descriptor.FileRules.Add(
                                    new KrPermissionsFileRule(
                                        this.dbScope,
                                        this.krFileOwnershipChecker,
                                        fileConstraint.Extensions,
                                        fileConstraint.Categories)
                                    {
                                        Priority = fileConstraint.Priority,
                                        FileSizeLimit = fileConstraint.Limit,
                                        FileCheckRule = fileConstraint.FileCheckRule,
                                        MaxCount = fileConstraint.MaxCount,
                                        Mandatory = fileConstraint.Mandatory,
                                    });
                            }
                        }
                    }
            ...
            int fileCheckRule)
        {
            ...
            string[] extensions = string.IsNullOrEmpty(extensionSettings.Extension)
                ? null
                : new[] { extensionSettings.Extension };
        ...
        /// </summary>
        /// <param name="innerContext">Контекст проверки доступа</param>
        /// <returns>Возвращает true, если проверка прав доступа полностью завершена, иначе false</returns>
        private async Task<bool> CheckRulesAsync(InnerContext innerContext)
        {
            ...
            bool force = false)
        {
            ...
            foreach (var ruleID in innerContext.StaticRolesCheckedRules)
            {
                if (!checkingRules.TryGetItem(ruleID, out var rule))
                {
                    continue;
                }

                checkingRules.RemoveByKey(ruleID);
                await this.CheckRuleWithConditionsAndExtensionsAsync(
                    innerContext,
                    rule,
                    force);
            ...
            innerContext.StaticRolesCheckedRules.AddRange(rulesForStaticRoles);

            foreach (var ruleID in rulesForStaticRoles)
            {
                await this.CheckRuleWithConditionsAndExtensionsAsync(
                    innerContext,
                    checkingRules[ruleID],
                    force);
            ...
            bool force = false)
        {
            ...
            foreach (var ruleID in innerContext.AclGenerationRolesCheckedRules)
            {
                if (!checkingRules.TryGetItem(ruleID, out var rule))
                {
                    continue;
            ...
            innerContext.AclGenerationRolesCheckedRules.AddRange(rulesForAclGenerationRules);
        ...
        /// <summary>
        /// Метод для проверки правила доступа по условиям и расширениям.
        /// </summary>
        /// <param name="innerContext">Контекст проверки прав доступа.</param>
        /// <param name="rule">Проверяемое правило доступа.</param>
        /// <param name="force">Определяет, должны ли настройки правила доступа установиться принудительно, даже если они не были запрошены.</param>
        /// <returns>Асинхронная задача.</returns>
        private async ValueTask CheckRuleWithConditionsAndExtensionsAsync(
            InnerContext innerContext,
        ...
        /// </summary>
        /// <param name="innerContext">Контекст проверки доступа.</param>
        /// <param name="checkingRules">Проверяемые правила.</param>
        /// <param name="force">Признак принудительного выполняния проверки.</param>
        /// <returns>Возвращает <c>true</c>, если проверка прав доступа полностью завершена, иначе <c>false</c>.</returns>
            ...
            IKrPermissionRuleSettings rule)
        {
            ...
            foreach (Guid roleID in rule.ContextRoles)
            {
                if (innerContext.SubmittedRoles.Contains(roleID))
                {
                    return true;
                }

                if (innerContext.RejectedRoles.Contains(roleID))
                {
                    continue;
                }

                var contextRole = await this.roleCache.GetContextRoleAsync(roleID, innerContext.Context.CancellationToken);
                ...
                if (userInRole)
                {
                    ...
                    innerContext.SubmittedRoles.Add(roleID);
                    return true;
                }

                //Запоминаем как не успешно проверенную роль
                innerContext.RejectedRoles.Add(roleID);
            }
        ...
        private async Task CheckFileReadAccessAsync(
            InnerContext innerContext,
            Guid fileID,
            Guid? fileVersionID,
            bool isCopy,
            bool checkReadFile,
            bool checkCreateFileLink)
        {
            var context = innerContext.Context;
            var descriptor = context.Descriptor;

            var flags = (checkReadFile, checkCreateFileLink) switch
            {
                (true, false) => KrPermissionsFileAccessSettingFlag.Read,
                (true, true) => KrPermissionsFileAccessSettingFlag.Read | KrPermissionsFileAccessSettingFlag.CreateLink,
                (false, false) => KrPermissionsFileAccessSettingFlag.None,
                (false, true) => KrPermissionsFileAccessSettingFlag.Read | KrPermissionsFileAccessSettingFlag.CreateLink,
            };

            if (flags == KrPermissionsFileAccessSettingFlag.None)
            {
                return;
            }
            ...
            if (descriptor.FileRules.Count < 1)
            {
                return;
            }
            ...
            var fileContext = new KrPermissionsFilesManagerContext(
                ...
                serverOperation: context.ServerOperation,
                info: context.Info);
            fileContext.SetFile(fileID, fileVersionID);

            var checkResult = await this.krPermissionsFilesManager.CheckPermissionsAsync(fileContext);

            if (checkReadFile && checkResult.AccessSettings.TryGetValue(KrPermissionsFileAccessSettingFlag.Read, out var currentAccessSetting))
            {
                innerContext.ResultInfo[KrPermissionsHelper.FileReadAccessSettings.InfoKey] = currentAccessSetting;
            }

            if (checkCreateFileLink && checkResult.AccessSettings.TryGetValue(KrPermissionsFileAccessSettingFlag.CreateLink, out var currentCreateLinkAccess))
            {
                // если флаги не установить здесь, то, несмотря на наличие расширенных прав доступа
                // на создание ссылки на файл, весь остальной код это игнорирует и устанавливает
                // только не расширенные флаги доступа.
                // этот кейс для не связанного с сохранением карточки запроса по правам файлов.
                switch (currentCreateLinkAccess)
                {
                    case KrPermissionsHelper.FileEditAccessSettings.Allowed:
                        descriptor.Set(KrPermissionFlagDescriptors.CreateFileLink, true);
                        break;

                    case KrPermissionsHelper.FileEditAccessSettings.Disallowed:
                        descriptor.Set(KrPermissionFlagDescriptors.CreateFileLink, false);
                        break;
                }
            }

            var validationResult = await this.krPermissionsFilesManager.GetValidationResultAsync(fileContext, checkResult, isCopy: isCopy);
            if (validationResult is { Items.Count: < 1 })
            {
                return;
            }
        ...
        private async Task CheckExtendedFilesConstraintsAsync(
            InnerContext innerContext)
        {
            var context = innerContext.Context;
            var descriptor = context.Descriptor;
            ...
            // Правила для проверки MaxCount (Допустимое количество файлов) и Mandatory (Обязательный файл).
            var rulesToCheck = descriptor.FileRules
                .OfType<KrPermissionsFileRule>()
                .Where(x => x.Mandatory || x.MaxCount.HasValue)
                .ToList();

            if (rulesToCheck.Count == 0
                || context.Card is not { } checkCard)
            {
                return;
            }
            ...
            var fileCheckContext = new KrPermissionsFilesManagerContext(
                ...
                cardId: context.CardID,
                docTypeId: context.DocTypeID,
                writeDisallowedErrors: true,
                serverOperation: context.ServerOperation,
                info: context.Info);

            IList<CardFile> filesToCheck;
            if (checkCard.StoreMode is CardStoreMode.Insert)
            {
                filesToCheck = checkCard.Files;
            }
            else
            {
                Card fullCard = await this.GetFullCardAsync(context);
                if (fullCard is null)
                {
                    return;
                }

                var contextFiles = checkCard.Files.Where(x => x.State != CardFileState.Deleted);
                filesToCheck = [.. fullCard.Files.ExceptBy(checkCard.Files.Select(x => x.RowID), x => x.RowID)
                    .Union(contextFiles)
            ...
            foreach (var rule in rulesToCheck)
            {
                ...
                {
                    fileCheckContext.SetFile(file, storeFile: file);
                    if (await rule.CheckFileAsync(fileCheckContext))
                    {
                        counter++;

                        // Если не нужно проверять на MaxCount, можно прервать цикл
                        if (!rule.MaxCount.HasValue)
                        {
                            break;
                        }
                    }
                ...
                if (rule.MaxCount.HasValue && counter > rule.MaxCount.Value)
                {
                    await KrPermissionsHelper.AddMaxFileCountExceededOrMandatoryErrorAsync(
                        context.ValidationResult,
                        this,
                        KrPermissionsHelper.KrPermissionsErrorType.MaxFilesCountExceeded,
                        rule.Extensions,
                        rule.FileCheckRule,
                        rule.Categories,
                        null,
                ...
                else if (rule.Mandatory && counter == 0)
                {
                    await KrPermissionsHelper.AddMaxFileCountExceededOrMandatoryErrorAsync(
                        context.ValidationResult,
                        this,
                        KrPermissionsHelper.KrPermissionsErrorType.MandatoryFileMissed,
                        rule.Extensions,
                        rule.FileCheckRule,
                        rule.Categories,
                        null,
            ...
            InnerContext innerContext)
        {
            ...
            Card? checkCard = null;
            var cardMeta = await this.cardMetadata.GetMetadataForTypeAsync(context.CardType.ID, context.CancellationToken);
            var cardMetaSections = await cardMeta.GetSectionsAsync(context.CancellationToken);
            var checkTasks = context.Mode is KrPermissionsCheckMode.WithCard or KrPermissionsCheckMode.WithStoreCard
                ? context.Card.TryGetTasks()?.Where(x => x is { Action: CardTaskAction.Complete, OptionID: not null })
                : null;
            ...
            foreach (var mandatoryRule in descriptor.ExtendedMandatorySettings)
            {
                ...
                switch (mandatoryRule.ValidationType)
                {
                    ...
                    case KrPermissionsHelper.MandatoryValidationType.WhenOneFieldFilled:
                        checkCard ??= await this.GetCardForMandatoryCheckAsync(context);
                    ...
                    case KrPermissionsHelper.MandatoryValidationType.OnTaskCompletion when checkTasks is not null:
                ...
                if (hasError)
                {
                    await this.AddErrorAsync(
                        context,
                        await LocalizeFormatAsync(mandatoryRule.Text));
                ...
                if (sectionMeta.Columns.TryGetValue(column, out var columnMeta))
                {
                    if (columnMeta.ColumnType == CardMetadataColumnType.Complex)
                    {
                        columnMeta = sectionMeta.Columns.FirstOrDefault(x =>
                            ...
                            && x is { ColumnType: CardMetadataColumnType.Physical, IsReference: true });
            ...
            InnerContext innerContext)
        {
            ...
            await this.TryCalcExtendedPermissionsAsync(
                ...
                // Если карточка только создаётся, то необходимо рассчитать расширенные настройки для файлов,
                // т.к. там могут быть требования к обязательности файлов.
                checkCard.StoreMode is CardStoreMode.Insert || checkCard.Files.Count > 0);
                ...
                && descriptor.ExtendedTasksSettings.Count > 0)
            {
                ...
                {
                    if (task.State is CardRowState.Inserted
                        or CardRowState.None)
                    {
                        continue;
                    }
            ...
            if (checkCard.Files.Count > 0)
            {
                foreach (var file in checkCard.Files)
                {
                    if (!file.HasContent()
                        || file.ExternalSource is not { } externalSource
                        || file.GetIgnoreExternalSourceContent())
                    {
                        continue;
                    }

                    // Если идёт проверка доступа к той же карточке, то можно проверить доступ на чтения файла
                    // иначе выполняем полноценную проверку доступа к файлу и карточке.
                    if (externalSource.CardID == context.CardID)
                    {
                        await this.CheckFileReadAccessAsync(innerContext, externalSource.FileID, externalSource.VersionRowID,
                            isCopy: true, checkReadFile: true, checkCreateFileLink: false);
                    }
                    else
                    {
                        var contextParam = new KrPermissionsCreateContextParams
                        {
                            CardID = externalSource.CardID,
                            CardTypeID = externalSource.CardTypeID,
                            FileID = externalSource.FileID,
                            FileVersionID = externalSource.VersionRowID,
                            WithExtendedPermissions = true,
                            PermissionsCache = context.PermissionsCache,
                            AdditionalInfo = context.Info,
                            ValidationResult = context.ValidationResult,
                            ExtensionContext = context.ExtensionContext,
                        };

                        var permissionContextResult = await this.TryCreateContextAsync(
                            contextParam,
                            innerContext.Context.CancellationToken);

                        switch (permissionContextResult.Status)
                        {
                            case KrPermissionsCreateContextStatus.Success:
                                await this.CheckRequiredPermissionsAsync(
                                    permissionContextResult.Context,
                                    KrPermissionFlagDescriptors.ReadCard);
                                break;

                            // Не относится к типовому решению или возникла ошибка, пропускаем
            ...
            if (descriptor.FileRules.Count > 0)
            {
                ...
                foreach (var file in checkCard.Files)
                {
                    ...
                    if (fileCheckResult.FileSizeLimit.HasValue
                        && file.State is CardFileState.Inserted or CardFileState.Replaced or CardFileState.ModifiedAndReplaced)
                    {
                        fileSizes ??= [];
                ...
                if (stillRequired.HasNot(KrPermissionsFileAccessSettingFlag.CreateLink))
                {
                    descriptor.Set(KrPermissionFlagDescriptors.CreateFileLink, true);
                }

                if (fileSizes is not null
                    && context.ExtensionContext is ICardStoreExtensionContext { ContentStorePending: true } storeContext)
                {
        ...
        /// <returns>Возвращает <c>true</c>, если проверка прошла успешно, или <c>false</c>, если при проверке произошли ошибки доступа.</returns>
        ...
        private async Task TryCalcExtendedMandatoryAsync(InnerContext innerContext)
        {
            ...
            if (!innerContext.ExtendedMandatorySettingsCalculated)
            {
                await this.CalcExtendedPermissionsAsync(
                    innerContext,
                    this.mandatoryCalcAction,
                    this.mandatoryFilterFunc);
                innerContext.ExtendedMandatorySettingsCalculated = true;
            }
        }
            ...
            bool withFiles)
        {
            if (!innerContext.ExtendedSettingsCalculated)
            {
                ...
                if (withCard)
                {
                    actions = new(4) { CalcExtendedPermissionsFromRuleWithEdit };
                    filterFunc = s => s.CardSettings.Count > 0;
                }

                if (withMandatory
                    && !innerContext.ExtendedMandatorySettingsCalculated)
                {
                    actions ??= new(3);
                    actions.Add(CalcExtendedMandatoryFromRule);
                    var filterFuncLocal = filterFunc;
                    filterFunc = filterFunc is null
                        ? new(s => s.MandatoryRules.Count > 0)
                ...
                if (withTasks)
                {
                    ...
                    var filterFuncLocal = filterFunc;
                    filterFunc = s => filterFuncLocal(s) || s.TaskSettingsByTypes.Count > 0;
                }

                if (withFiles
                    && !innerContext.ExtendedFileSettingsCalculated)
                {
                    actions ??= new(1);
                    actions.Add(CalcExtendedFileRulesFromRule);
                    var filterFuncLocal = filterFunc;
                    filterFunc = filterFunc is null
                        ? new(s => s.FileRules.Count > 0)
                        : new FilterFunc(s => filterFuncLocal(s) || s.FileRules.Count > 0);
                }

                innerContext.ExtendedSettingsCalculated = true;
                innerContext.ExtendedMandatorySettingsCalculated = withMandatory;
                innerContext.ExtendedFileSettingsCalculated = withFiles;
        ...
        {
            if (!innerContext.ExtendedFileSettingsCalculated)
            {
                innerContext.ExtendedFileSettingsCalculated = true;
            ...
            FilterFunc filterFunc = null)
        {
            ...
            {
                allExtendedRules = allExtendedRules.Where(filterFunc.Invoke);
            }
            ...
            foreach (var ruleID in innerContext.StaticRolesCheckedRules)
            {
                if (!extendedRules.TryGetItem(ruleID, out var rule))
                {
                    continue;
                }
            ...
            foreach (var ruleID in rulesWithStaticRoles)
            {
                ...
                // Правило либо будет успешно проверено, либо нет, поэтому сразу удаляем из списка правил, чтобы не проверять его при проверке по ACL и контекстных ролей
                extendedRules.RemoveByKey(ruleID);
                if (await this.CheckWithConditionsAsync(innerContext, rule)
                    && await this.CheckWithExtensionsAsync(innerContext, rule.ID))
                {
            ...
            if (context.Mode != KrPermissionsCheckMode.WithoutCard)
            {
                var rulesWithAcl = await this.GetRulesForAclGenerationRulesAsync(
                    extendedRules,
                    context.CardID!.Value,
                    context.CancellationToken);
                foreach (var ruleID in rulesWithAcl)
                {
                    var rule = extendedRules[ruleID];
            ...
            IKrPermissionRuleSettings rule)
        {
            ...
            if (taskTypesObj is IEnumerable<Guid> taskTypes)
            {
                foreach (var taskType in taskTypes)
                {
                    ...
                    {
                        extendedTaskSettingsResult = descriptor.ExtendedTasksSettings[taskType] = new(x => x.SectionID);
                    }
        ...
        private async Task ExtendRequiredPermissionsWithRulesAsync(InnerContext innerContext)
        {
            ...
            await this.CheckRulesForAclGenerationRulesAsync(
                innerContext,
                requiredRules,
                true);
            ...
            string methodName)
        {
            try
            {
                await extensions.ExecuteAsync(methodName, context);
            }
            catch (OperationCanceledException)
            {
                throw;
        ...
        private async Task ExtendPermissionsWithTasksAsync(IKrPermissionsManagerContext context)
        {
            ...
            try
            {
                ...
                {
                    await using var extensions = await this.extensionContainer.ResolveExecutorAsync<ITaskPermissionsExtension>();
        ...
        private async Task<bool> FillInnerContextAsync(InnerContext innerContext)
        {
            ...
            var extensionExecutor = await this.extensionContainer.ResolveExecutorAsync<IKrPermissionsRuleExtension>(synchronized: true);
            ...
            var conditionContext =
                    ...
                    this.unityContainer)
                {
                    ...
                    Info =
                    {
                        [nameof(IKrPermissionsManagerContext)] = innerContext.Context
                    }
                };
        ...
        private async Task<Card?> GetCardForTokenCheckAsync(
            Guid cardID,
        ...
        private async Task<IList<Guid>> GetRulesForStaticRolesAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using (this.dbScope.Create())
            {
                ...
                int step = db.Dbms == Dbms.PostgreSql ? 10000 : 1000;
                if (count <= step)
                {
        ...
        private async Task<IKrPermissionsManagerResult> CreateResultAsync(InnerContext innerContext)
        {
            ...
            return new KrPermissionsManagerResult(
                ...
                innerContext.SubmittedRules,
                innerContext.RejectedRules);
        }

        private IKrPermissionsManagerResult CreateFullResult(
            IKrPermissionsManagerContext context,
            KrPermissionFlagDescriptor[] requiredPermissions)
        {
            var descriptor = new KrPermissionsDescriptor(requiredPermissions);
            foreach (var permission in requiredPermissions)
            {
                descriptor.Set(permission, true, true);
            }
            ...
            return new KrPermissionsManagerResult(
                ...
                this.krFileOwnershipChecker,
                context.WithExtendedPermissions,
                context.PermissionsCache.Version,
                null,
                null);
        }
