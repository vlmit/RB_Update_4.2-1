...
{
    public abstract class KrSourceBuilder<T>(
        ...
        : IKrSourceBuilder<T>
    {
        ...
        protected List<IExtraSource> ExtraSources { get; } = [];

        protected List<string> DefaultConstructorParts { get; } = [];

        protected ICompilationSourceProvider CompileSourceProvider { get; } = NotNullOrThrow(compileSourceProvider);

        protected IKrPreprocessorProvider PreprocessorProvider { get; } = NotNullOrThrow(preprocessorProvider);
        ...
        protected IReadOnlyList<ICompilationSource> BuildExtraSources()
        {
            if (this.ExtraSources.Count == 0)
            {
                return ImmutableList<ICompilationSource>.Empty;
            }
            ...
            {
                var builder = this.CompileSourceProvider.AcquireSyntaxTree();
                var methodBody = source.HasSource
                ...
                builder
                    ...
                    .AddMethod(
                        ...
                        source.Name,
                        [
                            (source.ParameterType, source.ParameterName)
                        ],
                        methodBody,
                        isAsync: true);
