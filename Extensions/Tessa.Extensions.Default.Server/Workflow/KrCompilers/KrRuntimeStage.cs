...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers
{
        ...
        IKrRuntimeStage
    {
        ...
        /// <inheritdoc />
        public async ValueTask<IDictionary<string, object>> GetSettingsAsync(CancellationToken cancellationToken = default) =>
            StorageHelper.Clone(await this.settingsLazy.Value);
        ...
        /// <inheritdoc />
        public bool HasSource => !string.IsNullOrWhiteSpace(this.RuntimeSourceCondition)
            || !string.IsNullOrWhiteSpace(this.RuntimeSourceBefore)
            || !string.IsNullOrWhiteSpace(this.RuntimeSourceAfter)
            || this.ExtraSources.Any(static i => i.HasSource); // Вычисление оставлено в свойстве, что бы свойство ExtraSources могло вычисляться отложено.

        #endregion

        #region Private Methods

        private string GetDebuggerDisplay() => $"{DebugHelper.GetTypeName(this)}: "
                + $"{nameof(this.StageID)}={this.StageID:B}"
                + $", {nameof(this.StageName)}=\"{this.StageName}\""
                + $", {nameof(this.TemplateID)}={this.TemplateID:B}"
                + $", {nameof(this.TemplateName)}\"{this.TemplateName}\""
                + $", {nameof(this.GroupID)}={this.GroupID:B}"
