...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers
{
        ...
        IKrStageGroupCompilationCache
    {
        ...
        {
            var stageGroups = await this.krProcessCache.GetAllStageGroupsAsync(cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var contexts = new List<TessaCompilationContext<IKrCompilationContext>>(stageGroups.Count);
            var commonMethod = commonMethods.MinBy(static i => i.ID);
            ...
            foreach (var stageGroup in stageGroups)
            {
                ...
                {
                    contexts.Add(new(stageGroup.Key) { ValidationResult = result });
                    continue;
                }

                IKrCompilationContext? context = null;

                if (stageGroup.Value.HasSource)
                {
                    context = getCompilerContextFunc();
                    context.StageGroups.Add(stageGroup.Value);
                    context.SimpleAssemblyName = CompilationHelper.GetSimpleAssemblyName(this.CategoryID, stageGroup.Key);
                ...
                contexts.Add(new(stageGroup.Key) { CompilerContext = context });
            }
