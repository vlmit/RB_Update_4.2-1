...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers
{
    public static class KrCompilersSqlHelper
    {
        ...
        private sealed class KrSecondaryProcessData
        {
            ...
            public ButtonToolbarVisibilityMode ToolbarVisibilityMode { get; set; }
                ...
                IDataReader reader)
            {
                ...
                var conditionsStr = reader.GetNullableString(cnt++);
                ...
                {
                    var conditions = StorageHelper.DeserializeListFromTypedJson(conditionsStr);

                    this.Conditions = conditions is null
                        ? []
                        : ConditionSettings.GetFromList(conditions);
                }

                this.NotMessageHasNoActiveStages = reader.GetBoolean(cnt++);
                this.HandlerID = reader.GetNullableGuid(cnt++);
                this.ToolbarVisibilityMode = (ButtonToolbarVisibilityMode) reader.GetInt32(cnt++);
                this.Alias = reader.GetNullableString(cnt++);
                this.Hidden = reader.GetBoolean(cnt);
            }
            ...
            public IKrProcessButton ToProcessButton() =>
                new KrProcessButton(
                    ...
                    runOnce: false, // RunOnce актуально только для вторичных процессов типа "Действие".
                    notMessageHasNoActiveStages: this.NotMessageHasNoActiveStages,
                    contextRolesIDs: this.ContextRolesIDs ?? Enumerable.Empty<Guid>(),
                    executionSqlCondition: this.ExecutionSqlCondition,
                    executionSourceCondition: this.ExecutionSourceCondition,
                    ...
                    toolbarVisibilityMode: this.ToolbarVisibilityMode,
                    alias: this.Alias,
                    hidden: this.Hidden);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            await using (dbScope.Create())
            {
                ...
                var builder = dbScope.BuilderFactory
                    .Select().C(null,
                        KrStageTemplates.ID,
                        KrStageTemplates.NameField,
                        KrStageTemplates.Order,
                        KrStageTemplates.StageGroupID,
                        KrStageTemplates.StageGroupName,
                        KrStageTemplates.GroupPositionID,
                        KrStageTemplates.CanChangeOrder,
                        KrStageTemplates.IsStagesReadonly,
                        KrStageTemplates.SqlCondition,
                        KrStageTemplates.SourceCondition,
                        KrStageTemplates.SourceBefore,
                        KrStageTemplates.SourceAfter)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return [secondaryProcessID.Value];
            }
                    ...
                    .Where()
                        ...
                        .E(w => w
                            ...
                            .Exists(b => b
                                ...
                                .Where().C("v", RefGroupsHelper.RefGroupsValuesID).Equals().C("tt", KrStageTypes.TypeID)
        ...
        /// </summary>
        /// <param name="dbScope">Объект для взаимодействия с базой данных.</param>
        /// <param name="typeID">Идентификатор типа карточки или документа.</param>
            ...
            CancellationToken cancellationToken = default)
        {
            await using (dbScope.Create())
            {
                ...
                var builder = dbScope.BuilderFactory
                    ...
                    .V(DefaultSecondaryProcessGroupOrder) // 4 - StageGroupOrder
                    .C("ks",
                        KrStages.RowID, // 5
                        KrStages.NameField, // 6
                        KrStages.Order, // 7
            ...
            await using (dbScope.Create())
            {
                ...
                var query = dbScope.BuilderFactory
                    ...
                    .C(null,
                        ...
                        KrSecondaryProcesses.TileHandlerID,
                        KrSecondaryProcesses.ToolbarVisibilityModeID,
                        KrSecondaryProcesses.Alias,
                        KrSecondaryProcesses.Hidden)
                    .From(KrSecondaryProcesses.Name).NoLock();
