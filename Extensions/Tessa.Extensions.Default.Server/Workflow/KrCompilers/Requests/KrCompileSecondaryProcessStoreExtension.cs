...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers.Requests
{
        ...
        KrCompileSourceStoreExtensionBase
    {
        ...
        /// <inheritdoc/>
        protected override async ValueTask InvalidateAsync(Card card)
        {
            if (card.StoreMode == CardStoreMode.Update)
            {
                var cardIDArr = new[] { card.ID };

                await this.SecondaryProcessCompilationCache.InvalidateAsync(cardIDArr);
                await this.StageTemplateCompilationCache.InvalidateAsync(cardIDArr);
                await this.StageGroupCompilationCache.InvalidateAsync(cardIDArr);
            }
        }
