...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers.Requests
{
        ...
        CardStoreExtension
    {
        ...
        protected abstract ValueTask InvalidateAsync(Card card);
            ...
            ICardStoreExtensionContext context)
        {
            ...
            {
                await this.InvalidateAsync(card);
            }
            ...
            CancellationToken cancellationToken)
        {
            validationResult.AddInfo(
                ...
                compilationResult.IsSuccessful ?
                    await LocalizeNameAsync("KrMessages_KrStageSourceSuccessfulBuild") :
                    await LocalizeNameAsync("KrMessages_KrStageSourceFailedBuild"));

            validationResult.Add(compilationResult);
        }

        private async Task<ValidationResult> RebuildAllAsync(CancellationToken cancellationToken)
        {
            await this.ProcessCache.InvalidateAsync();

            var commonMethodsResults = await this.CommonMethodCompilationCache.RebuildAllAsync(cancellationToken);
            var stageTemplateCompilationResults = await this.StageTemplateCompilationCache.RebuildAllAsync(cancellationToken);
            var stageGroupCompilationResults = await this.StageGroupCompilationCache.RebuildAllAsync(cancellationToken);
            var secondaryProcessCompilationResults = await this.SecondaryProcessCompilationCache.RebuildAllAsync(cancellationToken);
