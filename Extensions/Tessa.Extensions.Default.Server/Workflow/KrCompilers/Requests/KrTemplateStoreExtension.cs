    ...
    public abstract class KrTemplateStoreExtension : CardStoreExtension
    {
        ...
        public override async Task BeforeRequestWhenTypeResolved(ICardStoreExtensionContext context)
        {
            ...
            if (sections is not null
                && sections.TryGetValue(KrStageTypes.Name, out var stageTypesSection)
            ...
            var hasStageChanges = await this.HasStageChangesAsync(
                card,
                ...
                && (hasStageChanges || this.DoUpdate(context)))
            {
                ...
                var storeContext =
                    await CardStoreContext.CreateAsync(
                        ...
                        context.DbScope.Db,
                        context.Request.TryGetNotFoundEntrySectionList(),
        ...
        private async ValueTask<bool> HasStageChangesAsync(
            Card card,
            CancellationToken cancellationToken = default)
        {
            ...
            switch (storeMode)
            {
                case CardStoreMode.Insert:
                    ...
                    return (await this.Serializer.GetSettingsAsync(cancellationToken))
                        .SettingsSectionNames
                        .Any(p =>
                            card.Sections.TryGetValue(p, out var sec)
                            && sec.HasChanges());
