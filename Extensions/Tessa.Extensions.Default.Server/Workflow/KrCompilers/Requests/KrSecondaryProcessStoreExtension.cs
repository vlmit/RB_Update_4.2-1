...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers.Requests
{
    ...
    public sealed class KrSecondaryProcessStoreExtension : KrTemplateStoreExtension
    {
        ...
        {
            await base.BeforeRequestWhenTypeResolved(context);

            if (!context.ValidationResult.IsSuccessful()
                || context.Request.Card.TryGetSections() is not { } sections)
            {
                return;
            }

            if (sections.TryGetValue(KrSecondaryProcessRoles.Name, out var secondaryProcessRolesSection))
            {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var conditionType = await this.typesProvider.GetAsync(conditionTypeID.Value, cancellationToken);
            if (conditionType is null)
            {
                return;
            }

            // 2. Создаем фейковую карточку
            var conditionCard = new Card { ID = Guid.NewGuid() };

            // 3. Заполняем ее настройками текущего условия, если есть тип карточек настроек условия
            if (conditionType.SettingsTypeID.HasValue)
            {
                ...
                await InitializationExtensionHelper.DeserializeSettingsToSectionsByTypeAsync(
                    conditionType.SettingsTypeID.Value,
            ...
            // 4. Создаем контекст для замены плейсхолдеров и документа
            ...
            var result = await this.placeholderManager.FindAndReplaceAsync(document, placeholderInfo, cancellationToken: cancellationToken);
