...
namespace Tessa.Extensions.Default.Server.Workflow.KrCompilers.UserAPI
{
    ...
    /// Если в описании не указан контекст, в котором допустимо использовать, значит свойство/метод допустимо использовать в любом контексте.<br/>
    ///
    /// Виды скриптов:
    /// <list type="bullet">
    /// <item>Скрипт выполнения этапа - скрипты, находящиеся в строке этапа в шаблоне этапов. Запускаются в процессе выполнения;</item>
    /// <item>Скрипт построения шаблона этапов - скрипты на основной вкладке шаблона этапов. Запускаются при построении маршрута;</item>
    /// <item>Скрипт построения группы этапов - скрипты на основной вкладке группы этапов. Запускаются при построении маршрута;</item>
    /// <item>Скрипт выполнения группы этапов - скрипты на основной вкладке группы этапов. Запускаются при выполнении маршрута;</item>
    /// <item>Скрипт видимости кнопки процесса - скрипты на основной вкладке кнопки процесса. Запускаются при загрузке карточки, включенной в типовой процесс;</item>
    /// <item>Скрипт выполнения процесса - скрипты на основной вкладке процесса. Запускаются при запуске вторичного процесса.</item>
    /// </list>
    ///
    /// Режимы:
    /// <list type="bullet">
    /// <item>Синхронный режим - процесс запускается полностью в памяти. Создание процессного сателлита не происходит;</item>
    /// <item>Асинхронный режим - процесс запускается с заполнением процессного сателлита. Для основного процесса контекстуальный и процессный сателлит совпадают, для вторичных создаются отдельные KrSecondarySatellite.</item>
    /// </list>
    ///
    /// Контексты запуска:
    /// <list type="bullet">
    /// <item>Глобальный - процесс запускается без карточки. Такой процесс может быть только синхронным. Например, нажатие на кнопку процесса в правой панели.</item>
    /// <item>Локальный - процесс запускается по карточке. Это основной процесс и кнопки процесса в левой панели.</item>
    /// </list>
    /// </remarks>
        ...
        IExtensionContext
    {
        ...
        /// <list type="bullet">
        /// <item>Скрипт выполнения этапа;</item>
        /// <item>Скрипт построения группы этапов;</item>
        /// <item>Скрипт выполнения группы этапов.</item>
        /// </list>
        ///
        /// Поддерживаемые режимы:
        /// <list type = "bullet" >
        /// <item>Асинхронный режим.</item>
        /// </list>
        ///
        ...
        /// Значения:
        /// <list type="bullet">
        /// <item><see cref="KrConstants.KrProcessName"/></item>
        /// <item><see cref="KrConstants.KrSecondaryProcessName"/></item>
        /// </list>
        ///
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        ...
        /// <item>Скрипт выполнения этапа;</item>
        /// <item>Скрипт выполнения группы этапов;</item>
        /// <item>Скрипт видимости кнопки процесса;</item>
        /// <item>Скрипт выполнения процесса.</item>
        /// </list>
        ///
        /// Поддерживаемые режимы:
        /// <list type = "bullet" >
        ...
        /// <item>Скрипт выполнения группы этапов;</item>
        /// <item>Скрипт видимости кнопки процесса;</item>
        /// <item>Скрипт выполнения процесс.</item>
        /// </list>
        ///
        /// Поддерживаемые режимы:
        /// <list type = "bullet" >
        ...
        /// <list type="bullet">
        /// <item>Скрипт выполнения этапа;</item>
        /// <item>Скрипт построения шаблона этапов;</item>
        /// <item>Скрипт построения группы этапов;</item>
        /// <item>Скрипт выполнения группы этапов.</item>
        /// </list>
        ///
        ...
        /// </summary>
        /// <remarks>
        /// Владелец процесса - это персональная роль, от имени которой выполняется пересчёт маршрута.<br/>
        ///
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        /// <item>Скрипт выполнения этапа;</item>
        ...
        /// Может использоваться для простого и лаконичного обращения к строковым секциями.<br/>
        /// Например:<br/>
        /// <c>Card.DocumentCommonInfo.Amount</c><br/>
        /// <c>Card.MyCustomSection.CustomField</c><br/>
        /// </summary>
        ...
        /// Может использоваться для простого и лаконичного обращения к коллекционным секциями.<br/>
        /// Например:<br/>
        ...
        /// </summary>
        /// <returns>
        /// <see langword="true"/>, если текущий процесс является основным KrProcess.
        /// <see langword="false"/>, если текущий процесс является вторичным KrSecondaryProcess.
        /// </returns>
        /// <remarks>
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        ...
        /// </summary>
        /// <returns>
        /// Значение <see langword="true"/>, если для текущей карточки запущен основной процесс, иначе - <see langword="false"/>.
        /// </returns>
        /// <remarks>
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        ...
        /// Позиция, на которую необходимо вставить этап. По умолчанию этап добавляется в конец.
        /// </param>
        /// <returns>Новый этап или значение <see langword="null"/>, если этап с таким именем уже существует в маршруте.</returns>
        /// <remarks>
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        /// <item>Скрипт построения шаблона этапов.</item>
        ...
        /// <summary>
        /// Сбрасывает исполнителя для этапа с режимом одиночного исполнителя <see cref="PerformerUsageMode.Single"/>.
        /// </summary>
        ...
        /// </param>
        /// <param name="ignoreManualChanges">Добавить исполнителя, даже если этап изменен пользователем.</param>
        /// <returns>Добавленный исполнитель или значение <see langword="null"/>, если исполнитель не был добавлен.</returns>
        /// <remarks>
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
        /// <item>Скрипт выполнения этапа;</item>
        ...
        /// Если <see langword="null"/>, тогда используется текущая карточка.</param>
        ...
        /// Если стоит <see langword="false"/>, то при попытке выполнить отсутствующий метод ничего не произойдет.
        /// <see langword="true"/> рекомендуется для скриптов, которые создаются вместе с типом этапа.
        /// <see langword="false"/> необходимо использовать в том случае, когда этап существовал ранее, а скрипт добавляется в
        ...
        /// <param name="throwOnError">
        /// Выбрасывать исключение при возникновении ошибки.
        /// Если стоит <see langword="false"/>, то при попытке выполнить отсутствующий метод будет возвращено значение по умолчанию для типа <typeparamref name="T"/>.
        /// <see langword="true"/> рекомендуется для скриптов, которые создаются вместе с типом этапа.
        /// <see langword="false"/> необходимо использовать в том случае, когда этап существовал ранее, а скрипт добавляется в
        /// уже существующий тип этапа.
        /// </param>
        ...
        /// </summary>
        /// <returns>
        /// Стратегия загрузки карточки, получаемой из <see cref="Stage.Info"/> этапа по ключу <see cref="KrConstants.Keys.NewCard"/>.
        /// </returns>
        /// <remarks>
        /// Поддерживаемые скрипты:
        /// <list type="bullet">
