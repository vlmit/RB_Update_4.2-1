...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow.Handlers
{
    ...
    public class ProcessManagementStageTypeHandler : StageTypeHandlerBase
    {
            ...
            string customSignal = null)
        {
            ...
            if (signal is KrConstants.KrStartProcessSignal
                or KrConstants.KrStartProcessUnlessStartedGlobalSignal)
            {
                var startingProcessID = Guid.NewGuid();

                parameters = new Dictionary<string, object?>()
                {
                    { KrConstants.KrProcessID, startingProcessID }
                };

                context.WorkflowProcess.InfoStorage[KrConstants.KrProcessID] = startingProcessID;
            }
