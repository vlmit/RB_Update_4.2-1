...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess.Workflow.Handlers
{
    ...
    public class ResolutionStageTypeHandler : StageTypeHandlerBase
    {
            ...
            ISession session)
        {
            ...
            this.ContextRoleCache = NotNullOrThrow(contextRoleCache);
        ...
        /// <summary>
        /// Возвращает или задаёт кэш для карточек контекстных ролей.
        /// </summary>
        protected ICardContextRoleCache ContextRoleCache { get; set; }
        ...
        public override async Task<StageHandlerResult> HandleStageStartAsync(IStageTypeHandlerContext context)
        {
            ...
            if (context.CardExtensionContext is not ICardStoreExtensionContext storeContext)
            {
            ...
            {
                author = await HandlerHelper.GetStageAuthorAsync(
                    context,
                    this.RoleGetStrategy,
                    this.ContextRoleManager,
                    this.ContextRoleCache,
        ...
        public override async Task<bool> HandleStageInterruptAsync(IStageTypeHandlerContext context)
        {
            ...
            await using (scope.Create())
            {
                ...
                var card = await this.KrScope.GetMainCardAsync(
                    storeContext.Request.Card.ID,
                    cardTypeID: storeContext.Request.Card.TypeID,
        ...
        public override Task<StageHandlerResult> HandleSignalAsync(IStageTypeHandlerContext context)
        {
            ...
            return signal?.Signal.Name == KrPerformSignal
                ? Task.FromResult(StageHandlerResult.CompleteResult)
                : Task.FromResult(StageHandlerResult.EmptyResult);
        }
