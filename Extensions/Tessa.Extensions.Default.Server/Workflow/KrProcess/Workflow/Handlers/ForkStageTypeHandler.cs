    ...
    public class ForkStageTypeHandler : ForkStageTypeHandlerBase
    {
            ...
            IStageTypeHandlerContext context)
        {
            ...
            var branchInfos = new ListStorage<BranchInfo>(
                context.Stage.InfoStorage.Get<List<object>>(PendingProcesses) ?? new List<object>(),
                BranchInfoFactory);
        ...
        protected async Task<StageHandlerResult> HandleBranchCompletionAsync(
            ...
            Guid pid)
        {
            var branchInfos = new ListStorage<BranchInfo>(context.Stage.InfoStorage.Get<List<object>>(PendingProcesses) ?? new List<object>(), BranchInfoFactory);
            ...
            var nestedProcessInfo =
                NotNullOrThrow(context.SignalInfo)
                .Signal
                .Parameters
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using (this.DbScope.Create())
            {
                ...
                while (await reader.ReadAsync(cancellationToken))
                {
                    ...
                    var paramsJson = await reader.GetSequentialNullableStringAsync(1, dbms, cancellationToken);
