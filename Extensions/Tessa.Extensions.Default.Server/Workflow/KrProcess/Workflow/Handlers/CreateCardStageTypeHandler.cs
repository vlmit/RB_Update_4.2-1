    ...
    public class CreateCardStageTypeHandler : StageTypeHandlerBase
    {
            ...
            IStageTypeHandlerContext context)
        {
            ...
            if (localCtx.Mode == CreateCardMode.Open)
            {
                ...
                // 2. Передача карточки-заготовки на клиент через клиентскую команду;
                ...
                // Это приводит к созданию некорректного пакета карточки на клиенте.
                var newFiles = newCard.TryGetFiles();
                newFiles = newFiles is { Count: > 0 }
                    ? newFiles.Clone()
                    : null;
                ...
                var serialized = Encoding.UTF8.GetBytes(newCard.ToTypedJson());
