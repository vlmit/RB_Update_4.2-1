...
{
    public sealed class KrInfoForInitiatorGetExtension(
        [Dependency(CardRepositoryNames.Default)] ICardRepository defaultRepository,
        IKrTypesCache krCache) : CardGetExtension
    {
        #region Fields

        private readonly ICardRepository defaultRepository = NotNullOrThrow(defaultRepository);

        private readonly IKrTypesCache krCache = NotNullOrThrow(krCache);
        ...
        public override async Task AfterRequest(ICardGetExtensionContext context)
        {
            if (!context.RequestIsSuccessful
                ...
                || context.CardType?.Flags.HasNot(CardTypeFlags.AllowTasks) is true
                ...
                || card.Tasks.Any(x => x.TypeID == CardHelper.ApprovalProcessInfoTypeID)
            ...
            await using (context.DbScope.Create())
            {
                ...
                await using (DbDataReader reader = await db.ExecuteReaderAsync(context.CancellationToken))
                {
                    while (await reader.ReadAsync(context.CancellationToken))
                    {
                        ...
                        string userName = reader.GetNullableString(1);
            ...
            if (rows is not null)
            {
                ...
                infoForInitiator.Rows.AddItemsByRef(rows);
