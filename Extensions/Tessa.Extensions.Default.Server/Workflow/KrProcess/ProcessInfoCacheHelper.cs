...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess
{
    ...
    {
        private const string CachedProcessInfo = StorageHelper.SystemKeyPrefix + nameof(CachedProcessInfo);
            ...
            Card satellite)
        {
            if (!satellite.TryGetKrApprovalCommonInfoSection(out var aci)
                || !aci.RawFields.TryGetValue(KrConstants.KrProcessCommonInfo.Info, out var infoObj)
            ...
            IDictionary<string, object> info)
        {
            if (satellite.TryGetKrApprovalCommonInfoSection(out var aci))
            {
                aci.Fields[KrConstants.KrProcessCommonInfo.Info] = serializer.Serialize(info);
            }
        }
            ...
            Card satellite)
        {
            if (satellite.TryGetKrApprovalCommonInfoSection(out var aci))
            {
                aci.Fields.Remove(KrConstants.KrProcessCommonInfo.Info);
            }
        }
