        ...
        public override async Task BeforeRequestWhenTypeResolved(ICardStoreExtensionContext context)
        {
            ...
            if (card.Sections.TryGetValue(KrConstants.KrStages.Virtual, out var stageSection))
            {
                foreach (var row in stageSection.Rows)
                {
                    ...
                    {
                        var role = await this.roleGetStrategy.GetRoleParamsAsync(authorID.Value);
                        if (role.Type != RoleType.Context && role.Type != RoleType.Personal)
                        {
                            context.ValidationResult.AddError(this, "$KrProcess_UniversalTask_AuthorShouldBePersonalOrContext");
                            return;
                        }
                    }
                }
