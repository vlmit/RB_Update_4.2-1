...
namespace Tessa.Extensions.Default.Server.Workflow.KrProcess
{
    ...
    public readonly struct KrProcessSectionMapper
    {
        ...
        private sealed class StagePosition
        {
                ...
                CardRow? modifiedRow)
            {
                ...
                this.StageGroupOrder = krStagePositionInfo?.GroupOrder
                    ?? modifiedRow!.Get<int>(KrConstants.KrStages.StageGroupOrder);
        ...
        /// </param>
        /// <param name="addDestinationSection">
        /// Значение <see langword="true"/>, если секция, в которую переносятся данные, расположенная в <paramref name="destinationSectionAlias"/>,
        /// должна быть добавлена при её отсутствии, иначе - <see langword="false"/>.
        /// Если значение не задано, то используется значение, указанное в конструкторе <see cref="KrProcessSectionMapper(Card,Card,bool)"/>.
        /// </param>
            ...
            CardRow destRow)
        {
            ...
            foreach (var pair in sourceRow
                .Where(k => !k.Key.StartsWith(StorageHelper.SystemKeyPrefix, StringComparison.Ordinal)
