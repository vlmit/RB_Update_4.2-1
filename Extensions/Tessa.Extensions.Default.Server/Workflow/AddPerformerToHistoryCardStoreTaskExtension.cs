...
namespace Tessa.Extensions.Default.Server.Workflow
{
    ...
    public class AddPerformerToHistoryCardStoreTaskExtension : CardStoreTaskExtension
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var roleType = performerTaskAssignedRoleRow.RoleType
                ?? await RoleHelper.TryGetRoleTypeAsync(roleID, dbScope.Db, dbScope.BuilderFactory, cancellationToken);

            if (roleType is not null &&
                CardComponentHelper.IsTemporaryTaskRole(roleType))
            {
            ...
            historySettings[TaskHistorySettingsKeys.PerformerRoleTypeID] = RoleHelper.Box(roleType);
        }
