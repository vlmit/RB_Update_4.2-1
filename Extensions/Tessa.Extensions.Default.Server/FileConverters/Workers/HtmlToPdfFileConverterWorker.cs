...
{
    /// <summary>
    /// Объект, ответственный за преобразование файла в формат <see cref="FileConverterFormat.Pdf"/> из формата HTML.
    /// Обычно не используется как самостоятельный Worker, а применяется в составе <see cref="PdfFileConverterWorker"/>.
    /// Регистрация выполняется по константе <see cref="FileConverterWorkerNames.HtmlToPdf"/>.
    /// </summary>
    /// <remarks>
        ...
        IAsyncDisposable
    {
        ...
        public HtmlToPdfFileConverterWorker(IJinniBalancerProxyFactory proxyFactory)
            : base(proxyFactory)
        {
        }
