...
namespace Tessa.Extensions.Default.Server.BusinessCalendar
{
        ...
        CardStoreExtension
    {
        ...
        public override async Task AfterRequest(ICardStoreExtensionContext context)
        {
            ...
            if ((info = context.Request.TryGetInfo()) is not null &&
                info.TryGet<bool?>(BusinessCalendarHelper.RebuildMarkKey) is true)
            {
                var cardID = context.Request.Card.ID;
                var errorCardID = Guid.NewGuid();
                ...
                context.Response!.Info[BusinessCalendarHelper.RebuildOperationErrorCardIDKey] = errorCardID;
            }
        }
                        ...
                        while (await reader.ReadAsync(context.CancellationToken))
                        {
                            ...
                            if (startTime >= endTime)
                            {
                                context.ValidationResult.AddError(this,
                                    await LocalizeAndEscapeFormatAsync("$BusinessCalendar_Exclusions_EndDateMustBeGEThenStartDate") ?? string.Empty,
                ...
                if (namedRangesSection.Rows.Count > 0)
                {
                    ...
                    await using (context.DbScope!.Create())
                    {
                        ...
                        while (await reader.ReadAsync(context.CancellationToken))
                        {
                            ...
                            {
                                context.ValidationResult.AddError(this,
                                    await LocalizeAndEscapeFormatAsync("$BusinessCalendar_NamedRanges_EndDateMustBeGEThanStartDate") ?? string.Empty,
                                    FormatDateTime(startTime, convertToLocal: false), 
                                    FormatDateTime(endTime, convertToLocal: false));
            ...
            if (context.Request.Card.Sections.TryGetValue(BusinessCalendarHelper.CalendarSettingsSection, out var settingsSection))
            {
                await using (context.DbScope!.Create())
                {
                    ...
                    if (await reader.ReadAsync(context.CancellationToken))
                    {
                        ...
                        {
                            context.ValidationResult.AddError(this,
                                await LocalizeAndEscapeFormatAsync("$BusinessCalendar_EndDateMustBeGEThenStartDate") ?? string.Empty,
                ...
                {
                    await this.calendarSettingsCache.InvalidateAsync(context.Request.Card.ID);
                }
            }
        }
