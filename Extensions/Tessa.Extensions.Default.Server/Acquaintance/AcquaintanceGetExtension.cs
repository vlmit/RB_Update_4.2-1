...
namespace Tessa.Extensions.Default.Server.Acquaintance
{
    ...
    public sealed class AcquaintanceGetExtension : CardGetExtension
    {
        ...
        public AcquaintanceGetExtension(IKrTypesCache typesCache)
        {
        ...
        public override async Task AfterRequest(ICardGetExtensionContext context)
        {
            ...
            await using (context.DbScope.Create())
            {
                ...
                if (rows.Count > 0)
                {
                    ...
                    {
                        string sendDateText = FormatDateTimeWithoutSeconds(row.Sent);
                        ...
                        {
                            acquaintanceResult.AddInfo(
                                this,
                                "$KrMessages_Acquaintance_Received",
                                row.SenderName,
                                sendDateText);
                        }
                        else
                        {
                            acquaintanceResult.AddInfo(
                                this,
                                "$KrMessages_Acquaintance_ReceivedAndCommented",
                                row.SenderName,
                                sendDateText,
                                await LocalizeAsync(comment));
                        }
                    }
