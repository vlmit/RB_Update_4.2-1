...
namespace Tessa.Extensions.Default.Server.AbTest
{
    ...
    /// </summary>
    public sealed class AbPermissionTestExtension : IKrPermissionsRuleExtension, ICardPermissionsExtension
    {
                    ...
                    .SetCommand(
                        dbScope.BuilderFactory
                            .SelectExists(b => b
                                ...
                                .And(bb => bb.C("MinAmount").IsNull().Or().C("MinAmount").LessOrEquals().P("Amount"))
                                .And(bb => bb.C("MaxAmount").IsNull().Or().C("MaxAmount").GreaterOrEquals().P("Amount")))
                            .Build(),
        ...
        public async Task CheckRuleAsync(IKrPermissionsRuleExtensionContext context)
        {
            switch (context.Mode)
            {
                ...
                {
                    decimal? amount = await SelectAmountFromDbAsync(context.DbScope, NotNullOrThrow(context.CardID), context.CancellationToken);
                    if (amount.HasValue
                        && !await CheckOnDbAsync(context.DbScope, context.RuleID, amount.Value, context.CancellationToken))
                    {
                        context.Cancel = true;
        ...
        /// <inheritdoc/>
        public async Task ExtendPermissionsBeforeTokenCheckAsync(IKrPermissionsManagerContext context)
        {
        ...
        /// <inheritdoc/>
        public async Task ExtendPermissionsAfterTokenCheckAsync(IKrPermissionsManagerContext context)
        {
            // При чтении карточки с суммой меньше 1000 - запрещаем добавлять файлы.
            if (context.Mode == KrPermissionsCheckMode.WithCard)
            {
                Card card = context.Card;
                if (card.Sections.ContainsKey("DocumentCommonInfo")
                    ...
                    && context.Descriptor.Permissions.Contains(KrPermissionFlagDescriptors.AddFiles))
                {
                    var amount = card.Sections["DocumentCommonInfo"].Fields.TryGetValue<decimal?>("Amount").Value;
                    if (amount is < 1000)
                    {
                        context.Descriptor.Set(KrPermissionFlagDescriptors.AddFiles, false);
                    }
                }
            }
        }
