...
namespace Tessa.Extensions.Default.Server.Notices
{
        ...
        IMessageHandler
    {
        ...
        private static readonly Regex krHeaderRegex = new(
        ...
        public async ValueTask<IMessageInfo?> TryParseAsync(NoticeMessage message, CancellationToken cancellationToken = default)
        {
            ...
            var match = krHeaderRegex.Match(message.Subject);
            if (!match.Success)
            {
                return null;
            }
            ...
            var messageText = (message.Body ?? string.Empty).Trim();

            var locCommentTextMessage = await LocalizeNameAsync("KrMessages_CommentTextMessage");

            var commentIndex = messageText.LastIndexOf(
                $"<{locCommentTextMessage}>",
                StringComparison.Ordinal);

            if (commentIndex < 0)
            {
                commentIndex = messageText.LastIndexOf(
                    $"&lt;{locCommentTextMessage}&gt;",
                    StringComparison.Ordinal);
            }

            taskInfo.Comment = commentIndex >= 0
                ? messageText[..commentIndex].Trim()
                : messageText;
        ...
        public async Task HandleAsync(IMessageHandlerContext context)
        {
            ...
            {
                var currentUser = context.Session.User;
