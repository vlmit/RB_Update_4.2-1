...
{
    public sealed class ImapMailReceiver(Func<IMessageProcessor> getProcessorFunc) : IMailReceiver, IPop3ImapSettingsContainer
    {
        #region Fields

        // получаем функцию Func<IMessageProcessor>, чтобы зависимости Unity (всякие кэши метаинфы)
        // не инициализировались раньше времени
        private readonly Func<IMessageProcessor> getProcessorFunc = NotNullOrThrow(getProcessorFunc);
        ...
        {
            ThrowIfNull(this.Pop3ImapSettings);

            logger.Trace("Loading messages.");

            using var client = new ImapClient();
            await client.ConnectAsync(this.Pop3ImapSettings.Host, NotNullOrThrow(this.Pop3ImapSettings.Port), this.Pop3ImapSettings.UseSsl ?? false, cancellationToken);
            ...
            try
            {
                ...
                for (int messageIndex = 0; messageIndex < messageCount; messageIndex++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (entity is MimePart { Content: { } content } part)
                            {
                    ...
                    finally
                    {
                        if (!canceled)
                        {
                            try
                            {
                                await inbox.AddFlagsAsync(uid, MessageFlags.Deleted, true, CancellationToken.None);
