        ...
        IExchangeSettingsContainer
    {
        ...
        private static string ExtractPlainText(MessageBody body) =>
            body.BodyType switch
            {
                ...
                _ => throw ArgumentOutOfRange(body.BodyType)
            };
        ...
        public async Task ReceiveMessagesAsync(CancellationToken cancellationToken = default)
        {
            if (this.service is null)
            {
                ...
                this.service = await ExchangeServiceHelper.CreateExchangeServiceAsync(NotNullOrThrow(this.settings), cancellationToken);
            }
            ...
            ItemView itemView = new(incomingFolder.TotalCount);
            ...
            PropertySet itemPropertySet = new(BasePropertySet.FirstClassProperties, ItemSchema.Attachments);
