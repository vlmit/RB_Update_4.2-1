...
{
    public sealed class Pop3MailReceiver(Func<IMessageProcessor> getProcessorFunc) : IMailReceiver, IPop3ImapSettingsContainer
    {
        #region Fields
                ...
                for (int messageIndex = 0; messageIndex < messageCount; messageIndex++)
                {
                    ...
                    finally
                    {
                        if (!canceled)
                        {
                            try
                            {
                                await client.DeleteMessageAsync(messageIndex, CancellationToken.None);
