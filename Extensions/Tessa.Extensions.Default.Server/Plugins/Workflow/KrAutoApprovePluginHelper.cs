...
{
    /// <summary>
    /// Предоставляет вспомогательные методы для работы автоматического согласования заданий.
    /// </summary>
    public static class KrAutoApprovePluginHelper
    {
        ...
        /// <summary>
        /// Возвращает список заданий, для которых возможно автоматическое согласование.
        /// </summary>
        ...
        /// <returns>Cписок заданий, для которых возможно автоматическое согласование.</returns>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            db
                .SetCommand(
                    builderFactory
                        ...
                        .InnerJoin("Types", "t").NoLock()
                        .On().C("t", "ID").Equals().C("ins", "TypeID")
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                var cardTypeName = reader.GetString(1);
                var taskID = reader.GetGuid(2);
                var approvalComment = reader.GetNullableString(3);

                var stageSettingsJson = await reader.GetSequentialNullableStringAsync(4, db.Dbms, cancellationToken);
                var taskSettingsJson = await reader.GetSequentialNullableStringAsync(5, db.Dbms, cancellationToken);

                var stageSettings = (string.IsNullOrEmpty(stageSettingsJson)
                        ? null
                        : StorageHelper.DeserializeFromTypedJson(stageSettingsJson))
                ...
                if (taskSettings.TryGet<bool>(WorkflowCommonConstants.IsDisableAutoApproval)
                    || stageSettings.TryGet<bool>("KrApprovalSettingsVirtual__DisableAutoApproval"))
                {
                    continue;
                }

                result.Add(
                    new KrAutoApproveTaskRecord(cardID, cardTypeName, taskID, approvalComment));
            }
        ...
        /// <param name="db"><inheritdoc cref="DbManager" path="/summary"/></param>
        /// <param name="builderFactory"><inheritdoc cref="IQueryBuilderFactory" path="/summary"/></param>
        /// <param name="cardMetadata"><inheritdoc cref="ICardRepository" path="/summary"/></param>
        /// <param name="cardRepository"><inheritdoc cref="ICardMetadata" path="/summary"/></param>
        ...
        /// <param name="transactionStrategy"><inheritdoc cref="ICardTransactionStrategy" path="/summary"/></param>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (var task in tasksToApprove)
            {
                ...
                await CompleteApproveTaskAsync(
                    task,
                    db,
                    builderFactory,
                    cardRepository,
        ...
        /// </summary>
        /// <param name="taskToApprove"><inheritdoc cref="KrAutoApproveTaskRecord" path="/summary"/></param>
        /// <param name="db"><inheritdoc cref="DbManager" path="/summary"/></param>
        /// <param name="builderFactory"><inheritdoc cref="IQueryBuilderFactory" path="/summary"/></param>
        /// <param name="cardMetadata"><inheritdoc cref="ICardRepository" path="/summary"/></param>
        /// <param name="cardRepository"><inheritdoc cref="ICardMetadata" path="/summary"/></param>
        ...
        /// <param name="cancellationToken"><inheritdoc cref="CancellationToken" path="/summary"/></param>
        /// <returns><inheritdoc cref="Task" path="/summary"/></returns>
        public static async Task CompleteApproveTaskAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await transactionStrategy.ExecuteInWriterLockAsync(
                taskToApprove.CardID,
                CardComponentHelper.DoNotCheckVersion,
                validationResult,
                async p =>
                {
                    var cardID = taskToApprove.CardID;
                    var taskID = taskToApprove.TaskID;
                    ...
                    if (cardGetContext is null || !p.ValidationResult.IsSuccessful())
                    {
                        p.ReportError = true;
                        return;
                    }
                    ...
                    if (taskContext is null)
                    {
                        validationResult.AddError(
                            nameof(KrAutoApprovePluginHelper),
                            "Failed to get task context, TaskID = {0}",
                            taskID);
                        p.ReportError = true;
                        return;
                    }
                    ...
                    var additionalApproveTasks =
                        await GetAdditionalApprovalInfoAsync(db, builderFactory, taskID, p.CancellationToken);

                    var responsibleTaskOrNull = additionalApproveTasks.FirstOrDefault(x => x.IsResponsible);
                    ...
                    taskToComplete.Card.Sections["KrTask"].Fields["Comment"] = GetAutoApproveComment(
                        responsibleTaskOrNull,
                        taskToApprove);
                    taskToComplete.Flags |= CardTaskFlags.HistoryItemCreated;

                    // Пишем в AutoApproveHistory
                    await AddRowToAutoApproveHistoryAsync(
                        db,
                        builderFactory,
                        cardID,
                        taskID,
                        card.TypeID,
                        card.TypeCaption ?? string.Empty,
                        digest,
                        taskToApprove.ApprovalComment,
                        p.CancellationToken);

                    // Сохраняем карточку

                    // Вычисляем дайджест, чтобы он попал в историю действий.
                    var cardDigest = await cardRepository.GetDigestAsync(
                        card,
                        CardDigestEventNames.ActionHistoryStoreAutoApprove,
                        p.CancellationToken);
                    ...
                    var response = await cardRepository.StoreAsync(
                        request,
                        p.CancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            db
                .SetCommand(
                    builderFactory
                        .Select()
                        .C("aai", "RowID", "UserName")
                        .C("pr", "Position")
                        .C("aai", "IsResponsible", "Answer", "Completed", "OptionID")
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                result.Add(
                    new KrAutoApproveAdditionalTaskRecord(
                        ...
                        reader.GetNullableDateTimeUtc(5) is { } completed && completed != DateTime.MinValue,
                        reader.GetNullableGuid(6)));
            }
            ...
            db
                .SetCommand(
                    builderFactory
                        ...
                        .Select()
                            .P("TaskID").P("CardID", "CardTypeID", "CardTypeCaption", "CardDigest", "Date").C("rou", "UserID").P("Comment")
                        .From("Tasks", "tsk").NoLock()
                        .LeftJoinLateral(rou => rou
                            .SelectDistinct()
                                .C("ru", "UserID")
                            ...
                            .Where()
                                .C("tsk", "RowID").Equals().C("tar", "ID"), "rou")
                        .Where()
                            .C("tsk", "RowID").Equals().P("TaskID")
                        .Build(),
                    db.Parameter("CardID", cardID, DataType.Guid),
                    db.Parameter("TaskID", taskID, DataType.Guid),
                    db.Parameter("CardTypeID", cardTypeID, DataType.Guid),
                    db.Parameter("CardTypeCaption", cardTypeCaption, DataType.NVarChar),
                    db.Parameter("CardDigest", SqlHelper.LimitString(cardDigest, 255), DataType.NVarChar),
                    db.Parameter("Date", DateTime.UtcNow, DataType.DateTime),
                    db.Parameter("Comment", comment, DataType.NVarChar))
                .LogCommand()
        ...
        private static string? GetAutoApproveComment(
            KrAutoApproveAdditionalTaskRecord? responsibleTask,
            KrAutoApproveTaskRecord approveTask)
        {
            if (responsibleTask is null || !responsibleTask.IsCompleted)
            {
        ...
        private static Guid GetCompletionOption(KrAutoApproveAdditionalTaskRecord? responsibleTaskOrNull) =>
            responsibleTaskOrNull is { IsCompleted: true, OptionID: { } }
