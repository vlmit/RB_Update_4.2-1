...
{
    /// <inheritdoc cref="IOnlyOfficeService"/>
        ...
        IOnlyOfficeService
    {
            ...
            ICardCache cardCache)
        {
            ...
            this.fileCacheInfoStrategy = NotNullOrThrow(fileCacheInfoStrategy);
            this.tokenManager = NotNullOrThrow(tokenManager);
            this.r7TokenManager = NotNullOrThrow(r7TokenManager);
            this.fileCache = NotNullOrThrow(fileCache);
        ...
        private async Task StoreNewVersionAsync(Guid infoID, string modifiedUrl, int repeatOnErrorTimes = 1, CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 0; i < repeatOnErrorTimes; i++)
            {
                ...
                try
                {
                    ...
                    (fileStream, string _) = await this.GetModifiedFileFromOnlyOfficeAsync(info, false, cancellationToken: cancellationToken);
        ...
        public async ValueTask<bool> IsEnabledAsync(CancellationToken cancellationToken = default) =>
            await this.cardCache.Cards.GetAsync("OnlyOfficeSettings", cancellationToken) is { IsSuccess: true } settings
            && settings.GetValue().TryGetSections()?.TryGet("OnlyOfficeSettings")?.RawFields.TryGet<string>("ApiScriptUrl") is { Length: > 0 };
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await this.tokenManager.CreateTokenAsync(id, versionId, settings.TokenLifetimePeriod, OnlyOfficeTokenPermissionFlags.Get, cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Guid? realId;
            bool coeditMode = true;
            if ((realId = await this.fileCacheInfoStrategy.TryGetInfoAsync(coeditKey, userId, cancellationToken)) is null)
            {
                realId = id;
                coeditMode = false;
            }
            ...
            switch (status)
            {
                ...
                case OnlyOfficeCallbackDocumentStatus.BeingEdited:
                    if (coeditMode && actionType == OnlyOfficeActionType.CoeditDisconnect)
                    {
                        await this.fileCacheInfoStrategy.UpdateInfoAsync(realId.Value, null, false, cancellationToken);
                    }
                    else
                    {
                ...
                case OnlyOfficeCallbackDocumentStatus.ReadyForSaving:
                    ...
                    if (coeditMode && newFileUrlAfterClose is not null)
                    {
                        await this.StoreNewVersionAsync(realId.Value, newFileUrlAfterClose, 3, cancellationToken);
                    }

                    await this.fileCacheInfoStrategy.UpdateInfoAsync(realId.Value, newFileUrlAfterClose, true, cancellationToken);
                    break;
                ...
                case OnlyOfficeCallbackDocumentStatus.BeingEditedButStateIsSaved:
                    ...
                    await this.fileCacheInfoStrategy.UpdateInfoAsync(realId.Value, newFileUrlAfterForceSave, null, cancellationToken);
                    break;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isNew)
            {
                ...
                accessToken = await this.tokenManager.CreateTokenAsync(fileInfo.ID, versionID, settings.TokenLifetimePeriod, OnlyOfficeTokenPermissionFlags.Get, cancellationToken);
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var results = await this.TryGetCurrentCoeditAsync(new List<Guid> { fileVersionID }, cancellationToken);
            if (results.Count == 0)
            {
        ...
        public async Task<IReadOnlyList<(Guid CurrentVersionID, string? UserNames, DateTime? LastAccessTime)>> TryGetCurrentCoeditAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var result = await this.fileCacheInfoStrategy.TryGetCurrentCoeditInfosAsync(fileVersionIDs, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!canClose)
            {
                throw new HttpStatusCodeException(HttpStatusCode.Forbidden, $"User ID={userID:B} can't close editor for file ID={id:B}.");
            }
        ...
        /// <inheritdoc/>
        public Task<string?> GetR7JWTAsync(
            Guid id,
            string config,
            CancellationToken cancellationToken = default) => this.r7TokenManager.CreateTokenAsync(config);
