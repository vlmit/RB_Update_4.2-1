        ...
        IOnlyOfficeFileCacheInfoStrategy
    {
        ...
        public async Task InsertAsync(IOnlyOfficeFileCacheInfo info, CancellationToken cancellationToken = default)
        {
            ...
            await executor.ExecuteNonQueryAsync(
                ...
                executor.Parameter("SourceFileName", SqlHelper.LimitFileName(info.SourceFileName, CardHelper.FileNameMaxLength), DataType.NVarChar),
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (logger.IsTraceEnabled)
            {
                if (oldestPreviewRequestTime.HasValue)
                {
                    logger.Trace(
                        ...
                        FormatDateTime(oldestPreviewRequestTime.Value, convertToLocal: false));
                }
                else
                {
            ...
            if (oldestPreviewRequestTime is { } dateTime)
            {
                db
                    .SetCommand(
                        builderFactory
                            ...
                            .Build(),
                        db.Parameter("OldestPreviewRequestTime", dateTime, DataType.DateTime));
            }
            else
            {
                db
            ...
            int deleted = await db.LogCommand().ExecuteNonQueryAsync(cancellationToken);
        ...
        public async Task<(IOnlyOfficeFileCacheInfo, bool)> CreateCoeditInfoAsync(Guid versionID, string name, Guid userID, CancellationToken cancellationToken = default)
        {
            ...
            db
                .SetCommand(
                    this.dbScope.BuilderFactory
                        .Select().Top(1).C("CoeditKey")
                        .From("OnlyOfficeFileCache").NoLock()
                        .Where().C("CoeditKey").IsNotNull()
                        .And().C("HasChangesAfterClose").IsNull()
                        .And().C("EditorWasOpen").Equals().V(true)
                        .And().C("SourceFileVersionID").Equals().P("versionID")
            ...
            await using var reader = await db
                .SetCommand(
                    this.dbScope.BuilderFactory
                        ...
                        .Where().C("fv", "RowID").Equals().P("FileVersionID")
                        .Build(),
        ...
        // <inheritdoc />
        public async ValueTask<List<OnlyOfficeCurrentCoedit>?> TryGetCurrentCoeditInfosAsync(IEnumerable<Guid>? fileVersionIDs, CancellationToken cancellationToken = default)
        {
            var versionIDs = fileVersionIDs as IReadOnlyCollection<Guid> ?? fileVersionIDs?.ToArray();
            ...
            return await db
                .SetCommand(
                    this.dbScope.BuilderFactory
                        .Select()
                        .C("oo", "SourceFileVersionID", "CoeditKey", "LastAccessTime")
                        .C("pr", "Name")
                        .From("OnlyOfficeFileCache", "oo").NoLock()
                        .LeftJoin("PersonalRoles", "pr").NoLock()
                        .On().C("pr", "ID").Equals().C("oo", "CreatedByID")
                        .Where().C("oo", "CoeditKey").IsNotNull()
                        .And().C("oo", "HasChangesAfterClose").IsNull()
                        .And().C("oo", "EditorWasOpen").Equals().V(true)
                        .And().C("oo", "SourceFileVersionID")
                        .InArray(versionIDs, nameof(fileVersionIDs), out var dpVersionIDs)
