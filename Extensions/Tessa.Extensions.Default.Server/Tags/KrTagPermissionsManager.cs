...
namespace Tessa.Extensions.Default.Server.Tags
{
    ...
    public class KrTagPermissionsManager : TagPermissionsManager
    {
        ...
        /// <inheritdoc/>
        public override async ValueTask<bool> CanAddTagsAsync(
            Guid cardID,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await base.CanAddTagsAsync(cardID, tagIDs, tokenInfo, result, cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                // Если карточка не относится к типовому решению, то не проводим проверку.
                return permContextResult.Status == KrPermissionsCreateContextStatus.NotAllowed;
            }

            var checkResult = await this.permissionsManager.CheckRequiredPermissionsAsync(permContextResult.Context, readCardPermissions);
