...
namespace Tessa.Test.Default.Shared
{
        ...
        SetupDbScopeAttribute
    {
        ...
        {
            ThrowIfNull(testScriptFileNames);
        ...
        public override void BeforeTest(ITest test)
        {
            ...
            testActions
                ...
                .Add(new TestAction(test, this.AfterInitializeContainerAsync));

            testActions
                .GetTestActions(ActionStage.AfterOneTimeTearDownScope)
                .Add(new TestAction(test, this.AfterOneTimeTearDownScopeAsync));
        }
        ...
        private async ValueTask AfterInitializeContainerAsync(object sender)
        {
            ...
            var configurationManager = await ConfigurationManager.GetDefaultAsync();
            ...
            {
                this.InitializeDbScopeContainerWithConnectionString(configurationManager, dbScopeContainer, scopeConnectionString);
                return;
            }
            ...
            using (await scopeContext.AsyncLock.EnterAsync())
            {
                ...
                this.connection = configuration.Connections[this.ConfigurationString];
                ...
                this.InitializeDbScopeContainer(configurationManager, dbScopeContainer);
        ...
        private void InitializeDbScopeContainerWithConnectionString(
            IConfigurationManager configurationManager,
            IDbScopeContainer dbScopeContainer,
            string scopeConnectionString)
        {
            var connection = configurationManager.Configuration.Connections[this.ConfigurationString];
            ...
            IDbScopeContainer dbScopeContainer)
        {
            ...
            dbScopeContainer.DbScope = this.SingleConnectionDbScope
                ? new SingleConnectionDbScope(configurationManager, factory.Create)
                : new DbScope(configurationManager, factory.Create);
        }

        private async ValueTask AfterOneTimeTearDownScopeAsync(object sender)
        {
            ...
            await using (container.DbScope.Create())
            {
                ...
                {
                    await TestHelper.DropDatabaseAsync(this.connection);
                }
            }
        }
