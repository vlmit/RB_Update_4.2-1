        ...
        IWrapSetUpTearDown
    {
        ...
        private sealed class ConnectionFactory
        {
            ...
            public ConnectionFactory()
            {
                ...
                try
                {
                    var valueTask = container.RegisterDatabaseForConsoleAsync();
                ...
                {
                    var valueTask = container.DisposeAllRegistrationsAsync();
                    if (!valueTask.IsCompletedSuccessfully)
                    {
                        valueTask.AsTask().GetAwaiter().GetResult();
                    }
            ...
            {
                // capture default according to current context
                var configurationManager = ConfigurationManager.Default;

                return new DbScope(configurationManager, () =>
                {
                    var connectionString =
                        (string) this.Properties.Get(DatabasePropertyNames.ConnectionString) ??
                        throw new InvalidOperationException("Connection string is not specified");
                    ...
                    var dataProvider = configurationManager.Configuration
                        .GetConfigurationDataProviderFromType(providerName)
