...
namespace Tessa.Test.Default.Shared.Roles
{
    ...
    public static class PersonalRoleTestHelper
    {
        ...
        public static PersonalRole Create(Card card) => Create(card, Session.SystemID);
        ...
        /// <returns>Объект персональной роли, полученный из карточки персональной роли.</returns>
        public static PersonalRole Create(Card card, Guid modifiedByUserID)
        {
            ...
            if (cardSections.TryGetValue(RoleStrings.PersonalRoles, out var personalRoles))
            {
                Dictionary<string, object> fields = personalRoles.RawFields;
            ...
            if (cardSections.TryGetValue(RoleStrings.RoleDeputies, out var roleDeputies))
            {
                foreach (CardRow row in roleDeputies.Rows)
                {
                    role.Deputies.Add(CreateDeputyRecord(role.ID, row));
                }
            }

            if (cardSections.TryGetValue(RoleStrings.RoleUsers, out var roleUsers))
            {
                foreach (CardRow row in roleUsers.Rows)
                {
        ...
        /// </returns>
        public static void AssertPersonalRole(Card card, Guid modifiedByID, PersonalRole role)
        {
