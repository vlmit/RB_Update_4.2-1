...
namespace Tessa.Test.Default.Shared
{
        ...
        IApplyToTest
    {
            ...
            IDbFactory
        {
            ...
            public DbManager Create()
            {
                ...
                try
                {
                    ...
                    var connection = db.DataConnection.OpenDbConnection();
                    if (connection.Database != this.databaseName)
                    {
        ...
        private readonly Type dbFactoryType;
        ...
        /// </summary>
        public string ConfigurationString { get; init; } = TestHelper.DefaultConfigurationString;

        /// <summary>
        /// Тип СУБД, к которой происходит подключение.
        /// </summary>
        public Dbms Dbms { get; init; } = TestHelper.DefaultDbms;
        ...
        /// что вызовы <see cref="DbScope.CreateNew()"/> (и перегрузки) не создают новых соединений с базой данных и продолжают работать в пределах Scope.
        ...
        public override void BeforeTest(ITest test)
        {
            ...
            testActions.GetTestActions(ActionStage.BeforeInitialize).Add(new TestAction(test, this.BeforeInitializeAsync));
            testActions.GetTestActions(ActionStage.AfterOneTimeTearDownScope).Add(new TestAction(test, AfterOneTimeTearDownScopeAsync));
        }
        ...
        private async ValueTask BeforeInitializeAsync(object sender)
        {
