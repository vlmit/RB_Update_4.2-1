...
namespace Tessa.Test.Default.Shared
{
        ...
        ITestBase
    {
        ...
        private readonly TestActionsContainer testActionsContainer = new();

        private readonly Lock testCardManagerLock = new();
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (this.fixtureName is null)
            {
                ...
                this.fixtureName = scopeContext is null
                    ? await this.GetNextRandomFixtureNameAsync(cancellationToken: cancellationToken)
                    : (string) await scopeContext.Info.GetOrAddAsync(
                        scopeKey,
                        async ct => (object) await this.GetNextRandomFixtureNameAsync(cancellationToken: ct),
                        cancellationToken);
            }
        ...
        {
            return forThis
                ? this.NameResolver.GetFixtureNameAsync(
                    this.GetType(),
                    cancellationToken)
                : this.NameResolver.GetFixtureNameAsync(
        ...
        public Task OneTimeTearDownAsync() =>
            this.testFixtureLifecycleController.OneTimeTearDownAsync();
        ...
        /// <summary>
        /// Объект, используемый для генерации случайных имён и дат.
        /// </summary>
        /// <remarks>
        /// По умолчанию используется зависимость, зарегистрированная в контейнере <see cref="UnityContainer"/>.
        /// В гибридных клиентских тестах определяется из серверного контейнера. Не рекомендуется изменять без необходимости.
        /// </remarks>
        protected ITestNameResolver NameResolver
        {
            get => this.nameResolver ??= this.UnityContainer.Resolve<ITestNameResolver>();
            set => this.nameResolver = value;
        }
        ...
        {
            if (!container.IsRegistered<ITestNameResolver>())
            {
                container.RegisterSingleton<ITestNameResolver, TestNameResolver>();
            }
        ...
        /// <summary>
        /// Выполняет проверку, что данные системы в Redis полностью инициализированы.
        /// </summary>
        /// <param name="container"><inheritdoc cref="IUnityContainer" path="/summary"/></param>
        protected virtual async ValueTask EnsureRedisInitializedAsync(IUnityContainer container)
        {
            // ensure Redis is initialized
            var redisConnectionProvider = container.Resolve<IRedisConnectionProvider>();
            await redisConnectionProvider.GetOpenedConnectionAsync();
        }
