...
namespace Tessa.Test.Default.Shared.Kr
{
        ...
        ICardLifecycleCompanion where T : ICardLifecycleCompanion<T>
    {
        ...
        /// <summary>
        /// Экспортирует карточку.
        /// </summary>
        /// <param name="modifyRequestAction">Метод изменяющий запрос на экспорт карточки.</param>
        /// <returns>Объект <typeparamref name="T"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.<para/>
        /// Можно указать дополнительную информацию, которая будет передана в запросе на экспорт карточки.
        /// </remarks>
        T Export(Action<CardGetRequest> modifyRequestAction = null);
        ...
        /// </summary>
        /// <param name="info">Дополнительная информация. Если для действия уже была задана дополнительная информация, то она будет объединена с указанной
        /// (см. <see cref="Tessa.Platform.Storage.StorageHelper.Merge(IDictionary{string,object?},IDictionary{string, object?},bool,bool)"/>).</param>
        /// <returns>Объект <typeparamref name="T"/> для создания цепочки.</returns>
        ...
        /// <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
