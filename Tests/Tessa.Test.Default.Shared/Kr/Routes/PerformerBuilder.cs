...
namespace Tessa.Test.Default.Shared.Kr.Routes
{
        ...
        IConfiguratorScopeManager<RouteBuilder>
    {
        ...
        /// </summary>
        /// <param name="stageBuilder">Объект, выполняющий создание и настройку этапа для которого требуется выполнить конфигурирование исполнителей.</param>
        ...
        public PerformerBuilder(
            RouteBuilder stageBuilder,
            Func<Card, CardRow> getStageRowFunc,
            Func<Card, CardSection>? getPerformersSectionFunc = null)
        {
            ThrowIfNull(getStageRowFunc);

            this.configuratorScopeManager = new ConfiguratorScopeManager<RouteBuilder>(stageBuilder);
        ...
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public PerformerBuilder AddPerformer(
            Guid roleID,
            string roleName,
            ...
            this.AddPendingAction(
                new PendingAction(
                    ...
                    (builder, _) =>
                    {
                        ...
                        newRow.Fields[KrConstants.KrPerformersVirtual.Order] = Int32Boxes.Box(order);

                        RepairKrPerformersVirtualOrders(rows);
        ...
        /// <summary>
        /// Возвращает исполнителей этапа.
        /// </summary>
        /// <returns>Список строк, содержащих исполнителей этапа.</returns>
        public IEnumerable<CardRow> GetPerformers()
        {
            var stageRowID = this.GetStageRow().RowID;

            return this.GetPerformersSection()
                .Rows
                .Where(i => i.Get<Guid>(KrConstants.KrPerformersVirtual.StageRowID) == stageRowID);
        }
        ...
        private static void RepairKrPerformersVirtualOrders(
            IEnumerable<CardRow> rows)
        {
            KrProcessSharedHelper.RepairStorageRowsOrders(
                rows
                    ...
                    .ToList(),
                KrConstants.KrPerformersVirtual.Order,
                KrConstants.KrPerformersVirtual.StageRowID);
        }
