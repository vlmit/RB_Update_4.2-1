...
namespace Tessa.Test.Default.Shared.Kr.Routes
{
    ...
    public sealed class KrSecondaryProcessBuilder :
        KrRouteObjectBuilder<KrSecondaryProcessBuilder>,
        INamedEntry
    {
        #region Constructors
        ...
        public KrSecondaryProcessBuilder(
            ...
            : base(
                  ...
                  DefaultCardTypes.KrSecondaryProcessTypeName,
                  deps)
        {
        }
        ...
        public KrSecondaryProcessBuilder(
            ...
            : base(
                  AssertOrThrow(
                      NotNullOrThrow(card),
                      card.TypeID == DefaultCardTypes.KrSecondaryProcessTypeID),
                  deps)
        {
        }
        ...
        public KrSecondaryProcessBuilder SetPureProcess(
            bool allowClientSideLaunch = false,
            bool checkRecalcRestrictions = false)
        {
        ...
        /// <summary>
        /// Задаёт указанное состояние в качестве ограничения при пересчёте.
        /// </summary>
        ...
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public KrSecondaryProcessBuilder ForState(
            KrState state)
        {
            this.AddPendingAction(new PendingAction(
                TestHelper.GetCallerMemberFullName(this),
                async (_, ct) =>
                {
                    var row = this.GetCardOrThrow()
                        .Sections[KrConstants.KrStageDocStates.Name]
                        .Rows
                        .Add();

                    row.RowID = Guid.NewGuid();
                    row[KrConstants.KrStageDocStates.StateID] = (short) state.ID;
                    row[KrConstants.KrStageDocStates.StateName] = await this.Dependencies.CardMetadata.TryGetDocumentStateNameAsync(state, ct) ?? state.ToString();
        ...
        /// <summary>
        /// Удаляет состояние из ограничения при пересчёте.
        /// </summary>
        /// <param name="state">Удаляемое состояние.</param>
        /// <returns>Объект <see cref="KrSecondaryProcessBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public KrSecondaryProcessBuilder RemoveState(KrState state) =>
            this.RemoveRow(
                KrConstants.KrStageDocStates.Name,
                row => row.Get<int>(KrConstants.KrStageDocStates.StateID) == state.ID);

        /// <summary>
        /// Удаляет все состояния из ограничения при пересчёте.
        /// </summary>
        /// <returns>Объект <see cref="KrSecondaryProcessBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public KrSecondaryProcessBuilder RemoveAllStates() =>
            this.RemoveAllRows(KrConstants.KrStageDocStates.Name);
