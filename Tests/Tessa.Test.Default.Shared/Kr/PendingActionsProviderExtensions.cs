...
{
    /// <summary>
    /// Содержит статические методы расширения <see cref="IPendingActionsProvider{TAction,T}"/>.
    /// </summary>
    public static class PendingActionsProviderExtensions
    {
            ...
            string name = default) where T : IPendingActionsProvider<IPendingAction, T>
        {
            ...
            var nameInternal = TestHelper.GetCallerMemberFullName(typeof(PendingActionsProviderExtensions));
        ...
        {
            ThrowIfNull(executor);
            ThrowIfNull(producer);

            executor.AddPendingAction(
                new PendingAction(
                    TestHelper.GetCallerMemberFullName(typeof(PendingActionsProviderExtensions)),
