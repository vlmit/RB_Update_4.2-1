...
namespace Tessa.Test.Default.Shared.Kr
{
    ...
    public interface IImportObjects
    {
        ...
        /// <summary>
        /// Возвращает значение, показывающее, необходимо ли выполнять импорт карточек с файлами из конфигурации.
        /// </summary>
        /// <remarks>Для настройки импортируемых карточек следует использовать функцию задаваемую <see cref="ImportCardsWithFilesCardLibPredicateAsync"/>.</remarks>
        bool IsImportCardsWithFiles { get; }
        ...
        /// <summary>
        /// Фильтрует импортируемые карточки при установленном свойстве <see cref="IsImportCardsWithFiles"/>.
        /// </summary>
        /// <param name="card">Импортируемая карточка.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>Значение <see langword="true"/>, если карточка должна быть импортирована, иначе - <see langword="false"/>.</returns>
        ValueTask<bool> ImportCardsWithFilesCardLibPredicateAsync(Card card, CancellationToken cancellationToken = default);
