...
namespace Tessa.Test.Default.Shared.Kr
{
        ...
        IPendingActionsExecutor<PermissionsConfigurator>
    {
        ...
        /// <inheritdoc cref="AddFlags(IEnumerable{KrPermissionFlagDescriptor})"/>
        public PermissionsConfigurator AddFlags(params KrPermissionFlagDescriptor[] flags) =>
            this.AddFlags((IEnumerable<KrPermissionFlagDescriptor>) flags);
                ...
                async (_, ct) =>
                {
                    await AddRowInternalAsync(
                        ...
                        async (stateID, r, ct) =>
                        {
                            ...
                            r.Fields["StateName"] = await this.deps.CardMetadata.TryGetDocumentStateNameAsync((KrState) stateID, ct) ?? stateID.ToString();
                        },
                        ct);
                    return ValidationResult.Empty;
                });
        ...
        /// <param name="contextRole">Признак того, что роль является контекстной.</param>
        /// <returns>Объект <see cref="PermissionsConfigurator"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
