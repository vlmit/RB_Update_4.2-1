...
namespace Tessa.Test.Default.Shared
{
    ...
    /// </list>
    /// </remarks>
    public sealed class TestMessageProvider(LoggerMessageProvider loggerMessageProvider) :
        IMessageProvider
    {
        #region Fields

        private readonly LoggerMessageProvider loggerMessageProvider = NotNullOrThrow(loggerMessageProvider);
        ...
        public async ValueTask ShowNotEmptyAsync(ValidationResult result, CancellationToken cancellationToken = default)
        {
            ...
            if (result.HasErrors)
            {
                ...
                TestHelper.SetAssertionResult(FailureSite.Test, str ?? "<empty>");
            }
            else
            {
