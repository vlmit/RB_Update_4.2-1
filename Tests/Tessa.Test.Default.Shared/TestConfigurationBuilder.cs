...
namespace Tessa.Test.Default.Shared
{
        ...
        PendingActionsProvider<IPendingAction, TestConfigurationBuilder>
    {
        ...
        private readonly IGroupInitializer groupInitializer;

        private readonly ISettingsUnitInitializer settingsUnitInitializer;
        ...
        /// <param name="groupInitializer"><inheritdoc cref="IGroupInitializer" path="/summary"/></param>
        /// <param name="settingsUnitInitializer"><inheritdoc cref="ISettingsUnitInitializer" path="/summary"/></param>
        public TestConfigurationBuilder(
            ...
            IGroupInitializer groupInitializer,
            ISettingsUnitInitializer settingsUnitInitializer)
        {
            this.ResourceAssembly = NotNullOrThrow(resourceAssembly);
            this.DbScope = NotNullOrThrow(dbScope);
            this.CardManager = NotNullOrThrow(cardManager);
            this.CardLibraryManager = NotNullOrThrow(cardLibraryManager);
            this.CardRepository = NotNullOrThrow(cardRepository);
            this.CardFileSourceSettings = NotNullOrThrow(cardFileSourceSettings);
            this.Dependencies = NotNullOrThrow(deps);
            this.OperationRepository = NotNullOrThrow(operationRepository);
            this.cardTypeServerRepository = NotNullOrThrow(cardTypeServerRepository);
            this.cardTypeClientRepository = null;
            this.cardMetadataCache = NotNullOrThrow(cardMetadataCache);
            this.cardCachedMetadata = null;
            this.tessaViewService = NotNullOrThrow(tessaViewService);
            this.jsonViewModelImporter = NotNullOrThrow(jsonViewModelImporter);
            this.jsonViewModelAdapter = NotNullOrThrow(jsonViewModelAdapter);
            this.groupInitializer = NotNullOrThrow(groupInitializer);
            this.settingsUnitInitializer = NotNullOrThrow(settingsUnitInitializer);
        }
            ...
            IJsonViewModelConverter jsonViewModelAdapter)
        {
            ...
            this.Dependencies = NotNullOrThrow(deps);
            this.OperationRepository = NotNullOrThrow(operationRepository);
            this.cardTypeServerRepository = null;
            this.cardTypeClientRepository = NotNullOrThrow(cardTypeClientRepository);
            this.cardMetadataCache = null;
            this.cardCachedMetadata = NotNullOrThrow(cardCachedMetadata);
            this.tessaViewService = NotNullOrThrow(tessaViewService);
            this.jsonViewModelImporter = NotNullOrThrow(jsonViewModelImporter);
            this.jsonViewModelAdapter = NotNullOrThrow(jsonViewModelAdapter);
        }
        ...
        /// <see cref="IPendingActionsExecutor{T}.GoAsync(System.Action{Tessa.Platform.Validation.ValidationResult}?,CancellationToken)"/>.
        /// </remarks>
        ...
        {
            ThrowIfNull(getDirectoryFuncAsync);
        ...
        /// <summary>
        /// Импортирует все карточки из ресурсов указанной сборки, описанные в файле библиотеки карточек (*.jcardlib).
        /// </summary>
        ...
        /// <param name="cardPredicateAsync">Функция определяющая возможность импорта карточки или значение по умолчанию для типа, если фильтрация не выполняется.</param>
        /// <param name="deleteCardIfExists">Флаг "Удалить карточку, если она уже существует", который будет передан в опции импорта.</param>
        /// <returns>Объект <see cref="TestConfigurationBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public TestConfigurationBuilder ImportCardsWithCardLib(
            ...
            Func<Card, CancellationToken, ValueTask<bool>> cardPredicateAsync = null,
            bool deleteCardIfExists = false
            )
        {
            ThrowIfNull(getCardLibFuncAsync);
                ...
                new PendingAction(
                    ...
                    async (action, cancellationToken) =>
                    {
                        await TestCardHelper.ImportCardsWithCardLibAsync(
                            await this.DbScope.GetDbmsAsync(),
                            this.ResourceAssembly,
        ...
        public TestConfigurationBuilder ImportTypesFromDirectory(
            string directory,
            Func<CardType, CancellationToken, ValueTask> callbackAsync = null)
        {
            ...
            bool clearViews = false)
        {
            var pendingAction = new PendingAction(
                ...
                async (action, ct) =>
                {
                    ...
                    var request = new ViewImportRequest
                    {
                        Models = viewModels,
            ...
            int? timeoutSeconds = null)
        {
            ...
            {
                var pendingAction = new PendingAction(
                    TestHelper.GetCallerMemberFullName(this, scriptFileNames),
                    async (_, ct) =>
                    {
        ...
        /// <summary>
        /// Пересчитывает группы ссылок.
        /// </summary>
        /// <param name="recalcMode"><inheritdoc cref="RefGroupsRecalcMode" path="/summary"/></param>
        /// <param name="ids">Список идентификаторов "Групп ссылок" или "Типов групп ссылок", или значение <see langword="null"/>. Задаваемое значение зависит от параметра <paramref name="recalcMode"/>.</param>
        /// <returns>Объект <see cref="TestConfigurationBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public TestConfigurationBuilder RecalcRefGroups(
            RefGroupsRecalcMode recalcMode = RefGroupsRecalcMode.AllGroups,
            IList<Guid> ids = null)
        {
            this.AddPendingAction(
                new PendingAction(
                    ...
                    async (_, ct) =>
                    {
                        // Пересчёт выполняется только от роли администратора.
                        await using var __ = SessionContext.Create(Session.CreateSystemToken());

                        var request = new CardRequest
                        {
                            RequestType = RefGroupsRequestTypes.RecalculateRefGroups,
                        };
                        request.Info[RefGroupsHelper.ReCalcMode] = Int32Boxes.Box((int) recalcMode);
                        request.Info[RefGroupsHelper.ReCalcIDList] = ids;

                        var response = await this.Dependencies.CardRepository.RequestAsync(request, ct);
        ...
        /// <summary>
        /// Инициализирует подсистему групп. При этом выполняется материализация дескрипторов групп, зарегистрированных в <see cref="IGroupDescriptorRegistry"/>.
        /// </summary>
        /// <returns>Объект <see cref="TestConfigurationBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public TestConfigurationBuilder InitializeGroups()
        {
            this.AddPendingAction(
                new PendingAction(
                    TestHelper.GetCallerMemberFullName(this),
                    async (_, ct) =>
                    {
                        // инициализация групп выполняется только от администратора
                        await using var __ = SessionContext.Create(Session.CreateSystemToken());

                        if (this.Dependencies.ServerSide)
                        {
                            return await NotNullOrThrow(this.groupInitializer).InitializeDescriptorsAsync(cancellationToken: ct);
                        }

                        var response = await this.Dependencies.CardRepository.RequestAsync(new() { RequestType = CardRequestTypes.InitializeGroups }, ct);
                        return response.ValidationResult.Build();
                    }));
            return this;
        }
        ...
        /// <summary>
        /// Инициализирует подсистему настроек.
        /// </summary>
        /// <returns>Объект <see cref="TestConfigurationBuilder"/> для создания цепочки.</returns>
        /// <remarks>
        /// Этот метод реализуется с помощью отложенного выполнения. Для выполнения запрошенного действия необходимо вызвать метод <see cref="IPendingActionsExecutor{T}.GoAsync(Action{ValidationResult}, CancellationToken)"/>.
        /// </remarks>
        public TestConfigurationBuilder InitializeSettingsUnits()
        {
            this.AddPendingAction(
                new PendingAction(
                    TestHelper.GetCallerMemberFullName(this),
                    async (_, ct) =>
                    {
                        // инициализация настроек выполняется только от администратора
                        await using var __ = SessionContext.Create(Session.CreateSystemToken());

                        if (this.Dependencies.ServerSide)
                        {
                            return await NotNullOrThrow(this.settingsUnitInitializer).InitializeAsync(ct);
                        }

                        var response = await this.Dependencies.CardRepository.RequestAsync(new() { RequestType = CardRequestTypes.InitializeSettingsUnits }, ct);
                        return response.ValidationResult.Build();
                    }));
            return this;
        }
        ...
        {
            var clc = new TimeZonesBuilder(this.Dependencies)
                .Create()
                .SetAllowToModify(true)
                .WithDefaultZone();

            foreach (var (zoneID, offsetMinutes) in TestHelper.TestTimeZonesWithoutDefaultTimeZone)
            {
                clc.AddTimeZone(
                    zoneID,
                    offsetMinutes);
            ...
            CancellationToken cancellationToken = default)
        {
            if (this.Dependencies.ServerSide)
            {
                await this.cardMetadataCache.InvalidateGlobalAsync();
            }
            else
            {
                await this.cardCachedMetadata.InvalidateAsync();
            }
