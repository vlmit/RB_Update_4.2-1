        ...
        IWebApplicationFactory
    {
        ...
        public IServiceProvider Services
        {
            get
            {
                ...
                return this.host?.Services ?? this.server.Services;
            }
        }
        ...
        /// <inheritdoc/>
        public async ValueTask InitializeAndStartAsync()
        {
            ...
            await this.OnBeforeHostStartedAsync(hostLocal);
            await hostLocal.StartAsync();
        ...
        /// <summary>
        /// Метод, вызываемый после конфигурации, но перед запуском веб-сервиса <see cref="IHost"/> методом <see cref="IHost.StartAsync"/>.
        /// </summary>
        /// <param name="host">Приложение веб-сервиса.</param>
        /// <returns>Асинхронная задача.</returns>
        protected virtual ValueTask OnBeforeHostStartedAsync(IHost host) => ValueTask.CompletedTask;
        ...
        /// </summary>
        /// <exception cref="InvalidOperationException">Сервер не инициализирован. Пожалуйста, вызовите метод <see cref="InitializeAndStartAsync"/>.</exception>
        protected void CheckInitializeServer()
        {
            if (this.server is null)
            {
                throw new InvalidOperationException($"Server is not initialized. Please call {nameof(this.InitializeAndStartAsync)} method.");
            }
        }

        /// <doc path='info[@type="IDisposable" and @item="CheckDisposed"]'/>
        /// <doc path='exception[@type="ObjectDisposedException"]'/>
        protected void CheckDisposed() => ThrowIfDisposed(this.isDisposed, this);
